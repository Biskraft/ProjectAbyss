...
| :-------- | :----------- | :-------- | :---------------------: | :--------------: | :--: | :----------------------------------- |
| Normal    | 흰색 #FFFFFF | x1.0      | 2                       | 0                | 2    | 2 지층 (4×4 고정)                    |
| Magic     | 파란 #6969FF | x1.3      | 3                       | 0                | 3    | 3 지층 (4×4 고정)                    |
| Rare      | 노란 #FFFF00 | x1.7      | 3                       | 1                | 4    | 3 지층 (4×4 고정)                    |
| Legendary | 주황 #FF8000 | x2.2      | 4                       | 2                | 6    | 4 지층 (4×4 고정)                    |
| Ancient   | 초록 #00FF00 | x3.0      | 5 (4 + 심연)            | 3                | 8    | 4 지층 + 심연 (4×4 고정)             |
...
- **모든 UI는 반드시 `game/docs/ui-components.html` 컴포넌트 가이드를 따른다.** 가이드에 정의된 토큰(색상, 폰트 크기, 여백)만 사용한다. 하드코딩 금지.
- **가이드에 없는 색상/폰트/사이즈가 필요하면, 코드 작성 전에 가이드에 먼저 추가하고 사용자 승인을 받아야 한다.** 승인 없이 임의 값을 사용하지 않는다.
