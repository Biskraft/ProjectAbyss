...
ECHORIS의 GDD 체계가 건강한 상태인지 **6개 레이어**로 검증하고, 발견된 이슈를 분류·보고한다. 1인 개발 컨텍스트를 전제로 하므로 엔터프라이즈급 프로세스가 아니라 실질적 드리프트 탐지에 집중한다. Layer 1-5는 평면 검증(수치·링크·구조·용어·형식), Layer 6은 계층 검증(상위 → 하위 권위 정합)이다.
...
### Layer 6 — 계층 정합성 (상위 → 하위 권위)

`Documents/Terms/Document_Authority_Map.md`(T-07)의 권위 계층을 기준으로, 하위 문서가 상위 문서(Design/Terms)의 캐논을 위배하는지 의미 검증한다. Layer 1-5가 평면(수치·링크·용어·형식)이라면 Layer 6은 계층(원칙·정의·결정)이다. 정규식이 아니라 의미 대조이므로 에이전트가 수행한다.

- 상세 규칙: `references/check_layer6_hierarchy.md` 참조
- 역인덱스/전파: `node Sheets/tools/authority_index.mjs` (부모→자식 + 선언 누락), `--changed <ID>` 또는 `--git` (변경 상위에 종속된 하위 재검증 큐)
- 의미 검증: 각 하위 문서를 상위의 캐논 주장(DEC·확정값·정의·SSoT 선언)과 대조해 모순 플래그
- 위배 정의(T-07 §5): (1) 폐기 개념 사용 (2) 확정값·규칙 불일치 (3) 정의 모순 (4) SSoT 영역 독자 재정의 (5) 상위 변경 후 하위 stale
- 보고: 하위 문서:라인, 위배 진술, 위배된 상위 주장, 권고(하위 수정 또는 상위 재확인)
...
| 요청 예시 | 실행 범위 |
| :--- | :--- |
| "정합성 체크해줘" / "전체 검증" | Layer 1-6 전체 |
| "/sync-check" / "수치 동기화" | Layer 1만 |
| "System_Combat_Action 체크" | 지정 문서에 대해 Layer 1-6 |
| "용어 체크" / "폐기 용어 검색" | Layer 4만 |
| "형식 체크" / "따옴표 체크" | Layer 5만 |
| "계층 체크" / "상위-하위 정합" / "권위 검증" | Layer 6만 (T-07 기준) |
