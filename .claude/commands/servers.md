...
게임 개발에 필요한 서버를 모두 백그라운드로 시작합니다.
...
| HUD Tool | 4330 | `node tools/hud-tool/server.mjs` | `game/` |
...
3000, 4321, 4330, 4331 포트가 이미 사용 중인지 확인한다. 점유 중이면 해당 서버는 스킵.

```powershell
Get-NetTCPConnection -LocalPort 3000,4321,4330,4331 -ErrorAction SilentlyContinue |
...
3. **HUD Tool** (포트 4330 미점유 시):
   - `game/` 디렉토리에서 `node tools/hud-tool/server.mjs` 백그라운드 실행
   - 접속 URL: `http://localhost:4330`

4. **Marketing Kanban** (포트 4321 미점유 시):
   - 프로젝트 루트에서 `node Documents/Plan/marketing/server.js` 백그라운드 실행
   - 접속 URL: `http://localhost:4321`

5. **Spec Canvas** (포트 4331 미점유 시):
...
모든 서버 실행 후 아래 형식으로 보고:

```
게임 서버      http://localhost:3000   ✓ 시작 / ⚡ 이미 실행 중
ASE Watcher    —                       ✓ 시작
HUD Tool       http://localhost:4330   ✓ 시작 / ⚡ 이미 실행 중
...
- 모든 서버 **run_in_background: true** 필수. 동기 대기 금지.
