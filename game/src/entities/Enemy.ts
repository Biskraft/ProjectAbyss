...
/**
 * Chase ???†Ìöå hysteresis + cooldown + Í¥Ä???ïÏ? (?¨Ïö©??Í≤∞Ï†ï 2026-05-04, Q1+Q3).
 * base Enemy.moveTowardTarget ??same-level Î∂ÑÍ∏∞Í∞Ä Îß??ÑÎ†à??dir ???¨Í≥Ñ?∞Ìï¥
 * ?åÎ†à?¥Ïñ¥Í∞Ä Î∞îÎ°ú ??Í≤πÏπú ?ÅÌÉú?êÏÑú sub-pixel ?ÑÏπò Ï∞®Ïù¥Î°?vx Í∞Ä +/- ?§Í?Î©?
 * Ï¢åÏö∞Î°?"?åÎ∞îÎ∞îÎ∞ß" ?®Î¶¨???ÑÏÉÅ Ï∞®Îã®. Î™®Îì† ground ?Å¬∑Î≥¥??Í≥µÌÜµ ?ÅÏö©.
 *
 *   - HYSTERESIS_PX:  ?åÎ†à?¥Ïñ¥Í∞Ä ??Í±∞Î¶¨ ?àÏóê ?àÏúºÎ©?chaseDir ?†Ï?
 *   - COOLDOWN_MS:    ?úÎ≤à ?†Ìöå?????§Ïùå ?†ÌöåÍπåÏ? Í∞ïÏ†ú ?ÄÍ∏?
 *   - PAUSE_MS:       ?†Ìöå ÏßÅÌõÑ vx=0 ?ºÎ°ú ÏßßÍ≤å Î©àÏ∂∞ Î™®ÏÖò ??Í∞ïÏ°∞ (?? ?ÑÎ†à??
 */
...
export abstract class Enemy<S extends string = EnemyState> extends Entity implements CombatEntity {
  ...
  // Tile hazard status (TileHazards.ts HazardTarget ?∏Ìôò).
  ...
  /** Sources that deal 0.5√ó damage. */
  elementResist: Set<ElementAffinity> | null = null;
  /** Sources that deal 1.5√ó damage. */
  ...
  // Player ?Ä ?ôÏùº???òÎ?Î•??†Ï??úÎã§:
  //  - inWater: AABB Ï§ëÏã¨??water ?Ä????
  //  - submerged: Î®∏Î¶¨ÍπåÏ? ?†Í? (Ï§ëÏã¨?êÏÑú 2 ?Ä???ÑÎèÑ water)
  //  - waterTransition: ?¥Î≤à ?ÑÎ†à?ÑÏùò enter(+1) / exit(-1) / none(0)
  inWater = false;
  submerged = false;
  waterTransition: 0 | 1 | -1 = 0;
  private prevInWater = false;

  // Ground/jump transition events ??Player ?Ä ?ôÏùº??consume ?®ÌÑ¥.
  //  - landedFallSpeed: ?¥Î≤à ?ÑÎ†à?ÑÏóê Ï∞©Ï??àÏúºÎ©?|?¥Ï†Ñ vy|, ?ÑÎãàÎ©?null
  //  - jumpedThisFrame: ?¥Î≤à ?ÑÎ†à?ÑÏóê grounded ?êÏÑú ?¥Î•ô?àÍ≥† vy < 0
  ...
  // Chase ?†Ìöå ?ÅÌÉú ??base moveTowardTarget ??same-level Î∂ÑÍ∏∞?êÏÑú ?¨Ïö©.
  /** ?ÑÏû¨ ?†Í∏¥ ?òÌèâ Ï∂îÍ≤© Î∞©Ìñ•. Îß??ÑÎ†à???¨Í≥Ñ?∞ÌïòÏßÄ ?äÍ≥† hysteresis + cooldown ?ºÎ°ú Í∞±Ïã†. */
  protected chaseDir: 1 | -1 = 1;
  /** ?§Ïùå ?†Ìöå Í∞Ä?•ÍπåÏßÄ ?®Ï? ?úÍ∞Ñ (ms). */
  protected turnCooldownMs = 0;
  /** ?†Ìöå ÏßÅÌõÑ vx=0 ?ºÎ°ú Î©àÏ∂î???îÏó¨ ?úÍ∞Ñ (ms). Î™®ÏÖò ?? */
  ...
  /**
   * ?êÏãù ?¥Îûò?§Í? own PNG/Atlas Sprite Î•??±Î°ù?òÎ©¥ hit flash Í∞Ä Í∑?sprite ??
   * ?åÌåå Ï±ÑÎÑê Î™®Ïñë Í∑∏Î?Î°??∞ÏÉâ?ºÎ°ú Î∞úÍ¥ë (?¨Ïö©??Í≤∞Ï†ï 2026-05-04). ÎØ∏Îì±Î°ùÏù¥Î©?
   * Í∏∞Ï°¥ Graphics rect fallback.
   */
  ...
  update(dt: number): void {
    ...
    // Facing ??chase / attack / cooldown / hit / detect ?ôÏïà chaseDir Î°??†Í∏à.
    // target.x ÏßÅÏ†ë Ï∂îÏ†Å?Ä player Í∞Ä Í∞ÄÍπåÏù¥??Ï¢åÏö∞Î°??ÄÏßÅÏù¥Í±∞ÎÇò ?ÑÎ°ú ?êÌîÑ?òÎ©¥
    // dx Î∂Ä?∏Í? Îß??ÑÎ†à???§Ïßë?Ä "ÎπôÍ?ÎπôÍ?" facing ÍπúÎπ°???†Î∞ú.
    // patrol ?Ä subclass Í∞Ä super ?∏Ï∂ú ??patrolDir Î°???ñ¥?¥Îã§ (Skeleton).
    if (this.target) {
    ...
    // Water/submerged ?ÅÌÉú + enter/exit ?ÑÏù¥ Í∏∞Î°ù ???¨Ïóê??Îß??ÑÎ†à??enemy Î•?
    // ?úÌöå?òÎ©∞ VFX(WaterSplash / WaterBubbles) Î•??∏Î¶¨Í±∞Ìï† ???¨Ïö©.
    if (this.roomData.length > 0) {
      this.inWater = isInWater(this.x, this.y, this.width, this.height, this.roomData);
      // Î®∏Î¶¨ Î∂Ä??Ï§ëÏã¨?êÏÑú -2 ?Ä?? ??water Î©?submerged
    ...
    // Land/jump ?ÑÏù¥ Í∞êÏ? (flying ?Ä grounded Í∞Ä ??ÉÅ false ?¥Î?Î°??êÏó∞?§ÎüΩÍ≤??úÏô∏??
  ...
  render(alpha: number): void {
    if (!this.container.destroyed) {
      ...
      // mainSprite ?±Î°ù ??Í∞ôÏ? texture ??Sprite overlay + blendMode 'add' Î°?
      // ?åÌåå Ï±ÑÎÑê Î™®Ïñë Í∑∏Î?Î°??∞ÏÉâ Î∞úÍ¥ë. ÎØ∏Îì±Î°ùÏù¥Î©?Í∏∞Ï°¥ Graphics rect fallback.
      if (this.flashTimer > 0) {
        ...
        if (this.mainSprite) {
          ...
          // Îß??ÑÎ†à??Í∞±Ïã† ??atlas frame Î≥ÄÍ≤?+ facing flip Ï∂îÏ†Å.
  ...
  protected moveTowardTarget(speed: number): void {
    ...
    // Flying enemies: direct XY movement toward target (¬ß2.4)
    if (this.movementType === 'flying') {
      this.moveTowardTargetFlying(speed);
      return;
    }

    // Ground enemies ??Í≥µÏ§ë(?êÌîÑ/?ôÌïò ?ÑÏ§ë)?êÏÑ† Ï∂îÍ≤© ?¨Í≥Ñ???§ÌÇµ.
    // ???òÎäî ?êÌîÑ ?ôÏïà player ?Ä??X Ï∞®Ïù¥Í∞Ä Î∂Ä??ÏßÑÎèô?òÎ©¥ chaseDir ??Îß??ÑÎ†à??
    // ?§Ïßë?Ä Ï¢åÏö∞Î°??®Î¶¨???ÑÏÉÅ (?¨Ïö©??Í≤∞Ï†ï 2026-05-08). ?¥Î•ô ??vx/chaseDir
    // ??Í≥†Ï†ï??Ï±ÑÎ°ú ?¨Î¨º???¥ÎèôÎß??òÌñâ, Ï∞©Ï? ???§Ïãú Ï∂îÍ≤© ?âÍ?.
    if (!this.grounded) return;

    // Ground enemies: vertical chase rules (¬ß2.2-A)
    ...
    if (heightDiff > HEIGHT_THRESHOLD) {
      // Player is below ??find floor gap and drop (¬ß2.2-A Case 2)
      ...
      // Same level ??horizontal chase (¬ß2.2-A Case 3) + ?†Ìöå hysteresis + cooldown + pause.
      // ?†Ìöå ÏßÅÌõÑ turnPauseMs ?ôÏïà vx=0 ?ºÎ°ú ÏßßÍ≤å Î©àÏ∂§ (Î™®ÏÖò ??Í∞ïÏ°∞ + ?®Î¶º Ï∞®Îã®).
  ...
  /**
   * Subclass ??chase state enter() ?êÏÑú ?∏Ï∂ú Í∂åÏû•. chaseDir ??Ï¶âÏãú target
   * ?ÑÏπò Í∏∞Ï??ºÎ°ú ?°Í≥† cooldown/pause 0 ??ÏßÑÏûÖ Ï≤??ÑÎ†à?ÑÏóê ?òÎèÑÏπ??äÏ?
   * vx=0 pause Í∞Ä Î∞úÏÉù?òÏ? ?äÎèÑÎ°?
   */
