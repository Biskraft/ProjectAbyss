 ...
 *  20 = cyro (passable, light DoT + Frozen status: ?¥Îèô -75% / 3s) [V2.4 2026-05-18, Iron primary signature.
 *           LDtk Editor enum value: SSoT ??Editor ?¨Ï†ï????cyro=20 ?ºÎ°ú Í≥†Ï†ï.
 *           Spelled "cyro" not "cryo" ??LDtk Cyro enum ?úÍ∏∞?Ä ÏΩîÎìúÎ≤†Ïù¥?§Í? mirror.]
...
// TILE_CYRO 2026-05-18: 14 ??20 (LDtk SSoT ?Ä ?ïÎ†¨, Editor ?¨Ï†ï????cyro=20).
...
/** Check if an entity AABB overlaps any cryo tile (any cell = Frozen ?ÅÌÉú?¥ÏÉÅ + light DOT). */
...
export function resolveY(
  ...
  if (vy === 0) {
    ...
    // flicker where player cycles ground?íair?íground every frame).
    const feetRow = Math.floor((y + height) / TILE_SIZE);
    const leftCol = Math.floor(x / TILE_SIZE);
    const rightCol = Math.floor((x + width - 1) / TILE_SIZE);
    for (let col = leftCol; col <= rightCol; col++) {
...
/**
 * Debug ?ÑÏö© ???Ä Î≤îÏúÑ [c0..c1] √ó [r0..r1] ?Ä Í≤πÏπò??Î™®Îì† Í∞Ä??2x1 ?¨Î°ú???∏Í∑∏Î®ºÌä∏Î•?
 * ?òÏßë?úÎã§. Í∞?(startCol, lowRow) Ï°∞Ìï©??upRight/upLeft ?ÑÎ≥¥Î•?1?åÏî©Îß??úÎèÑ?òÎ?Î°?
 * Ï§ëÎ≥µ?Ä Î∞úÏÉù?òÏ? ?äÎäî?? CollisionDebugOverlay ??Í≤ΩÏÇ¨Î©??úÏãú???¨Ïö©.
 */
export function collectSlopes2x1(
  roomData: number[][], c0: number, r0: number, c1: number, r1: number,
): SlopeSegment2x1[] {
  const out: SlopeSegment2x1[] = [];
  // startCol ?Ä ?∏Í∑∏Î®ºÌä∏ Ï¢åÏ∏° ?Ä ??Ï¢åÏ∏° 2Ïπ??®Îî©?ºÎ°ú Î∂ÄÎ∂?Í∞Ä???¨Î°ú?ÑÎèÑ ?¨Ìï®.
...
/**
 * Dash corner correction ???Ä??Ï§??òÌèâ ÏßÑÌñâ??Î≤ΩÏóê ÎßâÌûê ???∏Î°úÎ°??¥Ïßù Î∞Ä???µÍ≥º?úÌÇ§??Î≥¥Ï†ï.
 *
 * ?Ä?úÎäî ?òÌèâ(vy=0) ?ºÎ°ú ?ÑÏßÑ?òÎ?Î°? ÏßÑÌñâ Î∞©Ìñ• leading Ïª¨Îüº???åÎ†à?¥Ïñ¥ ?∏Î°ú ?§Ìå¨??
 * ?îÎ¶¨?úÍ? "?úÏ™Ω ??top-only or bottom-only)" Îß??àÍ≥† overlap ??tolerance ?¥ÎÇ¥Î©?
 * Í∑?Î∞òÎ? Î∞©Ìñ•?ºÎ°ú Î∞Ä??ÏßÄ?òÍ?Í≤??úÎã§. ?ëÏ™Ω ??ÎßâÌòîÍ±∞ÎÇò Ï§ëÍ∞Ñ??ÎßâÌ? ?àÏúºÎ©?ÎØ∏Îèô??
 *
 * ?ÅÏö© ?Ä?? ?Ä???ÅÌÉú ?ÑÏö©. ?ºÎ∞ò ?òÌèâ ?¥Îèô?êÎäî tryLedgeSnap ???¥Îã§ (Î∞©Ìñ•???∏ÎèÑ).
 * Returns: Î≥¥Ï†ï??y, ?êÎäî null (Î≥¥Ï†ï Î∂àÍ?).
 */
