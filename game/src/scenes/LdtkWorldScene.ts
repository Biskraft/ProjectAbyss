...
import { WorldFallingHazardRegistry } from './world/WorldFallingHazardRegistry';
import { WorldFallingHazardRuntime } from './world/WorldFallingHazardRuntime';
...
// RetireAfterFirstBoss field ?? u ??? ???ï¿½ï¿½? ????????? ??u.
...
export class LdtkWorldScene extends Scene {
  ...
  private readonly worldFallingHazardRegistry = new WorldFallingHazardRegistry();
  private worldFallingHazardRuntime!: WorldFallingHazardRuntime;
  ...
  /** Pattern D (proximity-interaction) ? ???? ?????? ?ï¿½ï¿½???? ?????. */
  ...
  private wireTerrainRuntimes(): void {
    ...
    this.worldFallingHazardRuntime = new WorldFallingHazardRuntime({
      game: this.game,
      getPlayer: () => this.player,
      getEnemies: () => this.worldEnemyRegistry.enemies,
      getCollisionGrid: () => this.collisionGridRuntime.grid,
      getEntityLayer: () => this.entityLayer,
      getRegistry: () => this.worldFallingHazardRegistry,
      getHud: () => this.hud,
      getScreenFlash: () => this.combatFeedbackRuntime.screenFlash,
      getDamageNumbers: () => this.combatFeedbackRuntime.damageNumbers,
      getLandingDust: () => this.movementVfxRuntime.landingDust,
  ...
  private wireItemWorldFlowRuntimes(): void {
    ...
    this.itemWorldSceneTransitionRuntime = new WorldItemWorldSceneTransitionRuntime({
      ...
      detachSharedUiForItemWorld: () => {
        ...
        // ???? visible=true ?? a?? ?ï¿½ï¿½? attach ???? ?????? ??????? ???? ????????? detach.
    ...
    this.deployBlurRuntime = new WorldDeployBlurRuntime({
      getTargets: () => compactContainers([
        ...
        this.fluidLayer,
        this.deploymentFxLayer,
        ...this.itemWorldGhostStream.getVisualTargets(),
      ]),
    });
  ...
  private wireCombatAndTransitionRuntimes(): void {
    ...
    this.prologueEndRuntime = new WorldPrologueEndRuntime({
      ...
      isWakeMovementPressed: () => this.game.input.isDown(GameAction.MOVE_LEFT) || this.game.input.isDown(GameAction.MOVE_RIGHT) || this.game.input.isRawKeyDown('ArrowLeft') || this.game.input.isRawKeyDown('ArrowRight') || this.game.input.isRawKeyDown('KeyA') || this.game.input.isRawKeyDown('KeyD'),
  ...
  async init(): Promise<void> {
    ...
    // cache:'no-store' + cache-bust query ?? ï¿½ï¿½?o? ?? ??????(?????? / Vite / SW / proxy)
    // ?? ????? ??? ??? .ldtk ?? ??ï¿½ï¿½?. prod ??????? ???? (init 1?).
    ...
    // Arc Tether ? Spark ??ï¿½ï¿½??? ?????? ???? ??? VFX. Player layer ??, entityLayer ??
    // ???? ?ï¿½ï¿½? player ???? *???* add (????? ??? ??ï¿½ï¿½ï¿½ï¿½ player ???? ???????).
    ...
    // input ??????? SELECT KEYBOARD ?? ??? preset ?? ??ï¿½ï¿½?.
    ...
    // Inventory UI ? uiContainer(native) ???. InventoryUI ???ï¿½ï¿½??? scale.set(uiScale)
    // ???? 640 ???? ???????? ??? ?ï¿½ï¿½? ???????? (UI native ???????? ?????? 1??).
  ...
  enter(): void {
    ...
    // ???? BGM ? intro 1? ?? loop. 5?? fade-in ???? ?ï¿½ï¿½ï¿½ï¿½?? ??????.
    ...
    if (this.itemWorldEntryState.consumeWorldVisualsReleased()) {
      ...
      // ?? ???? placedItem sprite ?? ?????? ?ï¿½ï¿½?. ?????? + ?????? ???? (disabled bypass).
  ...
  update(dt: number): void {
    ...
    if (this.itemWorldEntryState.isDeploymentBlocking()) {
      ...
      this.player.updateWithSuppressedInput(dt);
      this.game.camera.update(dt);
      this.worldWeatherRuntime.update(dt);
      this.combatFeedbackRuntime.updateImpactOnly(dt);
      return;
    ...
    // Shift+M ?? ?? handleDebugWarp ?? ???? consume ???? ??? M ???? ?ï¿½ï¿½???? ??ï¿½ï¿½?.
    ...
    // ??? ??? (2026-05-03): u IW ???? ?????? ?ï¿½ï¿½??? ?????? ???? INVENTORY
    // ??? ?????? Rustborn ???? ????? Ego ??? ??? 'Locked' ?ï¿½ï¿½??? ????.
    ...
    if (this.inventoryUI.visible) {
      ...
      if (inventoryResult === 'confirmed_equipment_change') {
        ...
        // 2026-05-18: ?ï¿½ï¿½??? hint ?? *Rustborn ??? ?????? ?????? ????*???? dismiss.
        // ?? ????? ???? ??u?????ï¿½ï¿½? ???? ??????? ??? ?????? ??????? hint ?? ???????.
        // "Rustborn ????" ?? ??? ???????? ?? ?? ????ï¿½ï¿½? ???? ??ï¿½ï¿½?.
        this.inventoryTutorialHint.clearIfRustbornEquipped();
      }
    ...
    // Pattern D (proximity-interaction): ???? ?????? ?ï¿½ï¿½???? o??.
    // ???? player.update() ???? ????? ? ?? ???? ????????? ???????
    ...
    if (prologuePlayerLocked) {
      ...
      } else if (this.prologueEndRuntime.shouldHoldWakeUpPose) {
    ...
    this.worldFallingHazardRuntime.update(dt);
    ...
    //   4) ???? ?????? pullingContainer=null, arcTether ?? hold ?? ???ï¿½ï¿½?.
    // ????? ??? ??? ???????? pull ?????? ????????? ??? ?????? ???.
  ...
  private updateMovementVfx(dt: number): void {
    ...
    if (p.consumeDropThroughEvent()) {
      ...
      // ???? u drop-through ???ï¿½ï¿½? ???? hint ?? ??? ??????? 1?? ?? fade,
      // ???ï¿½ï¿½??? handled flag ?? set.
  ...
  render(alpha: number): void {
    ...
    if (this.game.transitionDirector.isActive && !this.dungeonAtmosphereRuntime.isActive) {
      const cam = this.game.camera;
      this.parallaxBG.updateScroll(cam.renderX, cam.renderY);
    }
  ...
  private loadLevel(levelId: string, enterDirection: 'left' | 'right' | 'up' | 'down'): boolean {
    ...
    this.worldFallingHazardRuntime.spawn(level);
   ...
   * ???? ???? (n/s): row 0 ??? gridH-1 ???? ?ï¿½ï¿½?? passable ???? ??????.
