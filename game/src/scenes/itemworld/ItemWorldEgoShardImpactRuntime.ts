...
export class ItemWorldEgoShardImpactRuntime {
  ...
  handleImpact(px: number, py: number, element: ShardElement): void {
    const room = this.deps.getFullGrid();
  ...
  debugIgniteAtPlayer(): void {
    ...
    Debug.log(`[DebugFire] actions=${actions} burning=${tileMutator.burningCount} residueIgnited=${residueIgnited}`);
  }
  ...
  debugFreezeAtPlayer(): void {
    ...
    Debug.log(`[DebugIce] frozen=${frozen} total=${this.deps.getTileMutator().frozenCount}`);
  }
  ...
  debugThunderAtPlayer(): void {
    ...
    Debug.log(`[DebugThunder] lit=${totalLit} electric=${this.deps.getTileMutator().electricCount}`);
  }
  ...
  private igniteGrassClumps(cells: Array<[number, number]>): void {
    ...
    this.deps.getGrassClumpFire().igniteInCellAABB(minGx, minGy, maxGx, maxGy);
  }
  ...
  private forEachCellInAABB(
    ...
    for (let gy = top; gy <= bottom; gy++) {
      for (let gx = left; gx <= right; gx++) cb(gx, gy);
    }
  }
}
