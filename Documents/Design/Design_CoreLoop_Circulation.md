...
> **설계 원칙:** 자동 사냥은 직접 클리어한 층위/층에서만 해금된다. 정복하지 않은 콘텐츠를 자동화할 수 없다.
...
| 근거 | 설명 |
| :--- | :--- |
| 발견의 가치 보존 | 새 층위를 처음 밟는 경험은 단 한 번뿐이다. 이 경험을 자동화하면 탐험 기둥의 핵심 판타지("탐험가")가 소멸한다 |
| 성취의 순서 보장 | 수동 정복이라는 도전을 거쳐야 자동 사냥이라는 편의를 얻는다. 도전 없는 편의는 성취감을 훼손한다 |
| 순환 동력 유지 | 자동 사냥이 미정복 층위에서도 가능하면, 플레이어는 직접 탐험할 동기를 잃고 순환이 정지한다 |
...
| 역할 | 설명 |
| :--- | :--- |
| 순환 가속 | 이미 정복한 층위에서 소재/골드를 자동 수집하여, 다음 순환 진입 시간을 단축한다 |
| 순환 방향 결정 불가 | 새로운 층위 탐험, 새로운 보스 도전, 새로운 아이템계 진입은 반드시 수동으로만 가능하다 |
