...
| :--- | :--- | :--- | :---: | :--- | :--- |
| D-02-01 | 대순환 | 월드-아이템계-월드 순환 정의 | P0 | 대기 | 핵심 루프 |
| D-02-02 | 소순환 | 공간별 내부 미니 루프 정의 | P0 | 대기 | 3-Space 각 공간 |
| D-02-03 | 감정 설계 | 순환 고리별 감정 곡선 | P1 | 대기 | UX 연계 |
| D-02-04 | 안티패턴 | 순환 단절 방지 메커니즘 | P1 | 대기 | 밸런스 연계 |
| D-02-05 | 세션 설계 | 세션 길이별 순환 경로 | P2 | 대기 | 크로스 플랫폼 |
...
| 기둥 | Core Loop에서의 역할 |
| :--- | :--- |
| 메트로베니아 탐험 | 순환의 시작점이자 종착점. 새 구역 탐험이 순환을 기동시키고, 강화된 장비로 돌아오는 것이 순환을 완성한다 |
| 아이템계 야리코미 | 순환의 엔진. 아이템계에서의 강화 활동이 다음 탐험을 가능하게 하는 동력을 생산한다 |
| 온라인 멀티플레이 | 순환의 가속기. 파티 플레이가 아이템계 깊은 층 공략을 현실적으로 만들어 순환 속도를 높인다 |
...
| 참고 게임 | 재미 요소 |
| :--- | :--- |
| 월하의 야상곡 | 능력 획득 후 이전에 갈 수 없던 구역에 도달하는 순간의 쾌감 |
| 디스가이아 | 아이템계 100층을 돌파하며 장비가 점진적으로 강해지는 야리코미 만족감 |
| 디아블로 시리즈 | 더 좋은 장비를 위해 더 높은 난이도에 도전하는 파밍 루프 |

### 2-2. 분석 (Analyze)

세 게임의 순환 구조가 성공하는 이유는 동일하다. 플레이어가 현재 순환 고리에서 획득한 보상이 다음 순환 고리의 진입 조건이 된다는 점이다. 월하의 야상곡에서 능력은 다음 구역의 열쇠이고, 디스가이아에서 아이템 레벨은 더 깊은 층의 생존 조건이며, 디아블로에서 장비는 더 높은 토먼트의 입장권이다.
...
graph TD
    ...
    B -->|층 클리어, 이노센트 수집| C["장비 강화<br/>(성장의 쾌감)"]
...
| :--- | :--- | :--- | :--- | :--- |
| 1. 탐험 | 미지의 구역을 돌아다니며 맵을 밝힌다 | World | 장비 아이템, 월드 소재 | 획득한 장비가 아이템계 진입의 소재가 된다 |
| 2. 진입 | 획득한 장비의 아이템계에 들어간다 | Item World | 아이템 EXP, 이노센트 | 층 클리어가 장비 강화로 이어진다 |
| 3. 강화 | 층을 클리어하고 이노센트를 수집한다 | Item World | 스탯 상승, 이노센트 복종 | 장비 스탯이 스탯 게이트 임계값에 도달한다 |
| 4. 해금 | 강화된 스탯으로 게이트를 통과한다 | World | 새 구역 접근권 | 새 구역에서 더 높은 등급의 아이템이 나온다 |
...
| 아이템계 -> 월드 | 왜 아이템계에서 나오는가? | 강화된 스탯으로 해금되는 새 구역이 있고, 그곳에 더 좋은 장비가 있기 때문 |
...
```mermaid
graph LR
    I1["층 진입"] --> I2["적/이노센트 처치"]
    I2 --> I3["아이템 EXP 획득"]
    I3 --> I4["다음 층 이동 or 탈출 판단"]
    I4 -->|계속| I1
    I4 -->|탈출| I5["성과 확정"]
```
...
| 소순환 요소 | 감정 | 리스크 | 리턴 |
| :--- | :--- | :--- | :--- |
| 층 진입 | 집중, 긴장 | 전멸 시 진행 손실 | 아이템 EXP 누적 |
| 이노센트 조우 | 흥분, 기대 | 야생 이노센트는 강력한 적 | 복종 시 스탯 보너스 2배 |
| 탈출 판단 | 고민, 욕심 vs 안전 | 더 가면 보상이 크지만 전멸 위험 | Mr. Gency's Exit 사용 여부 |

아이템계 소순환의 핵심은 "더 가느냐, 여기서 탈출하느냐"의 리스크-리턴 판단이다. 이 판단이 매 10층마다 반복되며, 야리코미의 긴장감을 유지한다.
...
| 획득 -> 강화 | 가능성의 흥분 | "이 아이템을 어디까지 키울 수 있을까?" | 아이템계 미리보기(최대 층수, 보스 정보), 이노센트 슬롯 확인 |
| 강화 -> 해금 | 성취의 쾌감 | "드디어 그 벽을 넘었다!" | 스탯 게이트 해금 연출(봉인 해제 이펙트, 전용 SFX) |
| 해금 -> 탐험 | 새로운 시작 | "저 너머에 뭐가 있을까?" | 새 구역 진입 시 맵 안개 해제, 새 BGM 전환, 미지의 풍경 노출 |
...
[보스 처치: 성취] --> [아이템 확인: 흥분] --> [아이템계 진입: 집중]
    |
    v
[층 클리어 반복: 몰입] --> [깊은 층: 긴장/위기] --> [탈출 판단: 고민]
    |
    v
[성과 확정: 만족] --> [스탯 게이트 해금: 쾌감] --> [새 구역 진입: 경이]
    |
    v
[허브 복귀: 안도/자랑] --> 다음 순환
```
...
| 반복-돌파 대비 | 아이템계 층 클리어 반복(루틴) 중 이노센트 조우(돌발) |
...
| 항목 | 내용 |
| :--- | :--- |
| 증상 | 다음 구역의 스탯 게이트 임계값에 한참 미달. 월드에서 할 수 있는 것이 없다고 느낌 |
| 원인 | 스탯 게이트 임계값과 현재 구역에서 획득 가능한 장비의 잠재력 사이 간극이 과도함 |
| 해결책 | 아이템계로의 유도. HUD에 "이 장비를 아이템계에서 강화하면 게이트를 해금할 수 있습니다" 안내 표시. 현재 보유 장비의 아이템계 진입이 스탯 게이트 해금의 최단 경로가 되도록 수치 설계 |
| 검증 기준 | 현재 구역 최고 등급 장비의 아이템계를 적정 층수까지 클리어하면 다음 스탯 게이트를 통과할 수 있어야 한다 |
...
| 해결책 | 월드 전용 보상 설계. 능력 렐릭은 월드에서만 획득 가능하고, 능력 게이트 너머의 아이템계 진입 소재(고급 장비)는 월드에서만 드랍된다. 아이템계의 특정 층 이상 진입에 월드 소재가 필요한 잠금 장치를 설정한다 |
...
| 해결책 | 전생(Reincarnation) 시스템으로 제2의 성장 곡선 제공. 장비를 전생시키면 레벨과 스탯이 리셋되지만, 잠재력 상한이 상승하여 이전보다 더 높은 최종 스탯에 도달할 수 있다. 전생 횟수에 따라 외형 변화(아우라, 이펙트)를 부여하여 시각적 성취감을 제공한다 |
| 검증 기준 | 최고 레벨 장비에도 전생을 통한 추가 성장 여지가 남아있어야 한다 |

### 6-4. 멀티플레이 접근 장벽

| 항목 | 내용 |
| :--- | :--- |
| 증상 | 깊은 층 아이템계는 파티가 필요하지만, 파티를 구하기 어려움 |
...
| 검증 기준 | 솔로 플레이어도 아이템계 100층까지 도달할 수 있어야 한다 (난이도는 높지만 불가능하지 않음) |

---

## 7. 수동 정복 선행 원칙의 철학 (Manual Conquest First)

### 7-1. 원칙 정의

> **설계 원칙:** 자동 사냥은 직접 클리어한 구역/층에서만 해금된다. 정복하지 않은 콘텐츠를 자동화할 수 없다.

### 7-2. 철학적 근거

이 원칙은 순환 구조의 건강성을 보호하는 안전장치이다.

| 근거 | 설명 |
| :--- | :--- |
| 발견의 가치 보존 | 새 구역을 처음 밟는 경험은 단 한 번뿐이다. 이 경험을 자동화하면 탐험 기둥의 핵심 판타지("탐험가")가 소멸한다 |
| 성취의 순서 보장 | 수동 정복이라는 도전을 거쳐야 자동 사냥이라는 편의를 얻는다. 도전 없는 편의는 성취감을 훼손한다 |
| 순환 동력 유지 | 자동 사냥이 미정복 구역에서도 가능하면, 플레이어는 직접 탐험할 동기를 잃고 순환이 정지한다 |
| 직접 플레이의 가치 | 자동 사냥 효율에 상한을 두어(`System_Economy_AutoHunt.md` 참조), 직접 플레이가 항상 더 효율적이도록 설계한다 |

### 7-3. 자동 사냥과 순환의 관계

자동 사냥은 순환의 "가속 페달"이지 "핸들"이 아니다.

| 역할 | 설명 |
| :--- | :--- |
| 순환 가속 | 이미 정복한 구역에서 소재/골드를 자동 수집하여, 다음 순환 진입 시간을 단축한다 |
| 순환 방향 결정 불가 | 새로운 구역 탐험, 새로운 보스 도전, 새로운 아이템계 진입은 반드시 수동으로만 가능하다 |

---

## 8. 세션 길이별 순환 설계 (Session-Based Loop Design)
...
| 항목 | 내용 |
| :--- | :--- |
| 추천 활동 | 아이템계 10층 단위 도전, 허브 이노센트 관리, 자동 사냥 수확 |
| 순환 경험 | 소순환 1회 (아이템계 10층 클리어 -> 보스 처치 -> 아이템 EXP 확정) |
| 성취감 보장 | 10층마다 보스 처치 + 세이브 포인트. 15분 안에 최소 1회의 명확한 성과를 보장한다 |
| 플랫폼 | 모바일 통근/점심시간 |
...
| 항목 | 내용 |
| :--- | :--- |
| 추천 활동 | 아이템계 30층 연속 도전, 월드 새 구역 진입 시도, 파티 매칭 아이템계 |
...
| 항목 | 내용 |
| :--- | :--- |
| 추천 활동 | 대순환 완주. 월드 탐험 -> 아이템 획득 -> 아이템계 깊은 층 도전 -> 스탯 게이트 해금 -> 새 구역 진입 |
| 순환 경험 | 대순환 1-2회 완주 |
| 성취감 보장 | 새 구역 진입 또는 레어리티 승급 같은 질적 변화. 양적 성장(숫자 증가)이 아닌 질적 전환(새 경험)을 제공한다 |
| 플랫폼 | PC 주말 |
...
| 메커니즘 | 설명 |
| :--- | :--- |
| 목표 큐 시스템 | HUD에 현재 가장 가까운 스탯 게이트, 다음 아이템계 보스 층, 미탐험 구역을 표시한다 |
...
| :--- | :--- | :--- |
| World | 순환의 시작과 종결. 탐험 -> 획득 -> 해금 -> 재탐험 | 장비 획득 시 아이템계 진입 유도, 스탯 게이트 도달 시 다음 구역 안내 |
| Item World | 순환의 엔진. 획득 -> 강화 | 보스 층 클리어 후 탈출 유도(월드 귀환), 깊은 층에서 파티 매칭 유도(허브) |
...
| Item World -> World | 보스 층 클리어 후 "스탯 게이트 해금 가능" 알림 + 월드 귀환 옵션 | 100층까지 가야만 탈출할 수 있는 구조 |
...
| 해결 방향 | 능력 게이트로 열리는 경로(자유)와 스탯 게이트로 열리는 경로(성장)를 분리 배치. 모든 구역에 능력 게이트만으로 도달 가능한 최소 경로(Critical Path)가 존재하되, 스탯 게이트 너머에 더 좋은 보상을 배치한다 |
| 희생 | 완전한 탐험 자유를 일부 희생. 스탯 게이트 구역은 탐험 범위의 보너스 영역으로 위치시킨다 |
...
| 해결 방향 | 아이템계의 의무 구간(메인 순환에 필요한 최소 층수)과 선택 구간(야리코미 극한 도전)을 분리. 스탯 게이트 해금에 필요한 강화는 아이템계의 중간 층수에서 달성 가능하게 설계하고, 100층 및 재귀적 진입은 야리코미 전용 보상으로 유인한다 |
| 희생 | 야리코미를 완수하지 않아도 메인 스토리 진행이 가능. 극한 성장은 선택적 쾌감으로 존재한다 |

### 10-3. 자동 사냥 편의 vs 직접 플레이 동기

| 항목 | 내용 |
| :--- | :--- |
| 상충 | 자동 사냥은 바쁜 플레이어에게 편의를 제공하지만, 직접 플레이의 가치를 훼손할 수 있다 |
| 해결 방향 | 수동 정복 선행 원칙(섹션 7) + 자동 사냥 효율 상한 설정. 자동 사냥은 직접 플레이 대비 낮은 효율(`AutoHunt_Efficiency_Cap` 참조)로 제한하며, Legendary 이상 등급 아이템과 이노센트는 직접 플레이에서만 획득 가능하다 |
| 희생 | 완전한 자동화를 희생. 자동 사냥은 보조 수단이지 주 플레이 방식이 될 수 없다 |
...
- [ ] 수동 정복 선행 원칙이 자동 사냥 관련 모든 시스템에 적용되어 있는가?
