...
본 문서는 ECHORIS의 핵심 순환 구조(Core Loop)가 어떤 철학 위에 설계되었는지를 정의한다. 순환의 각 고리가 플레이어에게 부여해야 할 감정, 순환이 끊기지 않기 위한 원칙, 그리고 세션 길이에 따른 순환 경로를 다룬다.

### 1-2. 한 줄 요약

> **(DEC-046, 2026-05-24 재정의)** "탐험하면 한 사람의 잊혀진 무기가 발견되고, 그 사람의 인생을 끝까지 복원하면 그 무기 본래의 힘이 깨어나 새로운 세계가 열린다. 다음 사람의 이야기를 보기 위해 플레이어는 떠나지 않는다."
>
> 이전 정의(*"아이템을 키우면"*) 의 *키운다* 는 어휘는 폐기. *복원* / *발견* / *회수* 로 대체.
...
ECHORIS는 이 본질을 이중 게이트(능력 게이트 + 스탯 게이트)로 재구축한다.

| 기존 방식 | ECHORIS의 독창적 변형 |
| :--- | :--- |
...
graph TD
    ...
    B -->|지층 클리어, 기억 단편 수집| C["장비 강화<br/>(성장의 쾌감)"]
...
| 2. 진입 | 획득한 장비의 아이템계에 들어간다 | Item World | 아이템 EXP, 기억 단편 | 지층 클리어가 장비 강화로 이어진다 |
| 3. 강화 | 지층을 클리어하고 기억 단편를 수집한다 | Item World | 스탯 상승, 기억 단편 복종 | 장비 스탯이 스탯 게이트 임계값에 도달한다 |
...
| 전환점 | 동기 질문 | 정답 (DEC-046) |
| :--- | :--- | :--- |
| 탐험 -> 아이템계 | 왜 아이템계에 들어가는가? | 이 무기 속 한 사람의 인생이 누구의 것인지 알아내기 위해. 부수적으로, 그 인생을 복원하면 본래의 힘이 깨어나 다음 게이트가 열린다 |
| 아이템계 -> 월드 | 왜 아이템계에서 나오는가? | 한 인생을 끝까지 보았고, 그 무기 본래의 힘이 새 층위를 열어주기 때문. 새 층위에서 다음 사람의 무기를 만날 수 있기 때문 |
| 월드 -> 세이브 포인트 | 왜 세이브 포인트에 가는가? | Identity Archive에서 복원된 인물들을 정리하고, 다음으로 만나고 싶은 인물 카테고리를 결정하기 위해. 부수적으로 소모품 보충 |
...
| 기억 단편 조우 | 흥분, 기대 | 야생 기억 단편는 강력한 적 | 복종 시 스탯 보너스 2배 |
...
```mermaid
graph LR
    H1["세이브 포인트 도착"] --> H2["장비 정리<br/>(기억 단편 배치)"]
    H2 --> H3["시설 이용<br/>(상점, 대장간)"]
    H3 --> H4["다음 목표 설정<br/>(월드 탐험 or 아이템계)"]
    H4 --> H5["출발"]
```

세이브 포인트 소순환은 전투가 없는 준비와 휴식의 공간이다. 세이브 포인트에서 보내는 시간이 다음 순환의 효율을 높이므로, 세이브 포인트 방문 자체가 순환의 일부로 기능한다.
...
| 획득 -> 강화 | 가능성의 흥분 | "이 아이템을 어디까지 키울 수 있을까?" | 아이템계 미리보기(지층 수, 보스 정보), 기억 단편 슬롯 확인 |
...
| 반복-돌파 대비 | 아이템계 지층 클리어 반복(루틴) 중 기억 단편 조우(돌발) |
| 고독-안도 대비 | 월드 솔로 탐험(고독) 후 세이브 포인트 복귀(안도) |
...
| 해결책 | 더 높은 레어리티 장비 획득과 기억 단편 최적화로 제2의 성장 곡선 제공. 새로운 아이템계 지층 도전과 기억 단편 합성으로 기존 장비를 넘어서는 성장 경로를 제시한다. 레어리티 승급에 따른 외형 변화(아우라, 이펙트)를 부여하여 시각적 성취감을 제공한다 |
| 검증 기준 | 최고 레벨 장비에도 기억 단편 최적화와 레어리티 승급을 통한 추가 성장 여지가 남아있어야 한다 |
...
| 항목 | 내용 |
| :--- | :--- |
| 추천 활동 | 아이템계 1지층 도전, 세이브 포인트 기억 단편 관리 |
...
| Item World | 순환의 엔진. 획득 -> 강화 | 보스 지층 클리어 후 탈출 유도(월드 귀환) |
| 세이브 포인트 | 순환의 정비소 (월드 내). 정리 -> 준비 -> 재출발 | 시설 이용 완료 시 다음 목표 추천 |
...
| :--- | :--- | :--- |
| World -> Item World | 장비 획득 직후 "이 장비의 아이템계에 들어가시겠습니까?" 즉시 진입 옵션 | 세이브 포인트까지 돌아가서 메뉴를 찾아 아이템계에 진입해야 하는 구조 |
| Item World -> World | 보스 지층 클리어 후 "스탯 게이트 해금 가능" 알림 + 월드 귀환 옵션 | 모든 지층까지 가야만 탈출할 수 있는 구조 |

2-Space 아키텍처의 상세 설계는 `Documents/Design/Design_Architecture_2Space.md`를 참조한다.
...
| 해결 방향 | 아이템계의 의무 구간(메인 순환에 필요한 최소 지층)과 선택 구간(야리코미 극한 도전)을 분리. 스탯 게이트 해금에 필요한 강화는 아이템계의 초반 지층에서 달성 가능하게 설계하고, 최심층 지층 및 고레어리티 아이템 순환 진입은 야리코미 전용 보상으로 유인한다 |
...
- [ ] 소순환(월드/아이템계/세이브 포인트)이 각 공간 내에서 독립적으로 재미를 제공하는가?
