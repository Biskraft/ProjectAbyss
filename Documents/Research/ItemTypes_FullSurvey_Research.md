...
- **ECHORIS의 기억 단편 시스템은 장신구 슬롯의 "효과 부여자" 역할을 이미 수행**하고 있으므로, 별도의 "부적/각인" 슬롯을 추가하면 기능 중복이 발생합니다. 장신구 슬롯은 "영구 스탯 증가" 역할로 한정하는 것을 권장합니다.
...
ECHORIS의 기억 단편 시스템은 **"장비 아이템 내부에 거주하며 보너스 스탯 부여"** 구조로, 이는 다음과 중첩됩니다:

| 시스템 | 본질 | ECHORIS 기억 단편와의 중복도 |
| :--- | :--- | :---: |
| PoE Jewel Socket | 장비/트리 삽입형 영구 버프 | 높음 |
| Diablo Gem | 장비 소켓에 삽입 | 중간 |
| Disgaea Memory Shard (원조) | 장비에 거주, 스탯 부여 | 동일 (ECHORIS는 Disgaea 계승) |
| Last Epoch Idol | 별도 인벤토리 슬롯에 배치 | 낮음 |

- **결론:** 기억 단편가 이미 "장비에 삽입되는 버프 부여자" 역할을 수행하므로, ECHORIS에서 **PoE식 Jewel/Gem 소켓은 이중 시스템이 되어 삭제가 옳습니다.**
- **장신구(Ring/Amulet)의 역할 재정의:** "기억 단편를 수용하지 않는 장비"로 차별화. 즉 **기억 단편 슬롯이 없는 대신 고정된 유니크 효과**를 제공하는 방향. Soulslike의 Talisman과 유사한 설계.
...
| **장비 삽입형** | Disgaea Memory Shard, PoE Jewel, D4 Gem, LE Idol | 장비/트리에 넣어 스탯 부여 | **기억 단편가 이미 이 역할 수행** |
| **선택 전환형** | Hades Aspect, Blasphemous Mea Culpa Heart | 런/빌드마다 하나씩 선택 | 렐릭 시스템과 겹침 |
| **업적/컬렉션** | CV SotN Familiar, Blasphemous Rosary | 수집/성장 후 장착 | 기억 단편 성장과 겹침 |
...
| 시스템 | 이식 추천 | 이유 |
| :--- | :---: | :--- |
| Hollow Knight Charm | **기억 단편에 통합** | Notch 방식=기억 단편 슬롯 수 제한과 동형 |
...
| 장신구 | 3 | Ring x2 + Amulet x1 | 0 (기억 단편 불가) |
| **합계** | **9슬롯** | | |
...
**Phase 3 이후 2차 확장 후보:** Whip, Gun, Shield(오프핸드). 단, 추가할수록 기억 단편 풀/밸런싱 비용이 선형 증가하므로 신중 결정.

### 7.3 방어구 슬롯 구성

| 슬롯 | 주 스탯 | 보조 효과 | 레어리티별 차별화 지점 |
| :--- | :--- | :--- | :--- |
| Head | INT | 정신 저항, HP 소폭 | 기억 단편 슬롯 수 |
...
- **기억 단편 불가 이유:** 장신구는 "고정 유니크 효과" 전담. 기억 단편는 무기/방어구에서 "가변 스탯 성장"을 담당. 역할 분리 유지.
...
| 시스템 | 제외 이유 |
| :--- | :--- |
| PoE Jewel Socket | 기억 단편와 완전 중복 |
| LE Idol | 기억 단편와 완전 중복 |
