...
ECHORIS에 Lockstep과 Rollback이 적합하지 않은 이유:

1. **1~4인 PvE 구조:** 적(기억 단편, 보스)의 AI를 어떤 클라이언트가 실행할 것인가? Lockstep/Rollback은 AI 결정론성을 모든 클라이언트에 강요한다. Server-Authoritative에서는 서버만 AI를 실행하면 된다.
...
4. **보안:** 기억 단편 복종, 아이템 드랍, 보스 처치 등 경제적 가치가 있는 이벤트는 서버 권위 검증이 필수다.
...
[데이터 레이어]
  Redis: 세션 상태, 매치메이킹 큐
  PostgreSQL: 아이템 상태, 기억 단편 데이터 영구 저장
```
...
| 기억 단편 복종 | 이벤트 (서버 확인 필수) | 경제적 가치 있는 상태 변경 |
...
**ECHORIS에 WebSocket(TCP)이 적합한 이유:**
...
3. **아이템 드랍, 기억 단편 복종** 등 중요 이벤트는 신뢰성 있는 전송이 필수다. WebSocket TCP에서는 이를 무료로 얻는다.
...
4. **중요 패킷 확인 응답(ACK):**
   기억 단편 복종, 아이템 드랍 등 중요 이벤트는 클라이언트가 ACK를 보낼 때까지 서버가 재전송.
...
### 5.2 기억 단편 복종 이벤트 동기화

기억 단편 복종은 아이템 경제에 직접 영향을 미치는 중요 이벤트이므로, **서버 권위 + ACK 보장** 방식을 사용한다.

```
[기억 단편 복종 흐름]

플레이어가 야생 기억 단편 공격 → HP=0

서버:
  1. 기억 단편 사망 처리
  2. 복종 전환 계산
  3. DB 트랜잭션: 기억 단편 상태 WILD → TAMED 저장
  4. Memory ShardTamed 이벤트 전송 (ACK 요구) {
       memory shard_id: 12345,
       item_id: 67890,
       stat_type: 'ATK',
       level: 150,
       effect_bonus: 2.0
...
**파티 플레이 기억 단편 귀속:**
파티원이 동시에 같은 기억 단편를 공격할 경우, 서버가 최종 킬 플레이어를 결정하여 귀속 처리.
```
...
ECHORIS의 야리코미 컨셉과 아이템계 협동 파밍 구조상 **개인 귀속 방식**을 권장한다. 같은 파티원이 각자 아이템을 받는 것이 "함께 파밍하는 쾌감"을 배가시킨다.
...
| **ECHORIS (PvE 횡스크롤)** | **200ms** | 보스 Telegraph 긴 편, 기억 단편 사냥은 정밀성보다 지속성 |
...
- ECHORIS 내부 문서: `Reference/게임 기획 개요.md`, `Documents/System/System_Combat_Damage.md`
