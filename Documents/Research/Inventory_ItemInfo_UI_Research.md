...
| **시사점** | 소켓 링크를 색상 원(R/G/B/W)과 연결선으로 시각화. ECHORIS Innocent와 유사한 구조 |
...
| 표시 정보 | 이름, Rank, Rarity(0~100), 8개 스탯(HP/SP/ATK/DEF/INT/SPD/HIT/RES), Specialist(Innocent) 목록 + 상태(Wild/Subdued), MV/JMP, Item Level, Resident 수/최대 |
| 스탯 비교 | 장착 전후 스탯 변화를 캐릭터 스테이터스에서 확인 |
| 인터랙션 | 메뉴 선택 + 확인 버튼 |
| 화면 비율 | 100% (전체화면) |
| **시사점** | **ECHORIS의 직접적 레퍼런스.** Specialist(=Innocent) 표시: 아이콘 + 이름 + 레벨 + 상태(Wild=빨간 얼굴 / Subdued=노란 얼굴). Rarity 0~100 수치 표시. Item World 진입을 위한 별도 NPC(Item World Guide) |
...
| **시사점** | Relic 아이콘을 하단 바에 작게 나열 + Hover로 상세. ECHORIS Innocent 표시에 참고 가능 |
...
| **Full-Screen Detail** | Disgaea, SotN, FFT, Salt & Sanctuary | 충분한 공간, 복잡한 정보 표시 가능 | 몰입 단절, 전환 비용 | **높음** (Innocent/Memory Strata 상세용) |
...
- **Disgaea:** 아이템 8개 스탯 + Specialist(Innocent) 목록 + Rarity + Item Level + Item World 상태
- **SotN:** 장비 리스트 + 장비 스탯
...
| 소켓/Innocent 유무 | 작은 점/아이콘 | Diablo(소켓 원), PoE(소켓 색상) |
...
| 소켓/Specialist 목록 | 리스트 + 아이콘 + 상태 | Disgaea(Innocent), PoE(소켓), Diablo(젬) |
...
**ECHORIS 적용:** ATK/INT/HP 3개 핵심 스탯 + 레어리티 배율 + Innocent 보너스를 명확히 분리 표시

### 4.2 소켓/젬 시스템 (= ECHORIS Innocent)

**문제:** 아이템 내부에 거주하는 존재(Innocent)의 정보를 어떻게 표시할 것인가.
...
**ECHORIS 적용:** Innocent은 Disgaea 방식(리스트 뷰 + 상태 아이콘) + PoE 방식(슬롯 시각화) 조합 추천
...
**ECHORIS 적용:** Level 1에서 아이콘 우하단 배지(클리어 여부), Level 2에서 "Strata: 2/4 Cleared" 텍스트, Level 3에서 지층별 상세(보스 타입, 클리어 여부, 발견된 Innocent)
...
ECHORIS의 아이템은 일반 ARPG보다 복잡하고(Innocent + Memory Strata), 픽셀아트 웹 게임이라는 제약이 있다. 따라서 **Tooltip + Detail View 하이브리드**를 권장한다.
...
| **Innocent 인디케이터** | **좌하단 4x4** | 없음 | **추가 권장**: Innocent가 1개 이상이면 작은 점 표시 |
...
3. **무기/방어구 타입**
4. 구분선
5. **스탯 테이블:** Base Stats vs Final Stats (Innocent 보너스 반영)
6. 구분선
7. **Innocent 리스트:** 슬롯별 이름 + 레벨 + 상태(Wild/Subdued). 빈 슬롯도 표시
8. 구분선
...
| Level | 표시 |
|:---|:---|
| Level 1 (그리드) | 좌하단 4x4 점 (Innocent 1개 이상일 때) |
| Level 2 (선택 시) | "Innocents: 2/4" 요약 텍스트 |
...
| 순위 | 항목 | 이유 |
|:---|:---|:---|
| P0 | Level 2 Info Box (선택 시 확장 정보) | 현재 가장 부족한 부분. ATK/INT/HP + Innocent 요약 + Strata 요약 |
| P0 | 장착 비교 (Compare Mode) | 장비 교체 의사결정의 핵심 |
| P1 | Level 3 Detail View | Innocent 관리, Memory Strata 확인에 필수 |
| P1 | 장착 테두리 색상 변경 | Rare(노란색)와 충돌 해결 |
| P2 | Innocent 인디케이터 (Level 1) | 그리드에서 즉시 식별 |
