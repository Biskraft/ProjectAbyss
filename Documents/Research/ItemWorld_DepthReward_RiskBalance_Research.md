...
| 기억 단편 가치 | 깊이 비례 | 높은 층 = 높은 기억 단편 수치 |
| 스태티스티션 | 역해적 깊이 비례 | 높은 층에서 역해적 → 높은 확률 + 높은 수치 (최대 900, +900% EXP) |
| 보너스 게이지 | 행동 비례 | 적 처치/지오 파괴로 충전 → 등급별 보상 (Mr.Gency → Legend 장비 → 기억 단편) |
...
긴장감 ↗↗↗↘↗↗↗↘↗↗↗↘ (주기적 해소)
```
- Hades/디스가이아 스타일. 보스 처치 후 안전 구간(기억 단편 타운/상점)
...
| 사망 시 손실 | 해당 런에서 획득한 기억 단편만 손실. 아이템 레벨업은 보존 | 적절한 손실 (전부 잃지는 않음) |
...
**로그라이크/아이템계 적용:**
- 기억 단편 드랍: 1% 확률의 희귀 기억 단편 → VR 스케줄
...
| **도파민 루프** | 예측 불가 보상 + 빈번한 소규모 보상 | 기억 단편 드랍 + 보너스 게이지 |
| **메타 진행** | 런 실패해도 영구 해금 존재 | 보스 처치 시 영구 스탯 보너스 |
| **발견의 쾌감** | 시너지 발견, 비밀 경로 | 미스터리 룸, 비밀방, 기억 단편 조합 |
...
4지층 생존확률: 70%
4지층 추가 보상: 60 가치 단위
사망 시 손실: 누적 보상의 50% = 50 (기억 단편만 손실, 스탯은 유지)
...
| **부분 손실만** | 사망 시 기억 단편만 손실, 레벨/스탯은 유지 | 완전 손실 공포 제거 → 도전 의지 ↑ |
...
reward_scaling:
  ...
  # 3. 기억 단편 드랍 (변동 비율, 깊이 비례)
  memory shard_drop_rate:
    strata_1: "5% per room"
    strata_2: "8% per room"
...
[5] 기억 단편 드랍 기대값
E[memory shard_value] = drop_rate(s) × avg_value(s) × (1 + 방랑자보너스)
