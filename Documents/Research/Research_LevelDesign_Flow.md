...
7. [Project Abyss 적용 시사점](#7-project-abyss-적용-시사점)
8. [출처](#8-출처)
...
Project Abyss의 **스탯 게이트 + 능력 게이트 이중 레이어**는 "묵시적 제한 + 스케일링 난이도"의 하이브리드로, 스탯 기반 소프트 블록(STR 부족시 피해 대폭 증가)과 능력 기반 하드 블록(이단점프 없으면 물리적 진입 불가)을 동시에 사용한다.
...
### 7-2. 난이도 곡선 — Project Abyss 월드 구조 적용

Project Abyss의 수직 층위 구조는 Dark Souls의 **나선 하강 구조**와 유사하다.
...
### 7-3. UI 없는 유도 — Project Abyss 구현 방안
...
Project Abyss의 아이템계 구조는 **세션 내 자연 정지점**을 제공하는 고유 메커닉이다.
...
이 구조는 디스가이아 아이템계 리서치(`Disgaea_ItemWorld_CoreMechanics.md`)에서 도출된 **10층 단위 체크포인트 리듬**을 Project Abyss 월드-아이템계-허브 순환 루프에 통합한 것이다.
...
SotN의 200.6% 개념을 적용하면: 월드 100% + 아이템계(모든 장비의 아이템계 완전 탐험) = 총 탐험률 200%+가 될 수 있다. 이는 Project Abyss의 아이템계 재귀 구조(`System_ItemWorld_Recursion.md`)와 자연스럽게 결합한다.
