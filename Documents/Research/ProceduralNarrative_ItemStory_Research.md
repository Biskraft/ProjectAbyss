...
ECHORIS의 아이템계 스파이크는 **"아이템에 들어가면 그 아이템의 기억이 던전으로 나타난다"**는 것이다. 현재 기준 예시 (`SurveyorEchoWedge` — CNT-ITM-001 신판, 2026-05-20)는 완성도가 높지만, 수작업 의존도가 극히 높다. 이 리서치는 다음 질문에 답한다.
...
**PA에 적용 가능한 원리:**
- **기억 단편(Memory Shard)의 대사 시스템**이 바로 이 구조다. 야생 대사 / 복종 대사 / 첫 방문 / 재방문 / 지층 클리어 후 단 1회 대사 — 이미 Hades와 동일한 구조를 쓰고 있다.
...
PA의 기억 단편 대사 시스템에 즉시 적용 가능하다. 하나의 기억 단편 타입에 대해:
```
야생 대사 = 기억 단편 성격 × 지층 테마 조합
  예) 까다로운 요리사 기억 단편 + T-WAR 지층
  → "그 칼질 솜씨로 전쟁에서 살아남았다고? 믿기 힘드네."

복종 대사 = 기억 단편 성격 × (지층 테마 + 아이템 핵심 감정)
  예) 까다로운 요리사 + T-WAR + 슬픔
  → "전쟁터에서도 음식을 만들었어. 그 사람이 그랬어. 맛이 없었다고 하더군."
```

이 방식으로 **1개 기억 단편 성격 템플릿 × 10 테마 × 3 감정 기조 = 30개 대사**를 15-20개 작성으로 커버할 수 있다.
...
PA 지층 오브젝트 배치 원칙 (Dead Cells 파생):
  ...
  선택적 방 (좌우 분기): 보조 기억 단편, Ember 오브젝트 배치
...
| :--- | :--- | :--- | :--- | :--- |
| 교체 문법 (Replacement Grammar) | Caves of Qud | 템플릿 + 변수 치환 | 높음 | 플레이버 텍스트, 기억 단편 대사 |
| 역방향 인과 (Event-First) | Caves of Qud | 결과 먼저, 원인 나중 | 높음 | 기원 4요소 생성 순서 |
| 조건부 트리거 풀 | Hades | 상태 조건 × 대사 풀 | 높음 | NPC 유령, 기억 단편 대사 |
...
| **Magic** | 소유자 + 사건 1개 | 플레이버 1줄 + 지층별 오브젝트 오버라이드 + 기억 단편 1개 | 플레이버, 지층1 오브젝트, 기억 단편 야생대사 | 기억 단편 복종대사 |
| **Rare** | 짧은 단락 (전체 배경) | 3막 서사 + 2개 기억 단편 + NPC 유령 1회 | 3막 곡선, NPC 유령 최소 1상태 | Ember 시각 이벤트 |
...
오브젝트 배치 우선순위 규칙:
  ...
  우선순위 3 (권장, 분기 방):
    위치: 선택적 탐험 분기 방 1~2개
    오브젝트: Ember(시각적 스펙터클) 오브젝트, 보조 기억 단편
    기능: 탐험 보상, 비선형 서사 조각
...
| 기억 단편 대사 | 전투 중 / 안전 구역 | 낮음 (1줄) | 수동 | "그렇게 썰면 결이 죽어!" |
...
반복감 방지 전략:
  ...
  3. 기억 단편 대사 Exhausted 처리 (Hades 방식)
...
function generate_item_narrative(rarity, item_category, drop_zone):
  ...
  if rarity >= MAGIC:
    memory shards  = generate_memory shards(rarity, primary_theme, emotional_tone)
  ...
  return {flavor_text, memory shards, arc_3act, dungeon_seed, ...}
```
...
AUTO-CHECKABLE:
  ...
  F-08: 기억 단편 수 = 레어리티 슬롯 → rarity.memory shard_slots 비교

HUMAN-REQUIRED:
  F-02: 플레이버 텍스트가 보여주는가 → 리뷰 필요
  F-05: Fire 모멘트가 구체적 1문장인가 → 리뷰 필요
  F-07: 기억 단편 대사가 Fire와 연결되는가 → 리뷰 필요
...
| P1 | Hades Exhausted 패턴 | 기억 단편 대사 시스템 | 반복감 0% |
