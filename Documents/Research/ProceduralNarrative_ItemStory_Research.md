...
> **적용 대상:** Project Abyss — 아이템계(Item World) 서사 생성 시스템

---

## 0. 리서치 목적

Project Abyss의 아이템계 스파이크는 **"아이템에 들어가면 그 아이템의 기억이 던전으로 나타난다"**는 것이다. 현재 기준 예시 (`GrandfatherKitchenKnife`)는 완성도가 높지만, 수작업 의존도가 극히 높다. 이 리서치는 다음 질문에 답한다.
