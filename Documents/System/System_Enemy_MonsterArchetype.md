...
> **문서 ID:** SYS-ENM-ARC
> **작성일:** 2026-04-19
> **최종 개정:** 2026-06-07 (몹 데미지 균일 20% 앵커 재밸런스 — DEC-049 / §2.13.4)
> **문서 상태:** Draft
...
- **몹 데미지 균일 (DEC-049, 2026-06-07):** 보스를 제외한 전 몹의 1히트 피격 데미지는 신규 플레이어(HP 100·DEF 5) 기준 **일률 20%**(Lv1 ATK 23, 실 피해 ≈20)로 앵커한다. 위협 차별화는 데미지 크기가 아니라 **Tell·리치·발동 빈도·동시성(군집)**에서 나온다(§2.13.1~2.13.3). 보스(Guardian)는 30%(Lv1 ATK 33). SSoT = `Sheets/Content_Stats_Enemy.csv` . Lv2/3는 현 스케일 비율(L2=2×·L3=4×) 보존, 보스는 곡선 형태 유지(33/50/66).
  - **워치포인트:** Swarmer(CinderImp)도 균일 20%이므로 군집 다수 동시타 시 실효 DPS가 표준 단독 몹을 크게 상회할 수 있다. 군집 밀도·동시 활성 수가 §3 조합 난이도의 실질 노브다.
- **§2.13.3의 "(1.9x/2.4x/3.4x)" 표기:** 이는 **이동속도 배율**(leap/strike/dive의 `moveSpeed`×배율 vx·vy)이며 **데미지 배율이 아니다**. 데미지는 위 균일 규칙을 따른다.
