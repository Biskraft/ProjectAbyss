...
> **설계 의도:** 7카테고리 무기 x 7축 패시브 프레임워크(300무기)에 대응하는 몬스터 아키타입 시스템. 8개 BASE 아키타입이 Tier 업그레이드로 스케일링하며, 아키타입 조합이 특정 무기 패시브 축(A-G)을 검증하는 전술 상황을 생성한다.
> **2026-05-31 슈터 리서치 통합:** `Documents/Research/EnemyDesign_MobArchetype_Research.md`(Rivera et al. 2012 "Enemy NPC Design Patterns in Shooter Games" 통합분)의 슈터 계열 패턴을 SSoT로 흡수. 기존 6 BASE → 8 BASE (ARC-07 Bombardier, ARC-08 Skirmisher 추가). 슈터 family 3종 = ARC-03 Sentinel(고정 포대) + ARC-08 Skirmisher(기동 사격) + ARC-07 Bombardier(범위 포격).
...
**속성 필수 원칙 (예외 없음):** 모든 몬스터는 반드시 하나의 **속성(fluid 물질 = magma / water / oil / acid / charged / cyro 중 1)** 을 가진다. 속성 없는("무속성") 몬스터는 존재하지 않는다. 속성은 그 몬스터가 (a) 무엇으로 이뤄졌는지, (b) 죽거나 이동할 때 무엇을 남기는지, (c) 화학 반응 매트릭스에서 어떻게 반응하는지를 결정한다. 속성은 기본적으로 **지층 테마(5기질)** 가 자동 바인딩하며(forge→magma, iron→cyro/water, rust→acid, spark→charged, shadow→oil), 디자이너가 개별 override할 수 있다. 조성 규칙은 §2 말미 **2.9 조성 레이어** 참조.
...
1. **BASE 아키타입은 8종.** (2026-05-31 슈터 리서치 통합으로 6→8.) 이상 추가 금지 — 추가하려면 슈터 4 base 패턴(Soldier/Aggressor/Carrier/Tank)에 없는 새 위협 축을 입증해야 한다. 아키타입 조합이 다양성을 만든다.
2. **Tier 업그레이드는 새 행동을 추가하지 않는다.** 기존 행동의 강도/빈도/윈도우를 조정한다.
3. **아트 자산 = 기본 실루엣 8종 + Tier별 팔레트 스왑/사이즈 변형.** 신규 스프라이트 최소화.
4. **4x4 Room Grid 기준 설계.** 방당 최대 적 수 6, 최대 아키타입 조합 3.
5. **모든 몬스터는 속성을 가진다 (예외 없음).** 무속성 몬스터 금지. 속성 = fluid 물질 1종, 기본은 테마 자동 바인딩. 몬스터 정의·스폰 데이터에 속성 필드가 비면 검증 실패로 처리한다 (§2.9).
...
> ARC-01~06 = 초기 6종. ARC-07~08 = 슈터 리서치 통합(2026-05-31). 슈터 family(ARC-03/07/08)는 Rivera et al.(2012)의 Soldier/Tank 패턴에서 도출.
...
#### ARC-07: Bombardier (포격수)

> 슈터 리서치 통합 (A-03b 범위 포격형 / Rivera Soldier > Grenadier).

**컨셉:** 후방에서 포물선 범위 폭발 투사체를 쏘아 플레이어의 발판을 강제로 비우게 만드는 지역 거부(area-denial)형 위협. Sentinel(직선 고정)과 달리 *착탄 지점*을 위협한다.
...
| Tier | 명칭 | 스탯 배율 | 행동 변화 |
|:-----|:-----|:--------:|:---------|
| T1 | Bombardier | x1.0 | 단발 포격. 2.5초 주기. 착탄 마커 0.5s |
| T2 | Bombardier Mortar | x1.8 | 2연 착탄(1.5초 간격). 마커 0.4s |
| T3 | Bombardier Howitzer | x3.0 | 3점 동시 착탄(부채꼴 산포). 마커 0.35s |
| T4 | Bombardier Siege | x5.0 | 착탄 후 지속 장판(테마 원소 잔류 2s) + 플레이어 추적 착탄 1발 |

**비주얼 차별화:**
- T1: 소형 곡사포. 단순 포물선 궤적
- T2: 쌍포신. 사이즈 1.2배
- T3: 삼연포 + 산포 조준선. 사이즈 1.3배
- T4: 대형 공성포 + 잔류 장판 이펙트. 사이즈 1.5배

---

#### ARC-08: Skirmisher (유격수)

> 슈터 리서치 통합 (A-03a 직선 사격형(기동) / Rivera Soldier > Grunt).

**컨셉:** 직선 투사체를 쏘며 끊임없이 거리를 재조정(카이팅)하는 기동 사격형. 고정된 Sentinel과 달리 *도망치며 쏜다* — 추격·예측을 강제한다.
**실루엣:** 경장 사수형. 소형 총/활. 16x16px 기본 캔버스.

**핵심 행동:**
- 이동-정지 반복(Cautious): 사격 위치를 잡고 직선 투사체 1~3발 발사 후 재배치
- 플레이어 접근 시 후퇴하며 거리 유지(kiting). 사격 중에는 정지(취약 윈도우)
- 재배치 중에는 사격 불가. Sentinel과 차이 = 고정이 아니라 끊임없이 이동
- 벽에 막히면 측면 회피 대시
...
| E (Combo Variation) | 접근 성공 시 짧은 정지 윈도우에 콤보를 욱여넣어야 함 | Combo Breaker(대시 캔슬 접근), Finisher's Edge(3타 x1.8) |

**Tier 진화:**

| Tier | 명칭 | 스탯 배율 | 행동 변화 |
|:-----|:-----|:--------:|:---------|
| T1 | Skirmisher | x1.0 | 1발 사격 후 재배치. 2초 주기 |
| T2 | Skirmisher Ranger | x1.8 | 2발 점사 + 후퇴 스텝 |
| T3 | Skirmisher Outrider | x3.0 | 3발 + 측면 회피 대시(접근 회피) |
| T4 | Skirmisher Pathfinder | x5.0 | 이동 중 사격(후퇴하며 발사) + 느린 유도 조준 |

**비주얼 차별화:**
- T1: 경장 단안 사수. 기본 직선 투사체
- T2: 쌍열 사수. 사이즈 1.1배
- T3: 잔상 이펙트(빠른 재배치). 사이즈 1.2배
- T4: 유도 조준선 + 이동 사격 잔상. 사이즈 1.3배
...
| ARC-07 | Bombardier | 지역 거부(범위 포격) | F / A / G | (신규 필요) |
| ARC-08 | Skirmisher | 기동 사격(카이팅) | F / D / E | (신규 필요) |

---

### 2.9 조성 레이어 (Composition Layer) — 모든 몬스터는 속성을 가진다
...
| 레이어 | 정의 | 출처 시스템 | 필수 |
| :--- | :--- | :--- | :---: |
| **행동 (Archetype)** | 이동·공격 패턴 | ARC-01~08 | 필수 |
...
| **거동 변형 (MOD)** | 행동 모디파이어 | 11 MOD (리서치) | 선택 |
| **강도 (Tier)** | T1~T4 | Tier 진화 | 필수 |

> 손으로 저작하는 것은 사실상 **행동 × MOD** 뿐이다. 속성·껍질은 테마가 자동 바인딩하므로, 아키타입 1종이 5테마 × 6속성으로 5~6배의 체감 변종을 낳는다.

#### 2.9.2 속성 → 거동 규칙 (모든 몬스터 적용)
...
1. **Fluid 본체** — 적의 몸이 속성 fluid. 같은 Stalker가 forge=불 궤적 추격자 / iron=얼음 추격자 / rust=부식 추격자로 분화.
2. **Container 껍질** — 적이 용기를 껍질로 두름. Aegis 방패=MagmaCrucible → 파괴 시 magma 분출. 플레이어가 WaterBarrel을 magma 적에 투척 → `water+magma 폭발+굳음`으로 발판화(캐릭터-몬스터-플랫폼 융화).
...
| Aegis + Bombardier | **요새 포격** : Aegis가 전면을 막는 동안 후방 Bombardier가 발판에 범위 착탄 | 정면은 못 뚫고, 멈춰서 우회하면 착탄에 맞음. 이동하며 우회 필수 | Shiv(배후 침투), Railbow(후방 Bombardier 우선 저격) | F, A, G |
| Stalker + Skirmisher | **추격 카이팅** : Stalker가 한 방향으로 몰고 Skirmisher가 반대로 도망치며 사격 | 한쪽을 쫓으면 다른 쪽에 노출. 두 거리 축을 동시에 못 잡음 | Chain(리치로 Skirmisher 견제), Blink Cut(대시로 거리 압축) | F, D, E |

### 3.2. 무기 카테고리 x 아키타입 상성 매트릭스

각 칸은 해당 무기가 해당 아키타입에 대해 갖는 상대적 유불리를 표시한다.

| 무기 \ 아키타입 | Stalker | Swarm | Sentinel | Aegis | Phantom | Lurker | Bombardier | Skirmisher |
|:----------------|:-------:|:-----:|:--------:|:-----:|:-------:|:------:|:----------:|:----------:|
| **Blade** (3타 콤보) | **강** | 보통 | 약 | 보통 | 보통 | **강** | 보통 | 약 |
| **Cleaver** (2타 충격파) | 보통 | **강** | 약 | **강** | 약 | 보통 | 보통 | 약 |
| **Shiv** (4타 배후) | **강** | 약 | 약 | **강** | 보통 | **강** | **강** | **강** |
| **Harpoon** (관통) | 보통 | **강** | 보통 | **강** | 약 | 보통 | **강** | 보통 |
| **Chain** (가변 리치) | 보통 | **강** | **강** | 보통 | **강** | 보통 | **강** | **강** |
| **Railbow** (원거리 물리) | 약 | 보통 | **강** | 약 | **강** | 약 | **강** | **강** |
| **Emitter** (원거리 INT) | 약 | **강** | **강** | 약 | **강** | 보통 | **강** | 보통 |
...
- **Bombardier**는 후방 area-denial이므로 빠른 접근(Shiv)·관통/리치(Harpoon/Chain)·원거리 맞불(Railbow/Emitter)이 유리. 착탄 마커 이동 회피 필수, 느린 근접(Cleaver)은 접근 전 피격
- **Skirmisher**는 카이팅이므로 거리를 좁히는 빠른 접근(Shiv)·긴 리치(Chain)·원거리 맞불(Railbow)이 유리. 느린 근접(Blade/Cleaver)은 거리를 못 좁혀 사거리 낭비
