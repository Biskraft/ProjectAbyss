...
> 의존: `System_Combat_Damage.md`, `System_Combat_Action.md`, `System_Innocent_Core.md`, `System_Coop_Synergy.md`
...
| ELM-06-A | 이노센트 | 원소 이노센트 (Burner/Freezer/Shocker) | P2 | 보류 | `System_Innocent_Core.md` |
...
Project Abyss의 원소 시스템은 한 문장으로 정의한다:

> "어떤 원소 Echo를 가지고 아이템계에 들어가느냐가 그 아이템계 경험 자체를 바꾼다."

원소는 전투 피해 배율 레이어(약점/저항)에 그치지 않는다. Echo 인챈트 선택 → 아이템계 내 원소 이노센트 분포 → 인챈트 강화 → 더 강한 원소 공격의 순환 루프가 아이템계 스파이크를 직접 강화한다.
...
| 질문 | 답 |
|:---|:---|
| 이 시스템이 아이템계 진입 경험을 강화하는가? | Yes — 원소 선택이 어떤 아이템계에 들어갈지, 어떤 이노센트가 등장할지를 결정 |
| 이 시스템이 없으면 스파이크가 약해지는가? | Yes — 파밍 목표(원소 이노센트)가 무차별화됨 |
...
| 장인 | 원소별 이노센트(Burner/Freezer/Shocker) 수집이 야리코미 파밍 목표를 세분화. 원소 Echo × 아이템계 = 특화 빌드 완성 |
...
| "원소 이노센트 필수" vs. "이노센트 조합 자유" | 원소 이노센트는 인챈트 강화 보조. 원소 공격 활성화 조건이 아님. 없어도 기본 원소 공격 가능 |
...
| 빌드 전략 | 위험 | 보상 |
|:---|:---|:---|
| 단일 원소 집중 | 저항 적에게 0.5x 피해. 원소 전환 불가 시 비효율 구간 발생 | 해당 원소 이노센트 공명 보너스. 아이템계 원소 환경 시너지 |
| 원소 없는 물리 빌드 | 원소 상태이상(Burn/Freeze/Shock) 부재. Co-op 퓨전 기여 불가 | DEF 관통 스킬로 일관된 물리 딜. 원소 저항 고적 상대 시 유리 |
| 멀티 원소 혼용 | Echo 인챈트 전환 딜레이 발생. 원소 이노센트 분산 | 다양한 상황 대응. 퓨전 솔로 발동 가능 |
...
> **원소 해금 순서:** Fire → Ice → Thunder. 월드 보스 처치 시 Echo가 해당 원소를 영구 흡수한다. 해금 순서와 구체적 보스 배치는 `System_Innocent_Core.md` §2.2 참조.
...
아이템계 진입 시점의 Echo 인챈트 원소가 해당 아이템계의 **진입 원소**로 기록된다. 진입 원소는 아이템계 내 이노센트 분포 및 층별 원소 환경 연동에 사용된다 (ELM-05-B, Phase 3 평가 항목).

> **현재 구현 범위 (Phase 2):** 진입 원소 기록은 구현하되, 드롭 가중치 연동(ELM-05-B)은 Phase 3 플레이테스트 데이터 기반으로 결정한다. 현재는 진입 원소 = 이노센트 분포 가중치 없음.
...
아이템계 내 원소 이노센트(Burner/Freezer/Shocker)의 등장은 현재(Phase 2) 균등 분포이다.

- Phase 3 평가 항목(ELM-05-B): 진입 원소 = 해당 원소 이노센트 가중치 상향. 예: Fire Echo로 진입 시 Burner 이노센트 등장 확률 증가.
- 이 연동이 구현되면 원소 선택 → 아이템계 경험 → 이노센트 특화 → Echo 강화 선순환 루프 완성.

> 상세 이노센트 규칙: `System_Innocent_Core.md` §2.2 참조.
...
## 9. 원소 이노센트 연동 (Element Innocent Integration)

원소 이노센트의 상세 규칙은 `System_Innocent_Core.md` §2.2에서 관리한다. 본 섹션은 원소 시스템 관점의 연동 계약만 정의한다.
...
> 구체적 수치는 `System_Innocent_Core.md` §2.2 및 `Sheets/Content_System_Innocent_Pool.csv` 참조.

### 9.2. 원소 이노센트 역할 한정

원소 이노센트는 **인챈트 효과를 강화하는 역할**이다. 원소 공격의 활성화 조건이 아니다. 이노센트 없이도 Echo 인챈트로 원소 공격 가능. 이노센트는 보조 최적화 레이어이다.

### 9.3. Co-op 이노센트 공명 연동

파티원 양쪽이 같은 원소 이노센트(예: 양쪽 Burner)를 보유 시 이노센트 공명 +20% 효과 발동. (`System_Coop_Synergy.md` §1.3 참조.)
...
| `System_Innocent_Core.md` §2.2 | 원소 이노센트 강화 수치, 원소 해금 순서 |
...
| `System_Innocent_Core.md` | 원소 인챈트 활성 원소 타입 (이노센트 드롭 가중치 입력) |
...
*관련 문서: `System_Combat_Damage.md` | `System_Combat_Action.md` | `System_Innocent_Core.md` | `System_Coop_Synergy.md`*
