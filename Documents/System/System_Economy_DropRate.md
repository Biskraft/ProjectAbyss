...
| ECO-01-2 | 드롭 | 드롭 풀 분리 (장비/기억 단편/Gold) | P0 | 대기 | 독립 롤 방식 |
...
| ECO-05-2 | 기억 단편 | 기억 단편 합성 비용 | P1 | 대기 | 동종: 무료 |
...
| 아이템계 깊은 지층 도전 | 사망 시 런 내 기억 단편 손실 | 고레어리티 장비, 희귀 기억 단편 |
...
drop_pool_activation:
  ...
  memory shard_pool:    [ 0.05,  0.08, 0.20 ]
...
기억 단편는 장비 풀과 별개의 독립 풀에서 드롭된다(`ECO-01-2` 참조). 지층이 깊을수록 드롭률과 기억 단편 수치 범위가 증가한다.

```yaml
memory shard_drop_by_strata:
  # 방 1개 통과당 기억 단편 드롭 발동 확률
  strata_1: 0.05   # 5%
  strata_2: 0.08   # 8%
  strata_3: 0.12   # 12%
  strata_4: 0.18   # 18%
...
memory shard_level_range_by_strata:
  strata_1: [  1,  10 ]
  strata_2: [  5,  20 ]
  strata_3: [ 10,  35 ]
  strata_4: [ 20,  50 ]
...
Sink는 HL 공급을 흡수하여 인플레이션을 억제한다. 가장 규모가 큰 Sink는 기억 단편 복종 비용이다.

```yaml
gold_sink:
  blacksmith_shop:          # 월드 세이브 포인트(대장간/상점)
    consumable_purchase:    "탈출 아이템, 회복 아이템 구매"
    memory shard_subdue_cost:   "기억 단편 복종 시 HL 소모 (HL_Subdue_Base × 기억 단편 레벨)"
...
| 지표 | 경고 임계값 | 대응 |
| :--- | :--- | :--- |
| 활성 HL 총량 | 설계 기준의 200% 초과 | 기억 단편 복종 비용 상향, 임시 이벤트 Sink 추가 |
...
기억 단편는 항상 야생(Unsubdued) 상태로 드롭된다. 복종(Subdued) 상태 기억 단편의 직접 드롭은 없다.

```yaml
memory shard_drop_rules:
  drop_state:       "Unsubdued (야생) 고정"
  drop_source:
    - "아이템계 적 처치 (독립 풀 ECO-01-2)"
    - "아이템계 보스 처치 (보장 드롭 포함)"
    - "미스터리 룸 (특수 보상)"
  world_drop:       false   # 월드에서는 기억 단편 드롭 없음
...
```yaml
memory shard_rarity_weight:
  # 기억 단편 드롭 발동 시 종류 결정 롤
  Common_Memory Shard:   700   # 70% (스탯형 일반)
  Rare_Memory Shard:     250   # 25% (파밍형, 상태이상형)
  Legendary_Memory Shard: 45  # 4.5% (유니크 효과형)
  Ancient_Memory Shard:    5  # 0.5% (복합 유니크형, 심연 이상에서만 출현)
```
...
연속 40회 처치 동안 기억 단편가 드롭되지 않으면 다음 처치 시 1개 확정 드롭.
...
```yaml
legendary_memory shard_pity:
  base_rate:         0.045   # 4.5%
  soft_pity_start:   30      # 30회 기억 단편 드롭 누적 후
  guarantee_count:   30      # 기억 단편 드롭 30회 중 Legendary 1개 보장
```
...
아이템계 보스를 처치하면 해당 아이템의 주 스탯에 영구 보너스가 부여된다. 이 보너스는 기억 단편 복종과 별개로 누적된다.
...
이 순환에서 HL은 월드에서 생성되고 기억 단편 복종·합성 비용으로 소멸한다. 강한 장비를 만들수록 더 깊은 아이템계에 진입하고, 더 많은 HL이 소비된다(소비 확장형 Sink).
...
| 순수 랜덤 (Chase Item) | 60-70% | 갓 롤 기억 단편, Ancient 등급 |
...
갓 롤은 단순 Legendary 드롭이 아닌 원하는 기억 단편 조합까지 완성된 상태를 말한다. 이것이 야리코미의 궁극 목표다.

```
[갓 롤 확률 모델 — Ancient 검 기준]
Ancient 드롭 확률: 1%
원하는 아이템 타입(검): 1/총 아이템 종류
원하는 기억 단편 슬롯 8개 완성 확률: 수천~수만 분의 1

→ 순수 드롭 갓 롤: 극히 낮은 확률 (운의 영역, 강렬한 감정 반응)
→ 기억 단편 파밍 갓 롤: 노력으로 달성 가능 (성취감, 지속 참여 동기)
→ 두 경로의 병존이 야리코미의 핵심
```
...
```yaml
party_drop_bonus:
  # 드롭률/Gold/기억 단편 보너스 배율
  #               1인    2인    3인    4인
  drop_rate:    [ 1.00, 1.15,  1.25,  1.35 ]
  gold_rate:    [ 1.00, 1.10,  1.15,  1.20 ]
  memory shard_rate:[ 1.00, 1.20,  1.35,  1.50 ]
```
...
| 상황 | 처리 방침 |
| :--- | :--- |
| 아이템계 전멸 시 | 해당 런에서 획득한 기억 단편 손실. 아이템 레벨/스탯 보너스는 보존. 월드 귀환. |
...
| 기억 단편 성장 리서치 | `Documents/Research/Memory Shard_Growth_Economy_Research.md` | 기억 단편 합성/캡 설계 |
