...
* 레퍼런스 분석 - Brawl Stars / Archero: 모바일 가상 패드 + 스킬 슬롯 기반 직관 조작
* 기술 스택: PixiJS v8 + TypeScript (웹 브라우저 + 모바일 크로스 플랫폼, 60fps 타겟)
...
Project Abyss의 캐릭터 물리는 "스킬 슬롯 기반의 직관적 전투 + 모바일 크로스 플랫폼"을 지향한다. 플레이어는 복잡한 타이밍 입력 없이 스킬 버튼과 이동 패드만으로 전투와 탐험을 수행할 수 있어야 하며, 숙련도는 "스킬 조합 선택"과 "위치 선정"에서 발현되는 구조를 만든다.

**핵심 설계 목표:**

* 모바일 가상 패드(조이스틱 + 스킬 버튼 4개)에서도 쾌적한 조작감을 보장한다.
* 기본 공격은 자동 콤보 3타로 진행되어 별도 타이밍 입력이 불필요하다.
* 자동 조준 보조를 통해 모바일 환경의 조준 부담을 최소화한다.
...
| 장인 (Craftsperson)   | 아이템 월드에서 획득한 장비가 물리 파라미터를 직접 변경 (이속, 점프력 등) |
...
* **고정 높이 점프 채택 이유**: 모바일 터치 환경에서 버튼 유지 시간으로 높이를 제어하는 것은 비직관적이다. 고정 높이 점프는 "누르면 뛴다"라는 단순한 멘탈 모델을 제공하여 입력 복잡도를 제거한다. 높이 변화가 필요한 구간은 이중 점프(능력 해금)로 해결한다.
* **Coyote Time 채택 이유**: 플랫폼 끝에서 떨어진 직후에도 점프를 허용하면 "분명 눌렀는데 안 뛰었다"는 불만을 제거한다. 150ms는 모바일 터치 입력 지연(50~100ms)을 감안하여 넉넉하게 설정한 값이다.
* **스킬 슬롯 기반 조작 채택 이유**: 정밀 타이밍 입력(저스트 가드, 콤보 타이밍 등)은 모바일 가상 패드에서 실현 불가능에 가깝다. 스킬 슬롯에 원하는 능력을 배치하고 버튼 하나로 발동하는 구조가 PC와 모바일 모두에서 일관된 경험을 제공한다.
...
4. 모바일 터치 환경에서 "누르면 뛴다"라는 단순한 멘탈 모델을 제공한다.
...
4. 150ms는 모바일 터치 입력 지연(50~100ms)을 감안하여 관대하게 설정한 값이다.
...
4. 250ms는 모바일 가상 패드의 입력 지연과 터치 반응 편차를 흡수하기 위한 값이다.
...
character_physics:
  ...
  coyote_time_ms: 150            # ms, 지면 이탈 후 점프 허용 시간 (모바일 터치 지연 감안)
  jump_buffer_ms: 250            # ms, 착지 전 점프 입력 버퍼 (모바일 터치 지연 감안)
...
| coyote_time_ms: 150     | 9프레임. 모바일 터치 입력 지연(50~100ms) + 여유분. 모바일 쾌적 조작 보장               |
...
### 모바일 터치 지연 예외 (Mobile Touch Latency)

| 시나리오                                | 처리 방식                                                        |
| :-------------------------------------- | :--------------------------------------------------------------- |
| 터치 입력 지연 50~100ms                 | Coyote Time 150ms + Jump Buffer 250ms로 흡수. 추가 보정 불필요   |
| 가상 패드 드리프트 (의도치 않은 미세 입력) | 데드존(deadzone) 15% 적용. 스틱 변위 15% 미만은 입력 없음 처리   |
| 터치 입력 씹힘 (프레임 경계)            | 입력을 프레임 독립적 이벤트 큐에 저장. 다음 물리 업데이트에서 처리 |
| 동시 터치 충돌 (이동 + 스킬 동시 터치)  | 각 터치 영역을 독립 처리. 이동 패드와 스킬 버튼은 멀티 터치 지원 |
