...
ECHORIS의 캐릭터 물리는 "스킬 슬롯 기반의 직관적 전투"를 지향한다. 플레이어는 복잡한 타이밍 입력 없이 스킬 버튼과 이동키만으로 전투와 탐험을 수행할 수 있어야 하며, 숙련도는 "스킬 조합 선택"과 "위치 선정"에서 발현되는 구조를 만든다.
...
| 스킬 시전     | 스킬별 시전 경직 + 쿨다운 소비     | 높은 범위/위력, 스킬 조합 시너지       |
...
4. 각 스킬은 개별 쿨다운을 가진다. 쿨다운 중에는 해당 슬롯 입력을 무시한다. (MP 삭제, DEC-003)
...
~~  mist_form_duration_max_ms: 3000  # ms, 안개 형태 최대 지속~~ **DEPRECATED 2026-05-18 (안개 변신 폐기 — SotN 카피)**
  ...
  # --- MP (마력) — DEPRECATED (DEC-003: 스킬은 쿨다운 단일 축, MP 삭제). 아래 값 미사용 ---
...
* [ ] 스킬 시전 시 쿨다운이 적용되고, 쿨다운 중 시전 불가인가
...
| 파라미터 | 값 | 비고 |
|:--|:--|:--|
| 낙하 속도 | 900 px/s | 일반 최대 낙하(480)의 1.875 내외배 |
