...
### 10.10 Crest Foam — 폭포 포말 (Implemented)

> **목적:** 폭포 상단(절벽 모서리)에 가로 포말 띠 + 본체 내부 별빛 입자. 레퍼런스 — *2D 픽셀아트 폭포의 표준 시각 어휘*.

**SSoT — `Sheets/Content_System_FluidTypes.csv` 컬럼 추가:**
...
| 조건 | 처리 |
| :--- | :--- |
| `ceilingFed` (위 셀이 솔리드) | crest band 생략, side spray만 (수평 jet) |
| `foam_density ≤ 0.05` | crest band 비활성 (oil 0.15 는 표시되지만 미세) |
