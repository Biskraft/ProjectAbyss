...
- **행동형 수치 밸런스 연구 (R4):** `Documents/Research/InnocentBalance_BehavioralModifier_Research.md`
...
# Wild 효과 = Tamed 효과 × WILD_EFFECT_RATIO (0.5)
# 수치 근거: InnocentBalance_BehavioralModifier_Research.md §8.1 (R4 확정값)

Berserker:
  effect: 공격 속도 + (Lv × BERSERKER_ATK_SPD_PER_LV) %
  BERSERKER_ATK_SPD_PER_LV: 0.33   # +0.33%/레벨
  soft_cap_lv: 60                   # 소프트 캡 레벨 60 → 최대 +20%
  soft_cap_effect: +20% 공격 속도   # "must-pick" 방지 상한 (Hollow Knight Quick Slash +54% DPS 기준 역산)
  play_change: 빠른 콤보, 높은 DPS. 레벨이 오를수록 타격 연속성 체감 증가.
               단검/검 빌드와 자연 시너지. 대검 빌드 대비 체감 이득 낮음.

Vampire:
  effect: 적 처치 시 MaxHP × VAMPIRE_LEECH_RATIO % 회복
  VAMPIRE_LEECH_RATIO: 2.5          # 처치 시 MaxHP의 2.5% 회복 (기존 5% → 하향)
  # 근거: Hades Common 온-킬 2%, PA 처치 빈도 Hades 대비 1/10 수준 → 2.5%가 균형점
  # 역할: 잡몹 다수 구간(처치 빈도 높음) 전문. 보스 단독 구간은 Leech 담당.
  play_change: 공격적 플레이 = 생존. 피가 부족할수록 더 공격적으로 플레이해야 회복

Ironclad:
  effect: 넉백 거리 × (1 - IRONCLAD_RESIST_PER_LV × Lv)
  IRONCLAD_RESIST_PER_LV: 0.8       # +0.8%/레벨 저항
  soft_cap_lv: 60                   # 소프트 캡 레벨 60 → 최대 ~48% 넉백 저항
  soft_cap_effect: ~48% 넉백 저항   # 50% 상한 내 (Hollow Knight Steady Body 100% = must-pick 방지)
...
Sprinter:
  ...
  SPRINTER_SPEED_PER_LV: 0.25       # +0.25%/레벨 (확정)
  soft_cap_lv: 60                   # 소프트 캡 레벨 60 → 최대 +15%
  soft_cap_effect: +15% 이동 속도   # 탐험 편의성 이노센트 포지셔닝. +20% 초과 시 전투 교란 시작
  play_change: 탐험/스피드런 전용. 월드 이동 및 아이템계 방 이동 모두 적용.
               전투 빌드 대비 강점 없음 — 슬롯 트레이드오프에서 "탐험 효율 vs 전투 강화" 선택지

Leech:
  effect: 공격 적중 시 HP + LEECH_HP_PER_HIT 회복 (쿨다운 LEECH_COOLDOWN_SEC 초)
  LEECH_HP_PER_HIT: 1               # 1HP 고정 (레벨 무관, 확정)
  LEECH_COOLDOWN_SEC: 0.5           # 0.5초 쿨다운 (확정)
  # 수학적 검증: 60초 보스전 → 초당 2HP × 60초 = 120HP (MaxHP 200 기준 60% 회복)
  #             보스 피격 1회 피해 30~50HP 대비 상쇄 불가 → 회피 스킬 유효성 유지
  # 쿨다운 역할: 무기 유형 공정성 (단검 5타/초 vs 대검 1타/초 → 쿨다운 없으면 5배 불균형)
  # 역할: Vampire가 처치 기반(잡몹 구간)이라면 Leech는 적중 기반(처치 어려운 보스 구간) 전담
  play_change: 보스 지구전. 처치 어려운 탱커형 보스에서 Vampire 대비 우위
```
...
행동형 레벨별 효과 예시 (R4 확정 파라미터 기준):

```yaml
# R4 확정값 — InnocentBalance_BehavioralModifier_Research.md §8.1
BERSERKER_ATK_SPD_PER_LV:  0.33  # 공격 속도 +0.33%/레벨 (소프트 캡 60 → +20%)
VAMPIRE_LEECH_RATIO:        2.5   # 처치 시 MaxHP의 2.5% 회복 (고정, 레벨 무관)
IRONCLAD_RESIST_PER_LV:     0.8   # 넉백 저항 +0.8%/레벨 (소프트 캡 60 → ~48%)
GHOST_IFRAME_PER_LV:        0.5   # i-frame +0.5ms/레벨 (소프트 캡 60 → +30ms)
GHOST_DASH_RANGE_PER_LV:    0.33  # 대시 거리 +0.33%/레벨 (소프트 캡 60 → +20%)
SPRINTER_SPEED_PER_LV:      0.25  # 이동 속도 +0.25%/레벨 (소프트 캡 60 → +15%)
LEECH_HP_PER_HIT:           1     # 적중 시 HP +1 (고정, 레벨 무관)
LEECH_COOLDOWN_SEC:         0.5   # 0.5초 쿨다운 (고정)
```

| 이노센트   | Lv 20 효과                    | Lv 40 효과                    | 소프트 캡 Lv 60 효과           |
| :-------- | :---------------------------- | :---------------------------- | :----------------------------- |
| Berserker | 공격 속도 +6.6%               | 공격 속도 +13.2%              | 공격 속도 **+20%** (상한)      |
| Vampire   | MaxHP × 2.5%/처치 (레벨 무관) | MaxHP × 2.5%/처치 (레벨 무관) | MaxHP × 2.5%/처치              |
| Ironclad  | 넉백 저항 +16%                | 넉백 저항 +32%                | 넉백 저항 **48 내외%** (상한)      |
| Ghost     | i-frame +10ms, 거리 +6.6%    | i-frame +20ms, 거리 +13.2%   | i-frame **+30ms**, 거리 **+20%** |
| Sprinter  | 이동 속도 +5%                 | 이동 속도 +10%                | 이동 속도 **+15%** (상한)      |
| Leech     | HP +1/적중, 0.5s CD (레벨 무관) | HP +1/적중, 0.5s CD (레벨 무관) | HP +1/적중, 0.5s CD → 초당 2HP |
...
수치 근거: InnocentBalance_BehavioralModifier_Research.md §7.2 (R4 확정)
...
Berserker 예시 (단일 소프트 캡 효과 +20% 기준):
  1개: +20%
  2개: +40%
  3개: +40% + (20% × 0.30) = +46%
  4개: +46% + (6% × 0.30) = +47.8%
...
결과: 1개 +20% vs 8개 +52% = 2.6배 격차
  → 슬롯을 다양한 이노센트로 구성하는 것이 단종 최대 장착보다 유리한 구조
```

| 동종 이노센트 수 | 스태킹 효율 | Berserker 누적 효과 (단위 +20% 기준) |
| :---: | :--- | :--- |
| 1개 | 100% | +20% |
| 2개 | 100% | +40% |
| 3개 | 30% (이전 추가분 대비) | +46% |
...
하드캡 (Ancient 8슬롯 극단적 스태킹 방어선):

| 이노센트 | 하드캡 |
| :--- | :--- |
| Berserker | 공격 속도 보너스 최대 **+30%** |
| Sprinter | 이동 속도 보너스 최대 **+25%** |
| Ironclad | 넉백 저항 최대 **60%** |
...
**최종 업데이트:** 2026-04-07
**변경 이력:**
- 2026-04-07: R4 행동형 수치 밸런스 반영 — Berserker 0.33%/레벨, Vampire 2.5%, Ironclad 0.8%/레벨, Ghost i-frame 0.5ms/레벨+거리 0.33%/레벨, Sprinter 0.25%/레벨, Leech 1HP/0.5s 확정. 스태킹 규칙 1-2개 선형 → 3번째부터 30% 체감으로 개정. 소프트 캡 하드캡 추가.
- 2026-04-05: 초기 작성
