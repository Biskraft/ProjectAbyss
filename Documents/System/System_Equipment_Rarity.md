...
> 최근 업데이트: 2026-03-23
> 문서 상태: `작성 중 (Draft)`
> 2-Space: 전체 (World - 드랍/장비 관리, Item World - 아이템계 지층/승급)
> 기둥: 야리코미
...
| RAR-03-A | 기억 단편   | 레어리티별 기억 단편 슬롯 수 정의       |    P1    | 대기      | Phase 2 기억 단편 시스템 연동          |
...
ECHORIS의 레어리티 시스템은 다음 한 문장으로 정의한다:

> "레어리티는 단순한 강함의 등급이 아니라, 그 아이템이 품고 있는 세계의 깊이다"

Common 검 하나를 드랍받는 순간과, 처음으로 Legendary 검이 화면에 쏟아지는 순간은 다른 감정이어야 한다. 그리고 Legendary 검을 손에 쥔 플레이어는 "이 검의 아이템계 모든 지층을 클리어하면 어떤 기억 단편가 기다리고 있을까"를 자연스럽게 떠올려야 한다. 레어리티는 아이템계 야리코미 깊이의 지표이자, 플레이어의 다음 목표를 제시하는 나침반이다.
...
| Normal 드랍률 55% (no_drop 15% 별도) | 초반 플레이어가 성장을 계속 느끼려면 Normal이 주류여야 함. 희소성은 상위 등급에 집중. SSoT: Design_Economy_FaucetSink.md §5.1 |
| Ancient 드랍률 0.5%              | 야리코미의 궁극 목표. 드랍 자체가 이벤트급이 되어야 감정 반응 극대화. SSoT: Design_Economy_FaucetSink.md §5.1 |
| 기억 단편 슬롯 레어리티 비례 증가 | 고레어리티 아이템을 얻어도 기억 단편를 채우지 않으면 잠재력 절반. 파밍 동기를 유지          |
...
| 기둥                  | 레어리티 시스템에서의 구현                                                                 |
| :-------------------- | :----------------------------------------------------------------------------------------- |
| Metroidvania 탐험     | 고레어리티 장비의 높은 EquipStat이 스탯 게이트를 해금. 새 층위(Tier)를 여는 열쇠           |
| Item World 야리코미   | 레어리티가 아이템계 지층 수와 기억 단편 슬롯 수를 결정. 고레어리티 = 더 깊은 야리코미       |
...
| 문제                                                   | 해결 방향                                                                                       |
| :----------------------------------------------------- | :---------------------------------------------------------------------------------------------- |
| Ancient 드랍률 0.5%가 너무 낮아 박탈감을 주지 않는가?  | 드랍률 보정 기억 단편로 보정 가능. 파밍 자체가 재미가 되도록 아이템계 기억 단편 보상 설계       |
| Normal 아이템이 Rare 드랍 후 쓸모없어지지 않는가?      | Normal 아이템계의 기억 단편를 고레어리티 아이템에 이식 가능 (Phase 2). 수집 가치 유지            |
| 고레어리티 집중 드랍 구간이 생기면 레어리티 의미가 퇴색되지 않는가? | 특정 층위(Tier)/보스별 드랍 풀을 별도 정의. 전체 풀이 아닌 층위별 풀에서 확률 적용        |
| Ancient 아이템계 4지층 + 심연이 진입 장벽이 되어 콘텐츠 이탈을 유발하지 않는가? | 아이템계 중간 탈출(탈출 아이템) 시스템. 진행 일부 보존으로 부담 완화 |
| 레어리티 승급이 기존 파밍 노력을 무효화하지 않는가?    | 승급은 레어리티 배율만 증가. 기억 단편, 기존 아이템 레벨은 유지하여 파밍 가치 보존              |

### 1.5. 위험과 보상 (Risk & Reward)

| 전략                        | 위험 (Risk)                                  | 보상 (Reward)                                         |
| :-------------------------- | :------------------------------------------- | :---------------------------------------------------- |
| Normal 장비 완전 강화        | 고레어리티 대비 스탯 열세                    | 짧은 아이템계(1지층, DEC-039), 빠른 기억 단편 확보, 낮은 부담 |
| Legendary 즉시 아이템계 진입 | 4지층 클리어 부담, 파티 필요 가능성          | x2.2 스탯 배율 + 6 기억 단편 슬롯, 스탯 게이트 해금  |
| Ancient 획득 추구 파밍       | 1% 확률, 장시간 월드 파밍 필요               | x3.0 최대 스탯, 8 기억 단편 슬롯, 최종 빌드 달성      |
| 레어리티 승급 전략           | 모든 지층 클리어 선행 필요                   | 기존 기억 단편 유지 + 레어리티 배율 상승               |
...
| 등급      | 코드 키    | 색상명 | Hex 코드 | 스탯 배율 | 기억 단편 슬롯 | 아이템계 지층 수                         | 드랍 확률 비중 |
| :-------- | :--------- | :----- | :------- | :-------- | :------------ | :--------------------------------------- | :------------- |
| Normal    | `normal`   | 흰색   | #FFFFFF  | x1.0      | 2             | 1 지층 (DEC-039 단일 다이브)              | 55%            |
| Magic     | `magic`    | 파란색 | #6969FF  | x1.3      | 3             | 2 지층 (4×4 고정)                        | 20%            |
| Rare      | `rare`     | 노란색 | #FFFF00  | x1.7      | 4             | 3 지층 (4×4 고정)                        | 7%             |
| Legendary | `legendary`| 주황색 | #FF8000  | x2.2      | 6             | 4 지층 (4×4 고정)                        | 2.5%           |
| Ancient   | `ancient`  | 초록색 | #00FF00  | x3.0      | 8             | 4 지층 + 심연 (4×4 고정)                 | 0.5%           |

Ancient 아이템계 심연(Abyss) 페이즈: 4지층 클리어 후 진입 가능한 심연 단계. 보너스 기억 단편 등장, 기억 단편 밀도 증가. 상세 규칙은 `System_ItemWorld_Core.md` 참조.
...
```
EquipStat_Final = ceil(baseStats[stat] * Rarity_Multiplier)
```

> 레어리티 배율 적용 후 소수점은 올림(ceil) 처리한다.

예시 - 검 ATK/INT 계산:

```
Normal    검 ATK: ceil(15 * 1.0) = 15,  INT: ceil(10 * 1.0) = 10
Magic     검 ATK: ceil(15 * 1.3) = 20,  INT: ceil(10 * 1.3) = 13
Rare      검 ATK: ceil(15 * 1.7) = 26,  INT: ceil(10 * 1.7) = 17
Legendary 검 ATK: ceil(15 * 2.2) = 33,  INT: ceil(10 * 2.2) = 22
Ancient   검 ATK: ceil(15 * 3.0) = 45,  INT: ceil(10 * 3.0) = 30
```

위 계산에서 15/10은 해당 장비의 `baseStats.atk`/`baseStats.int` 값이다. 레어리티 배율은 `Sheets/Content_System_Rarity_Table.csv`에서 관리하며, baseStats는 `Sheets/Content_Stats_Weapon_List.csv`에서 관리한다. ATK/INT/HP 3스탯 체계에 따라 장비는 ATK, INT, DEF, RES 등의 스탯을 보유할 수 있다.
...
graph TD
    ...
    ZONE --> BONUS[기억 단편 기반 보정값 계산]
    BONUS --> ROLL[가중 랜덤 추출]
    ROLL --> ND{no_drop 15%}
    ROLL --> N{Normal 55%}
    ROLL --> MG{Magic 20%}
    ROLL --> R{Rare 7%}
    ROLL --> L{Legendary 2.5%}
    ROLL --> A{Ancient 0.5%}
    N --> ITEM[아이템 생성]
    MG --> ITEM
    R --> ITEM
...
드랍률 보정 기억 단편(Phase 2)는 전체 드랍률을 상향 이동시키며, 구역별 드랍 풀에서 확률이 적용된다.

> **설계 변경:** 기존 LCK 스탯 기반 드랍률 보정은 삭제. 드랍률은 적 테이블 기본값 + 기억 단편 보정으로 결정.
...
| :-------- | :--------------- | :------------------------------------------------ | :-------------------------------- |
| Normal    | 1 지층           | 지층 1: 아이템 장군 (DEC-039 단일 다이브)         | 단일 클리어로 완료. 재진입 차단 (DEMO_BLOCK_REDIVE) |
| Magic     | 2 지층           | 지층 1: 아이템 장군, 지층 2: 아이템 왕            |                                   |
...
- 아이템 이름 (등급 접두사 추가)
- 기존 기억 단편 (슬롯 수 유지 + 추가 슬롯 개방)
- 현재 아이템 레벨

승급 후 변경되는 요소:

- 레어리티 등급 및 스탯 배율
- 기억 단편 슬롯 수 (상위 등급 기준으로 증가)
- 아이템계 최대 지층 수
...
| RAR-R03 | 드랍률 보정 기억 단편는 드랍 확률 보정에만 적용. 등급 자체를 강제하지 않음 | 없음                               |
...
| RAR-R12 | Ancient 심연 페이즈는 4지층 클리어 후 진입 가능             | 심연 내 기억 단편 밀도는 별도 정의          |
| RAR-R13 | 아이템계 도중 사망 시 현재 지층부터 재시작 (진행 일부 손실) | 탈출 아이템 사용 시 진행 보존 후 탈출      |

### 3.3. 기억 단편 슬롯 규칙 (Memory Shard Slot Rules)

| 규칙 ID | 규칙                                             | 예외                                           |
| :------ | :----------------------------------------------- | :--------------------------------------------- |
| RAR-R20 | 기억 단편 슬롯 수는 레어리티에 의해 결정           | 레어리티 승급 시 상위 등급 슬롯 수로 증가 (Phase 2) |
| RAR-R21 | 슬롯 초과 기억 단편는 장착 불가                   | 기억 단편 이식 시 슬롯 여유 확인 필수            |
| RAR-R22 | 기억 단편 슬롯의 Memory ShardBonus는 장비 착용 시에만 활성화 | 창고 보관 중 아이템은 Memory ShardBonus 비활성     |
...
rarity_table:
  Normal:
    ...
    stat_multiplier: 1.0
    memory shard_slots: 2
    item_world_strata: 2             # 지층 수 (4×4 고정)
    drop_weight: 55          # 드랍 가중치 (no_drop 15% 별도, SSoT: Design_Economy_FaucetSink.md §5.1)
...
```yaml
drop_rate_params:
  RAR_NO_DROP_WEIGHT: 15            # no_drop 가중치 (SSoT: Design_Economy_FaucetSink.md §5.1)
  RAR_TOTAL_WEIGHT: 100             # 전체 가중치 합계 (no_drop 포함, 변경 금지)
  # RAR_LCK_BONUS_PER_POINT: DEPRECATED (LCK 스탯 삭제, 기억 단편 보정으로 대체)
  # RAR_LCK_BONUS_CAP: DEPRECATED
  RAR_MEMORY_SHARD_DROP_BONUS_CAP: 0.20  # 기억 단편 드랍률 보정 상한 (최대 +20%)
...
rarity_upgrade_params:
  ...
  # 승급 재료: Remnant Fragment (기억 단편 분해로 획득, System_Memory Shard_Core.md §2.7 참조)
  # 기억 단편 분해 공식: floor(기억 단편_레벨 / FRAGMENT_DIVISOR) 개
  remnant_fragment_cost:
    normal_to_magic:      10   # Remnant Fragment 10개
    magic_to_rare:        30   # Remnant Fragment 30개
    rare_to_legendary:    80   # Remnant Fragment 80개
    legendary_to_ancient: 200  # Remnant Fragment 200개

  # 승급 HL 비용 (Remnant Fragment와 별도)
  hl_cost:
    normal_to_magic:      500
    magic_to_rare:        2000
    rare_to_legendary:    8000
    legendary_to_ancient: 25000
```
...
| `Documents/System/System_Memory Shard_Core.md` §2.7    | Remnant Fragment 분해 공식 (승급 재료 SSoT) |
...
| 케이스                                        | 처리 방식                                                              |
| :-------------------------------------------- | :--------------------------------------------------------------------- |
| 드랍 가중치 합계(no_drop 포함)가 100 미만인 경우 | 나머지 가중치를 Normal에 추가하여 총합 100 유지. 개발 로그 경고 기록  |
| 구역 드랍 풀에 Ancient가 정의되지 않은 경우    | Ancient 드랍 없음으로 처리. 오류 아님                                  |
| 기억 단편 보정 적용 후 Ancient 비율이 0이 되는 경우 | Ancient 최소 가중치 0.1 보장 (RAR_ANCIENT_MIN_WEIGHT 파라미터)    |
...
| Ancient 심연 페이즈에서 탈출 아이템 미보유 사망       | 아이템계 처음(지층 1)으로 복귀. 심연 기억 단편는 손실                        |

### 5.3. 시각 표현 관련 예외

| 케이스                               | 처리 방식                                                              |
| :----------------------------------- | :--------------------------------------------------------------------- |
| 저사양 환경에서 Legendary 이펙트 성능 저하  | 파티클 수 50% 감소 + 화면 정지 제거. 사운드는 유지              |
...
| memory shard_slots가 음수인 경우          | 0으로 폴백. 기억 단편 슬롯 없음 처리                                   |

---

## 검증 기준 (Verification Checklist)

- [ ] 5등급 레어리티(Normal-Ancient)의 드랍 가중치 + no_drop(15%) 합이 항상 100인가?
...
- [ ] 레어리티별 기억 단편 슬롯 수가 `System_Equipment_Slots.md`의 아이템 데이터 구조(memory shards[])와 일관되게 연동되는가?
- [ ] 드랍 이펙트(색상, 파티클, 사운드)가 레어리티별로 명확히 차별화되어 있는가?
- [ ] 기억 단편 드랍률 보정이 적용될 때 상한(`RAR_MEMORY_SHARD_DROP_BONUS_CAP`)이 동작하는가?
