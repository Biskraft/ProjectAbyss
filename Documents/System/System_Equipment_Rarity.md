...
| :-------- | :--------- | :----- | :------- | :-------- | :------------ | :--------------------------------------- | :------------- |
| Normal    | `normal`   | 흰색   | #FFFFFF  | x1.0      | 2             | 2 지층 (3×3 → 4×4)                      | 60%            |
| Magic     | `magic`    | 파란색 | #6969FF  | x1.3      | 3             | 3 지층 (3×3 → 4×4 → 4×4)               | 25%            |
| Rare      | `rare`     | 노란색 | #FFFF00  | x1.7      | 4             | 3 지층 (3×3 → 4×4 → 5×5)               | 10%            |
| Legendary | `legendary`| 주황색 | #FF8000  | x2.2      | 6             | 4 지층 (3×3 → 4×4 → 5×5 → 5×5)        | 4%             |
| Ancient   | `ancient`  | 초록색 | #00FF00  | x3.0      | 8             | 4 지층 + 심연 (3×3 → 4×4 → 5×5 → 5×5) | 1%             |
...
MVP 1지층 고정 규칙: Phase 1에서는 모든 레어리티의 아이템계가 1지층(3×3)으로 고정된다. 단, 데이터 구조는 위 테이블의 전체 지층 수로 정의하여 Phase 2 활성화 시 코드 변경 없이 데이터 수정만으로 전환 가능하도록 설계한다.
...
rarity_table:
  Normal:
    ...
    stat_multiplier: 1.0
    innocent_slots: 2
    item_world_strata: 2             # 지층 수 (3×3 → 4×4)
  ...
  Magic:
    ...
    stat_multiplier: 1.3
    innocent_slots: 3
    item_world_strata: 3             # 지층 수 (3×3 → 4×4 → 4×4)
    drop_weight: 25
  ...
  Rare:
    ...
    stat_multiplier: 1.7
    innocent_slots: 4
    item_world_strata: 3             # 지층 수 (3×3 → 4×4 → 5×5)
    drop_weight: 10
  ...
  Legendary:
    ...
    stat_multiplier: 2.2
    innocent_slots: 6
    item_world_strata: 4             # 지층 수 (3×3 → 4×4 → 5×5 → 5×5)
    drop_weight: 4
  ...
  Ancient:
    ...
    stat_multiplier: 3.0
    innocent_slots: 8
    item_world_strata: 4             # 지층 수 (3×3 → 4×4 → 5×5 → 5×5) + 심연
...
```yaml
mvp_overrides:
  RAR_MVP_STRATA_OVERRIDE: 1         # MVP에서 모든 레어리티 아이템계를 1지층(3×3)으로 고정
