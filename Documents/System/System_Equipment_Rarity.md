...
| 문제                                                   | 해결 방향                                                                                       |
| :----------------------------------------------------- | :---------------------------------------------------------------------------------------------- |
| Ancient 드랍률 0.5%가 너무 낮아 박탈감을 주지 않는가?  | 드랍률 보정 이노센트로 보정 가능. 파밍 자체가 재미가 되도록 아이템계 이노센트 보상 설계       |
...
드랍률 보정 이노센트(Phase 2)는 전체 드랍률을 상향 이동시키며, 구역별 드랍 풀에서 확률이 적용된다.

> **설계 변경:** 기존 LCK 스탯 기반 드랍률 보정은 삭제. 드랍률은 적 테이블 기본값 + 이노센트 보정으로 결정.
...
| RAR-R03 | 드랍률 보정 이노센트는 드랍 확률 보정에만 적용. 등급 자체를 강제하지 않음 | 없음                               |
...
drop_rate_params:
  ...
  # RAR_LCK_BONUS_PER_POINT: DEPRECATED (LCK 스탯 삭제, 이노센트 보정으로 대체)
  # RAR_LCK_BONUS_CAP: DEPRECATED
  RAR_INNOCENT_DROP_BONUS_CAP: 0.20  # 이노센트 드랍률 보정 상한 (최대 +20%)
...
rarity_upgrade_params:
  ...
  # 승급 재료: Remnant Fragment (이노센트 분해로 획득, System_Innocent_Core.md §2.7 참조)
  # 이노센트 분해 공식: floor(이노센트_레벨 / FRAGMENT_DIVISOR) 개
...
| `Documents/System/System_Innocent_Core.md` §2.7    | Remnant Fragment 분해 공식 (승급 재료 SSoT) |
...
| 이노센트 보정 적용 후 Ancient 비율이 0이 되는 경우 | Ancient 최소 가중치 0.1 보장 (RAR_ANCIENT_MIN_WEIGHT 파라미터)    |
...
- [ ] 이노센트 드랍률 보정이 적용될 때 상한(`RAR_INNOCENT_DROP_BONUS_CAP`)이 동작하는가?
