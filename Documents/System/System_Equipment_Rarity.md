...
> 최근 업데이트: 2026-03-23
> 문서 상태: `작성 중 (Draft)`
> 3-Space: 전체 (World - 드랍, Item World - 아이템계 층수/승급, Hub - 장비 관리)
> 기둥: 야리코미
...
| RAR-06-A | 승급       | 레어리티 승급 (Rarity Upgrade)         |    P2    | 대기      | Phase 2 아이템계 100층 클리어 보상    |
...
Common 검 하나를 드랍받는 순간과, 처음으로 Legendary 검이 화면에 쏟아지는 순간은 다른 감정이어야 한다. 그리고 Legendary 검을 손에 쥔 플레이어는 "이 검의 아이템계 100층을 클리어하면 어떤 이노센트가 기다리고 있을까"를 자연스럽게 떠올려야 한다. 레어리티는 아이템계 야리코미 깊이의 지표이자, 플레이어의 다음 목표를 제시하는 나침반이다.

### 1.2. 설계 근거 (Reasoning)

| 결정                             | 근거                                                                                       |
| :------------------------------- | :----------------------------------------------------------------------------------------- |
| 5등급 체계 (Common ~ Mythic)     | 디스가이아의 레어리티 체계 참조. 너무 많은 등급은 인지 부담, 너무 적으면 차별화 부족      |
| Common 드랍률 60%                | 초반 플레이어가 성장을 계속 느끼려면 Common이 주류여야 함. 희소성은 상위 등급에 집중       |
| Mythic 드랍률 1%                 | 야리코미의 궁극 목표. 드랍 자체가 이벤트급이 되어야 감정 반응 극대화                      |
| 이노센트 슬롯 레어리티 비례 증가 | 고레어리티 아이템을 얻어도 이노센트를 채우지 않으면 잠재력 절반. 파밍 동기를 유지          |
| 아이템계 층수 레어리티 비례 증가 | 고레어리티 아이템계는 더 길고 어려운 던전. 강함의 대가로 더 많은 노력이 필요               |
| MVP는 3층 고정, 데이터는 전체 정의 | 핵심 루프 검증이 우선. 층수 로직은 나중에 활성화되도록 데이터는 미리 준비                 |
...
| Item World 야리코미   | 레어리티가 아이템계 층수와 이노센트 슬롯 수를 결정. 고레어리티 = 더 깊은 야리코미          |
...
| 문제                                                   | 해결 방향                                                                                       |
| :----------------------------------------------------- | :---------------------------------------------------------------------------------------------- |
| Mythic 드랍률 1%가 너무 낮아 박탈감을 주지 않는가?     | LCK 스탯으로 드랍률 보정 가능. 파밍 자체가 재미가 되도록 아이템계 이노센트 보상 설계           |
| Common 아이템이 Rare 드랍 후 쓸모없어지지 않는가?      | Common 아이템계의 이노센트를 고레어리티 아이템에 이식 가능 (Phase 2). 수집 가치 유지            |
| 고레어리티 집중 드랍 구간이 생기면 레어리티 의미가 퇴색되지 않는가? | 특정 구역/보스별 드랍 풀을 별도 정의. 전체 풀이 아닌 구역별 풀에서 확률 적용              |
| Mythic 아이템계 100+ 층이 진입 장벽이 되어 콘텐츠 이탈을 유발하지 않는가? | 아이템계 중간 탈출(탈출 아이템) 시스템. 진행 일부 보존으로 부담 완화 |
...
| :-------------------------- | :------------------------------------------- | :---------------------------------------------------- |
| Common 장비 완전 강화       | 고레어리티 대비 스탯 열세                    | 짧은 아이템계(30층), 빠른 이노센트 확보, 낮은 부담    |
| Legendary 즉시 아이템계 진입 | 100층 클리어 부담, 파티 필요 가능성          | x2.2 스탯 배율 + 6 이노센트 슬롯, 스탯 게이트 해금  |
| Mythic 획득 추구 파밍        | 1% 확률, 장시간 월드 파밍 필요               | x3.0 최대 스탯, 8 이노센트 슬롯, 최종 빌드 달성      |
| 레어리티 승급 전략           | 100층 클리어 선행 필요                       | 기존 이노센트 유지 + 레어리티 배율 상승               |

---

## 2. 메커닉 (Mechanics)

### 2.1. 레어리티 등급 체계 (Rarity Tier System)

| 등급      | 스탯 배율 | 이노센트 슬롯 | 아이템계 층수 | 드랍 확률 비중 | 이름 색상   |
| :-------- | :-------- | :------------ | :------------ | :------------- | :---------- |
| Common    | x1.0      | 2             | 30층          | 60%            | 흰색 #FFFFFF |
| Uncommon  | x1.3      | 3             | 50층          | 25%            | 초록 #00C853 |
| Rare      | x1.7      | 4             | 70층          | 10%            | 파랑 #2196F3 |
| Legendary | x2.2      | 6             | 100층         | 4%             | 금색 #FFD600 |
| Mythic    | x3.0      | 8             | 100층 + 보너스 | 1%            | 보라 #AA00FF |

Mythic 아이템계 보너스 층: 100층 클리어 후 무한 반복 가능한 추가 층 (보너스 이노센트 등장, 이노센트 밀도 증가). 상세 규칙은 `System_ItemWorld_Core.md` 참조.
...
```
Common  검 ATK: 15 * 1.0 = 15
Uncommon검 ATK: 15 * 1.3 = 19  (소수점 버림)
Rare    검 ATK: 15 * 1.7 = 25
Legendary검 ATK: 15 * 2.2 = 33
Mythic  검 ATK: 15 * 3.0 = 45
```
...
| :-------- | :-------------------------------------------- | :---------- | :------------------ |
| Common    | 흰 빛 소멸 후 아이템 출현                     | #FFFFFF     | 짧은 획득음         |
| Uncommon  | 초록 파티클 소량 + 아이템 출현                | #00C853     | 획득음 + 짧은 멜로디 |
| Rare      | 파란 파티클 + 작은 광원 효과                  | #2196F3     | 멜로디 + 효과음     |
| Legendary | 금빛 파티클 폭발 + 화면 일시 정지 0.2초 + 광원 | #FFD600    | 팡파레 짧은 버전    |
| Mythic    | 보라 폭발 + 화면 플래시 + 큰 광원 + 이름 강조 표시 | #AA00FF | 팡파레 풀버전 + 글로우 |
...
| 등급      | 아이템계 층수 | 보스 배치                              | 비고                          |
| :-------- | :------------ | :------------------------------------- | :---------------------------- |
| Common    | 30층          | 10층: 아이템 장군                      | MVP: 3층 고정 (데이터는 30층 정의) |
| Uncommon  | 50층          | 10/20층: 아이템 장군 x2               |                               |
| Rare      | 70층          | 10/20/30층: 장군 x3                   |                               |
| Legendary | 100층         | 10~90층: 장군, 50층: 아이템 왕, 100층: 아이템 신 | 파티 권장 (70층+) |
| Mythic    | 100층 + 보너스 | 100층: 아이템 대신 + 보너스 층 무한   | 파티 강력 권장                |

MVP 3층 고정 규칙: Phase 1에서는 모든 레어리티의 아이템계가 3층으로 고정된다. 단, 데이터 구조는 위 테이블의 전체 층수로 정의하여 Phase 2 활성화 시 코드 변경 없이 데이터 수정만으로 전환 가능하도록 설계한다.
...
승급은 Mythic에서 멈춘다. Mythic 이상 등급은 없다.
...
| RAR-R04 | Mythic 드랍은 특정 고난이도 구역 또는 보스 전용 풀에서만 발생 | 이벤트 드랍 풀 별도 정의 가능       |
| RAR-R05 | 드랍된 아이템의 레어리티는 픽업 전까지 변경 불가             | 없음                                |

### 3.2. 아이템계 층수 규칙 (Item World Floor Rules)

| 규칙 ID | 규칙                                                   | 예외                                    |
| :------ | :----------------------------------------------------- | :-------------------------------------- |
| RAR-R10 | 레어리티별 최대 층수는 `RAR_FLOOR_CAP` 파라미터 기준   | MVP: `RAR_MVP_FLOOR_OVERRIDE = 3` 적용  |
| RAR-R11 | 이미 클리어한 층은 재진입 시 층수 카운트 유지           | 아이템계 완전 초기화 아이템 사용 시 제외 |
| RAR-R12 | Mythic 보너스 층은 100층 클리어 후 무한 반복           | 보너스 층 내 이노센트 밀도는 별도 정의  |
| RAR-R13 | 아이템계 도중 사망 시 현재 층부터 재시작 (진행 일부 손실) | 탈출 아이템 사용 시 진행 보존 후 탈출  |
...
rarity_table:
  Common:
    ...
    stat_multiplier: 1.0
    innocent_slots: 2
    item_world_floors: 30
    drop_weight: 60          # 드랍 가중치 합계 100 기준
    name_color_hex: "#FFFFFF"
    drop_particle: "fx_drop_common"
    drop_sound: "sfx_drop_common"
  ...
  Uncommon:
    ...
    stat_multiplier: 1.3
    innocent_slots: 3
    item_world_floors: 50
    drop_weight: 25
    name_color_hex: "#00C853"
    drop_particle: "fx_drop_uncommon"
    drop_sound: "sfx_drop_uncommon"
  ...
  Rare:
    ...
    stat_multiplier: 1.7
    innocent_slots: 4
    item_world_floors: 70
    drop_weight: 10
    name_color_hex: "#2196F3"
  ...
  Legendary:
    ...
    stat_multiplier: 2.2
    innocent_slots: 6
    item_world_floors: 100
    drop_weight: 4
    name_color_hex: "#FFD600"
  ...
  Mythic:
    ...
    stat_multiplier: 3.0
    innocent_slots: 8
    item_world_floors: 100   # + 보너스 층 (무한)
    item_world_bonus_floor: true
    drop_weight: 1
    name_color_hex: "#AA00FF"
    drop_particle: "fx_drop_mythic"
    drop_sound: "sfx_drop_mythic"
    screen_flash: true
```
...
```yaml
mvp_overrides:
  RAR_MVP_FLOOR_OVERRIDE: 3         # MVP에서 모든 레어리티 아이템계를 3층으로 고정
  RAR_MVP_FLOOR_ENABLED: true       # true: 3층 고정 적용 / false: 등급별 층수 적용
  # Phase 2 활성화 시: RAR_MVP_FLOOR_ENABLED를 false로 변경만 하면 전체 층수 시스템 활성화
```
...
drop_rate_params:
  ...
  RAR_MYTHIC_BOSS_ONLY: true        # Mythic을 보스 전용 드랍 풀에서만 발생 여부
```
...
rarity_upgrade_params:
  RAR_UPGRADE_REQUIRE_FLOOR: 100    # 승급 조건: 아이템계 해당 레어리티 최대 층수 클리어
  RAR_UPGRADE_MATERIAL_REQUIRED: true  # 승급 재료 아이템 필요 여부 (Phase 2 정의)
  RAR_UPGRADE_MAX_TIER: "Mythic"    # 최상위 등급. 이 이상 승급 불가
...
| 구역 드랍 풀에 Mythic이 정의되지 않은 경우    | Mythic 드랍 없음으로 처리. 오류 아님                                   |
| LCK 보정 적용 후 Mythic 비율이 0이 되는 경우 | Mythic 최소 가중치 0.1 보장 (RAR_MYTHIC_MIN_WEIGHT 파라미터)          |
...
| 케이스                                           | 처리 방식                                                              |
| :----------------------------------------------- | :--------------------------------------------------------------------- |
| MVP 3층 고정 중 데이터에 층수가 30으로 정의된 경우 | `RAR_MVP_FLOOR_OVERRIDE = 3`이 적용되어 3층으로 진입. 데이터 값 무시 |
| 아이템계 도중 아이템 레어리티 승급 발생 (Phase 2) | 현재 아이템계 완료 후 승급 적용. 진행 중 층수 변경 없음               |
| Mythic 보너스 층에서 탈출 아이템 미보유 사망      | 아이템계 처음(1층)으로 복귀. 보너스 층 이노센트는 손실               |
...
| 접근성 모드 (색맹 지원)               | 색상 외 등급 기호 추가 표시: Common(○), Uncommon(◎), Rare(★), Legendary(★★), Mythic(★★★) |
...
| 케이스                               | 처리 방식                                                              |
| :----------------------------------- | :--------------------------------------------------------------------- |
| 알 수 없는 레어리티 값               | Common으로 폴백. 개발 로그 경고 기록                                  |
...
- [ ] 5등급 레어리티의 드랍 가중치 합이 항상 100인가?
- [ ] Common 검 Lv1의 ATK 15에 레어리티 배율을 적용한 결과가 `System_Combat_Damage.md`의 데미지 공식에 정확히 반영되는가?
- [ ] MVP에서 모든 레어리티의 아이템계가 3층으로 고정되며, `RAR_MVP_FLOOR_ENABLED = false` 설정만으로 전체 층수 시스템으로 전환 가능한가?
...
- [ ] 이 시스템이 야리코미 기둥의 핵심 경험("이 아이템계 100층을 클리어하면 레어리티 승급 가능")을 명확히 지원하는가?
