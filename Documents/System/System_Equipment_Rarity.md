...
| Ancient 아이템계 100+ 층이 진입 장벽이 되어 콘텐츠 이탈을 유발하지 않는가? | 아이템계 중간 탈출(탈출 아이템) 시스템. 진행 일부 보존으로 부담 완화 |
...
| :-------------------------- | :------------------------------------------- | :---------------------------------------------------- |
| Normal 장비 완전 강화        | 고레어리티 대비 스탯 열세                    | 짧은 아이템계(30층), 빠른 이노센트 확보, 낮은 부담    |
...
| 등급      | 코드 키    | 색상명 | Hex 코드 | 스탯 배율 | 이노센트 슬롯 | 아이템계 층수 | 드랍 확률 비중 |
| :-------- | :--------- | :----- | :------- | :-------- | :------------ | :------------ | :------------- |
| Normal    | `normal`   | 흰색   | #FFFFFF  | x1.0      | 2             | 30층          | 60%            |
| Magic     | `magic`    | 파란색 | #6969FF  | x1.3      | 3             | 50층          | 25%            |
| Rare      | `rare`     | 노란색 | #FFFF00  | x1.7      | 4             | 70층          | 10%            |
| Legendary | `legendary`| 주황색 | #FF8000  | x2.2      | 6             | 100층         | 4%             |
| Ancient   | `ancient`  | 초록색 | #00FF00  | x3.0      | 8             | 100층 + 보너스 | 1%            |

Ancient 아이템계 보너스 층: 100층 클리어 후 무한 반복 가능한 추가 층 (보너스 이노센트 등장, 이노센트 밀도 증가). 상세 규칙은 `System_ItemWorld_Core.md` 참조.
...
| 등급      | 아이템계 층수 | 보스 배치                              | 비고                          |
| :-------- | :------------ | :------------------------------------- | :---------------------------- |
| Normal    | 30층          | 10층: 아이템 장군                      | MVP: 3층 고정 (데이터는 30층 정의) |
| Magic     | 50층          | 10/20층: 아이템 장군 x2               |                               |
| Rare      | 70층          | 10/20/30층: 장군 x3                   |                               |
| Legendary | 100층         | 10~90층: 장군, 50층: 아이템 왕, 100층: 아이템 신 | 파티 권장 (70층+) |
| Ancient   | 100층 + 보너스 | 100층: 아이템 대신 + 보너스 층 무한   | 파티 강력 권장                |
...
| RAR-R12 | Ancient 보너스 층은 100층 클리어 후 무한 반복           | 보너스 층 내 이노센트 밀도는 별도 정의  |
...
| Ancient 보너스 층에서 탈출 아이템 미보유 사망      | 아이템계 처음(1층)으로 복귀. 보너스 층 이노센트는 손실               |
