...
* **토폴로지 결정:** `memory/wiki/decisions/DEC-037-Item-World-Topology-AntColony.md ` (4×4 그리드 → 방사형 개미굴)
* **토폴로지 레퍼런스:** `Documents/Research/ItemWorld_AntColony_Topology_Research.md ` (Dead Cells / Hollow Knight 매핑)

---

## DEC-037 적용 안내 (Topology Override)

> **2026-04-28 기준** — 본 문서의 절차 생성 토폴로지는 **방사형 개미굴 (Hub-and-Spoke 그래프)** 로 전환되었다.
> 이전 4×4 Room Grid 기반 서술 중 일부는 **콘텐츠 (룸 프리팹/시각 테마/해저드)** 측면에서 유효하나,
> **토폴로지 자료구조** (RoomGrid → RoomGraph), **레어리티별 규모 정의** (Grid 크기 → 노드/가지 수),
> **Critical Path 알고리즘** (Spelunky 그리드 → MST + 랜덤 분기) 은 모두 갱신되었다.
> 코드 측 SSoT 도 PR-C4 (2026-04-28) 에서 정리됨 — `game/src/level/RoomGrid.ts ` 의 legacy `generateUnifiedGrid` 제거,
> `game/src/level/RoomGraphAdapter.ts ` 의 `generateUnifiedGridFromGraph` 가 단일 진입점이다.
> `?topo=graph` opt-in 플래그도 폐기 (그래프가 기본).

---

## 구현 현황 (Implementation Status)

> **최근 업데이트:** 2026-06-01 (스승/마르타 mentor framing 제거)
> v3 정합 (D-20)
> **문서 상태:** `재작성 (DEC-037 토폴로지 전환 반영)`
> **2-Space:** Item World
> **기둥:** 야리코미
...
| IWF-02-A      | 시스템 | 지층별 Room Graph 레이아웃 생성        |    P1    | 🔄 재구현 필요 | DEC-037: 방사형 (Hub-and-Spoke) |
| IWF-03-A      | 시스템 | MST + 랜덤 분기 알고리즘                |    P1    | 🔄 재구현 필요 | DEC-037: 광장→가지 그래프 생성   |
...
| IWF-09-A      | 시스템 | 기억 단편 배치                           |    P2    | ⬜ 제작 필요 | 야생 기억 단편                  |
| IWF-10-A      | 시스템 | 멀티플레이 스케일링                     |    P1    | ⬜ 제작 필요 | 1-2인 체력 보정 (Phase 3)                |
...
| 문제 (100층) | 해법 (기억의 지층) |
| :--- | :--- |
| 선형 반복이 메트로베니아의 비선형 탐험과 충돌 | 각 지층이 비선형 Room Graph — 광장(Hub) + 가지(Branch) + 비밀방 + 루프 |
...
| 레퍼런스 | 차용 요소 | ECHORIS 적용 |
| :------- | :-------- | :------------------ |
| 인셉션 (영화) | 꿈의 층위, 시간 팽창, 림보, 킥, 투영체 | 지층 구조, 깊이별 맵 확장, 심연(Abyss), 탈출 제단, 적대도 상승 |
| 디스가이아 시리즈 | 아이템계, 레어리티별 깊이, 기억 단편 | 레어리티별 지층 수, 아이템 성장, 기억 단편(Phase 2) |
| 할로우 나이트 (City of Tears / Mantis Village / Hive) | 광장형 NPC 거점 + 가지 끝 보스, 마을형 클러스터 | 각 지층의 방사형 RoomGraph — 광장(Resident Quarter) + 가지(Distortion Branch) + 가지 끝 보스 |
| 스펠렁키 | Room Grid + Critical Path + Chunk 조립 | **월드(World) 절차 생성에만 적용** — 아이템계는 DEC-037로 그래프 전환. Chunk 조립 단계만 차용 |
| Dead Cells (Concept Graph) | 핸드메이드 룸 프리팹 + 그래프 조립 + PCG Fill | RoomGraph 자료구조 + 룸 사이 회랑 자동 채움 (PCG Fill) |
...
현실 (월드)
  ...
  ║  ─ 아이템의 핵심 사건                ║
  ║  ─ 환경에 변화 시작 (어둡고 불안정)  ║
  ║  ─ RoomGraph (가지 증가 + 왜곡 분기) ║
  ╠══════════════════════════════════════╣
  ║      ▼ 보스: 기억의 문 (Gate) ▼     ║
  ╠══════════════════════════════════════╣
  ║  지층 3+: 원초 기억 (Primal Memory)  ║
  ║  ─ 아이템이 만들어진 순간의 기억     ║
  ║  ─ RoomGraph (다중 허브 + 심연 침식) ║
  ╠══════════════════════════════════════╣
  ║      ▼ 최종 보스: 기억의 핵 ▼       ║
  ╠══════════════════════════════════════╣
  ║  ??? 심연 (Abyss) — Ancient 전용    ║
...
**토폴로지:** 모든 지층 방사형 RoomGraph (DEC-037). 광장(Hub) 1개 + 가지(Branch) N개 구조. 레어리티 차이는 **지층 수 + 가지 수**로 표현한다.

| 레어리티 | 지층 수 | 노드 수 (지층당) | 가지 수 (지층당) | 보스 배치 모델 | EXP 배율 (지층별) | 심연 |
| :--- | :---: | :---: | :---: | :--- | :--- | :---: |
| Normal | 2 | 4-5 | 2 | 광장 직결 (Mantis Village 축소) | 1.0x → 1.5x | ✕ |
| Magic | 3 | 6-7 | 3 | 광장 직결 + 가지 끝 혼합 | 1.0x → 1.5x → 2.0x | ✕ |
| Rare | 3 | 8-10 | 4 | 가지 끝 (City of Tears 미니) | 1.0x → 1.5x → 2.5x | ✕ |
| Legendary | 4 | 11-13 | 5 | 가지 끝 (City of Tears 표준) | 1.0x → 1.5x → 2.5x → 3.5x | ✕ |
| Ancient | 4+심연 | 14-16 + ∞ | 6 + 심연 | 다중 허브 + 심연 보스 (Hive) | 1.0x → 1.5x → 2.5x → 3.5x | ✔ |

> 노드/가지 수치는 1차 가설. 프로토타입 후 조정. 자세한 레퍼런스 매핑은 `Documents/Research/ItemWorld_AntColony_Topology_Research.md ` 참조.

**방사형 그래프 채택 근거 (DEC-037):**
- 광장(Hub)은 NPC 클러스터링 거점이 되어 "회상된 주민(Resident Quarter)" 내러티브와 정합
- 가지(Branch)는 왜곡된 기억 분기 — Forgotten 단편(적 NPC)을 가지에 배치
- 4×4 그리드는 모든 셀이 동등 비중이라 마을 본질과 어긋남 (DEC-037 §결정 근거)
- 월드(수직 거대 구조 The Shaft) 와 시각 대비 극대화 — 카메라만으로 두 공간 즉시 구분
- 레어리티 확장이 직관적 (가지 수만 늘리면 됨, 그리드 크기 강제 없음)

#### 2.2.1. 지층 깊이별 내부 변화 (토폴로지 보존, 콘텐츠 변화)

같은 RoomGraph 골격이지만 지층이 깊어질수록 내부가 변한다 (DEC-037 이후, "그리드 고정" 표현은 "그래프 골격 고정"으로 해석):

| 지층 | 조명 | 적 밀도 | 환경 변화 | 그래프 변형 | 느낌 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 지층 1 | 밝음 | 낮음 (방당 1-2) | 안정적 | 단일 광장 + 짧은 가지 | 편안한 탐색 |
| 지층 2 | 어두워짐 | 보통 (방당 2-3) | 함정 등장, 색조 변화 | 가지 길어짐, 일부 우회 분기 | 긴장 시작 |
| 지층 3 | 어둡고 불안정 | 높음 (방당 3-4) | 환경 왜곡, 벽 균열 | 가지 끝 분기 증가, 막다른 길 | 미로 속으로 |
| 지층 4 | 심연 침식 | 매우 높음 | 바닥 붕괴, 심연 파티클 | 다중 허브, 광장 간 비밀 통로 | 세계가 무너짐 |
...
> BLAME!의 핵심 시각 테크닉은 좁은 터널→거대한 보이드(void)의 극단적 대비. 이 리듬을 RoomGraph의 가지(Branch) 진행 위에서 구현한다 (DEC-037).

**원칙:** RoomGraph의 광장→가지→가지 끝 진행을 따라 노드 내부의 **천장 높이, 방 폭, 시야 범위**를 극단적으로 대비시켜 "숨 막힘 → 경외 → 숨 막힘" 리듬을 만든다. "노드 수"가 아닌 **노드 내부의 공간감**과 **가지 진입 시점의 대비**로 표현한다.
...
> **Void 방의 핵심:** 배경 패럴랙스가 5-7레이어로 확장되어 끝없는 깊이를 시사. 캐릭터가 화면의 극히 작은 부분만 차지. 먼 배경에 거대 빌더(유사 존재)의 실루엣이 무언가를 건설하는 모습이 보임.

##### RoomGraph 내 리듬 패턴

> **DEC-037 적응 안내:** 아래 4×4 Grid 예시 다이어그램은 이전 토폴로지 기준이다. 패턴(터널→보이드→터널)의 의도는 **광장(M/V) ↔ 가지 진입(S) ↔ 가지 끝 보스 룸(V/T)** 으로 재해석된다. 다이어그램 자체의 그래프 표기 갱신은 후속 작업.

**패턴 A: "터널 → 보이드 → 터널" (기본 리듬)** *(이전 4×4 그리드 예시 — 그래프 적응 보류)*

가장 기본적인 BLAME! 리듬. Critical Path를 따라 좁은→넓은→좁은이 교차.

```
[이전 4×4 Grid 예시 — DEC-037 그래프 토폴로지로 재서술 필요]
...
| **배경 동작** | 먼 배경에서 무언가(거대 빌더 유사체)가 천천히 이동하는 실루엣 |
...
> **DEC-037 적응 안내:** 본 절의 합체(Merge)/분할(Split) 메커니즘은 4×4 Grid의 16셀 가정에 기반한다. 그래프 토폴로지(DEC-037)에서는 "합체"=인접 노드 룸 프리팹을 더 큰 룸 프리팹으로 교체, "분할"=가지에 좁은 룸 프리팹 연쇄 배치로 재해석된다. 무기별 공간 성격(전장/미로/긴 복도/마탑/사냥터)은 그대로 유지된다.

RoomGraph의 노드(룸 프리팹)는 **무기 카테고리에 따라 형태와 배치가 달라진다.** 이것이 "아이템의 기억이 다른 공간 경험을 만든다"의 핵심 메커니즘이다.

노드는 **합체(Merge)와 분할(Split)**이 가능하다:
- 합체: 인접 노드 2-4개를 큰 룸 프리팹 1개로 교체 (대검, 활)
- 분할: 노드 1개를 좁은 룸 프리팹 연쇄 2-3개로 교체 (단검)
- 표준: 노드 = 룸 1:1 (검)

| 무기 카테고리 | 실제 방 수 | 방 비율 | 천장 높이 | 특수 구조 | 공간 성격 | 에르다의 체감 |
| :--- | :---: | :--- | :--- | :--- | :--- | :--- |
> **7무기 정합(DEC-026):** 아래 표는 구 5종 기준. 하픈(Harpoon)·체인(Chain)의 아이템계 테마 행 추가 필요(설계 대기).

| 블레이드 (Blade) | 16 (표준) | 1:1 정방 | 보통 | 없음 | 균형 잡힌 탐험 | "잘 설계된 작업장이네. 동선이 막히질 않아." |
| 클리버 (Cleaver) | 6-8 (합체) | 2:1 넓음 | 높음 | 넓은 전장, 소수 강적 | 전장 — 느린 진행, 무거운 타격 | "넓다. 대형 용광로급 공간. 발걸음 소리가 울려." |
| 시브 (Shiv) | 20-24 (분할) | 1:2 좁고 긴 | 낮음 | 좁은 틈새, 막다른 길 | 미로 — 빠른 이동, 급습 | "좁은데 환기는 좋다. 소형 공방 같아." |
| 레일보우 (Railbow) | 8-10 (합체) | 2:1 넓음 | 매우 높음 | 고저차 지형, 엄폐물 | 사냥터 — 높은 곳을 잡는 전투 | "천장이 엄청 높아. 이 아이템, 야외 기억을 갖고 있네." |
| 에미터 (Emitter) | 16 (표준) | 불규칙 | 높음 | 부유 플랫폼, 수직 구조 | 마탑 — 위로 오르는 탐험 | "수직 구조물인데... 이거 지지 구조가 말이 안 되잖아." |

**설계 원칙:**
> "같은 RoomGraph 골격, 같은 T-HOME 테마의 부엌이라도, 검의 부엌과 단검의 부엌은 **걷는 경험이 다르다.** 검의 부엌은 넉넉한 조리대 사이를 걷지만, 단검의 부엌은 좁은 선반 틈새를 뛰어다닌다."
...
> **핵심 규칙: 원소 공격은 해당 원소 기억 단편를 장비에 장착했을 때만 가능하다. 무기 고유 속성으로는 원소 공격이 불가능하다.**

이 규칙이 위의 모든 원소×지형 상호작용에 선행 조건으로 적용된다. 플레이어가 화 속성 공격으로 거미줄을 소각하려면, 장비에 화(火) 기억 단편가 장착되어 있어야 한다.

| 원소 기억 단편 | 확정 획득 경로 | 비고 |
| :--- | :--- | :--- |
...
**지형 상호작용과 기억 단편의 관계:** 모든 원소×지형 상호작용은 편의 수단이며, 원소 없이도 해당 구역을 통과할 수 있다.
...
| 무기 | 원소 | 지형 | 시그니처 전략 |
| :--- | :--- | :--- | :--- |
| **블레이드 (Blade)** | 빙 (Ice) | 얼음(4) + 가시(5) 인접 | 빙으로 가시를 얼음 가시로 강화한 뒤, 보통 넉백으로 적을 가시로 밀어넣는다. 빙결 + 고정 피해 2중 타격. |
| **시브 (Shiv)** | 뇌 (Thunder) | 물(2) | 연사력으로 물 타일 한 곳에 뇌 공격을 먼저 꽂아 flood-fill 감전을 깔고, 감전 구역 안에서 근접 난전을 유도한다. 감전으로 적 속도가 30% 감소하여 단검의 이동 이점이 극대화된다. |
| **클리버 (Cleaver)** | 빙 (Ice) | 얼음(4) + 가시(5) | 빙으로 물을 얼린 뒤, 가시를 얼음 가시로 강화한다. 대검 강공격 넉백 ×2로 적을 얼음 가시에 밀어넣어 빙결 + 고정 피해 극단적 콤보. |
| **에미터 (Emitter)** | 빙 → 뇌 연계 | 물(2) | "번개 홍수" 체인 2의 직접 실행자. 지팡이의 원거리 연사로 빙 → 얼음(4) 변환 후 즉시 뇌 → flood-fill 감전. 파티에서 지팡이 사용자가 이 조합을 맡으면 광역 제어를 독점한다. |
| **레일보우 (Railbow)** | 화 (Fire) | 거미줄(6) + 어둠(8) | 어둠 구역에서 화살로 거미줄을 소각하면 파티클 발화 순간 주변이 밝아진다. 정찰용 화살이 동시에 지형 제거와 조명 역할을 겸한다. 어둠 구역 탐색의 핵심 도구. |
...
에르다는 에코의 기억 속을 걷는 동안 직업적 감수성으로 공간을 읽는다. 지층이 깊어질수록 그 시선은 기술적 호기심에서 무게감으로, 다시 침묵으로 변한다. 이 감정 변화는 내레이션, 환경음, 에르다의 표정 스프라이트 변화로 표현한다.
...
| 지층 4 / 심연 — 최심층·Abyss | **진지 / 경외** | "잊혀진 자도 이런 곳까지 내려왔을까. ...내려왔겠지." | 침식 파티클 극대화. 독백 없이 긴 정적 후 대사. 에르다 걸음 속도 -10% |

**설계 의도:**
- 지층 1의 코미디 독백은 플레이어를 공간에 빠르게 정착시키는 온보딩 역할을 한다
- 지층 4의 침묵과 잊혀진 자 에코 언급은 에르다의 서사적 동기를 자연스럽게 드러낸다
...
flowchart TD
    ...
    B --> C["Step 3: RoomGraph 생성\ngenerateRoomGraph(nodes, branches, rng)"]
    C --> D["Step 4: MST + 랜덤 분기\n광장→가지 + 보장된 연결"]
    D --> DA["Step 4.5: 무기 프로파일 적용\n노드 합체/분할 + 지형 타일 배치"]
    DA --> E["Step 5: 룸 프리팹 조립 + PCG Fill\nassembleRoom(node, roomRng) + 회랑 자동 채움"]
    E --> F["Step 6: 적 배치\n지층 스케일링 적용 + Forgotten 단편을 가지에 배치"]
    F --> G["Step 7: 보스 배치\n레어리티별 모델 (광장 직결 / 가지 끝)"]
    G --> H["Step 8: 탈출 제단 배치\n광장 또는 크리티컬 가지 노드에 25%"]
    H --> I["Step 9: 탐험 상태 복원\nvisited/cleared 플래그"]
```
...
### 3.2. Room Graph 규칙 (Room Graph Rules) — DEC-037

| 규칙 ID | 규칙 | 예외 |
| :--- | :--- | :--- |
| IWF-R10 | 노드 수 / 가지 수는 StratumDef.nodeCount / branchCount 로 결정 | 없음 |
| IWF-R11 | MST는 항상 광장(Hub) → 모든 가지 끝까지 연결 보장 (Always Winnable) | 없음 |
| IWF-R12 | 탐험한 노드는 재진입 시 visited/cleared 상태 유지 | 없음 |
| IWF-R13 | 클리어한 노드 재방문 시 적이 리스폰되지 않음 | 없음 |
| IWF-R14 | 광장(Hub) 노드는 시작 노드와 동일하거나 시작 노드에 인접해야 함 | 없음 |
| IWF-R15 | 가지 끝 노드는 보스 / 보물 / 막다른 길 중 하나 | 없음 |
| IWF-R16 | 광장↔광장 직접 연결 금지 (Ancient 다중 허브 제외) | Ancient 등급 |
| IWF-R17 | 보스는 spoke를 승격하지 않고 **별도 노드(role='boss')** 로 생성 | 없음 |
| IWF-R18 | Shrine(탈출 제단)은 **별도 노드(role='shrine')** 로 생성. spawn_chance 에 무관하게 결정적 1개 배치 | Normal 1지층은 생략 가능 (온보딩 단순화) |
| IWF-R19 | Ancient 다중 허브는 `hubCount=2` 고정. 두 허브는 angular 거리 π(정반대)로 배치 | Phase 2에서 가변 검토 |

**확정 결정 (2026-04-28, DEC-037 후속):**

| 항목 | 결정 |
| :--- | :--- |
| Critical Path 정의 (IWF-R30 적격 노드) | **보스 가지 위 spoke 노드 + hub 노드** (보스/시작 제외) |
| Shrine 노드 위치 | hub와 보스 가지의 첫 spoke 사이의 별도 노드. 보스로 향하는 동선 위에 회수 거점 형성 |
| `bossPlacement: 'hub_adjacent'` (Normal) 의미 | 가지 2개 중 1개를 **"spoke 1개 + boss 1개" 가지**로 구성 (Mantis Village 신전 비유). 가지 깊이 2 |
| `bossPlacement: 'branch_end'` (Magic~Legendary) | 가장 긴 가지의 끝에 별도 boss 노드 부착 |
| `bossPlacement: 'multi_hub'` (Ancient) | 두 hub를 hub-hub 엣지로 연결 + 가장 긴 가지 끝에 boss 1개 (Phase 2에서 가지별 mini-boss 검토) |
...
| 규칙 ID | 규칙 | 예외 |
| :--- | :--- | :--- |
| IWF-R30 | Shrine은 **별도 노드(role='shrine')** 로 생성. 위치는 hub와 보스 가지 첫 spoke 사이 (Critical Path 위) | Normal 1지층 생략 가능 |
| IWF-R31 | 결정적 배치 (시드 기반 — 같은 아이템·지층은 항상 같은 위치) | 없음 |
| IWF-R32 | UP 키로 상호작용 | 없음 |
| IWF-R33 | 사용 시 모든 보상 보존 + 안전 귀환 | 없음 |

> **DEC-037 후속:** 그래프 토폴로지에서 "Critical Path" = **hub + 보스 가지 spoke 사슬**. 다른 가지는 보상 가지로 분류되어 shrine 후보에서 제외된다.
...
```yaml
# DEC-037: grid 컬럼 → nodes / branches / hubCount / bossPlacement
# nodes        = boss/shrine 포함 총 노드 수
# branches     = hub에서 뻗는 가지 수 (보스 가지 포함)
# hubCount     = 광장 노드 개수 (Ancient만 2, 그 외 1)
# bossPlacement= hub_adjacent | branch_end | multi_hub
strata_by_rarity:
  normal:   # 2지층, 보스 가지 깊이 2 (Mantis Village 축소)
    - { nodes: 4,  branches: 2, hubCount: 1, bossPlacement: hub_adjacent, enemyHp: 40, enemyAtk: 8,  bonus: +0, bossHp: 160, bossAtk: 16, exp: 1.0x }
    - { nodes: 5,  branches: 2, hubCount: 1, bossPlacement: hub_adjacent, enemyHp: 50, enemyAtk: 10, bonus: +1, bossHp: 200, bossAtk: 20, exp: 1.5x }
  magic:    # 3지층, 보스 = 가장 긴 가지 끝
    - { nodes: 6,  branches: 3, hubCount: 1, bossPlacement: branch_end, enemyHp: 60,  enemyAtk: 12, bonus: +0, bossHp: 240, bossAtk: 24, exp: 1.0x }
    - { nodes: 7,  branches: 3, hubCount: 1, bossPlacement: branch_end, enemyHp: 75,  enemyAtk: 15, bonus: +1, bossHp: 300, bossAtk: 30, exp: 1.5x }
    - { nodes: 7,  branches: 3, hubCount: 1, bossPlacement: branch_end, enemyHp: 90,  enemyAtk: 18, bonus: +1, bossHp: 360, bossAtk: 36, exp: 2.0x }
  rare:     # 3지층, 가지 끝 보스 (City of Tears 미니)
    - { nodes: 8,  branches: 4, hubCount: 1, bossPlacement: branch_end, enemyHp: 90,  enemyAtk: 18, bonus: +0, bossHp: 360, bossAtk: 36, exp: 1.0x }
    - { nodes: 9,  branches: 4, hubCount: 1, bossPlacement: branch_end, enemyHp: 110, enemyAtk: 22, bonus: +1, bossHp: 440, bossAtk: 44, exp: 1.5x }
    - { nodes: 10, branches: 4, hubCount: 1, bossPlacement: branch_end, enemyHp: 130, enemyAtk: 25, bonus: +2, bossHp: 520, bossAtk: 50, exp: 2.5x }
  legendary: # 4지층, 가지 끝 보스 + 롱 가지 (City of Tears 표준)
    - { nodes: 11, branches: 5, hubCount: 1, bossPlacement: branch_end, enemyHp: 130, enemyAtk: 25, bonus: +0, bossHp: 520, bossAtk: 50, exp: 1.0x }
    - { nodes: 12, branches: 5, hubCount: 1, bossPlacement: branch_end, enemyHp: 155, enemyAtk: 30, bonus: +1, bossHp: 620, bossAtk: 60, exp: 1.5x }
    - { nodes: 12, branches: 5, hubCount: 1, bossPlacement: branch_end, enemyHp: 180, enemyAtk: 35, bonus: +2, bossHp: 720, bossAtk: 70, exp: 2.5x }
    - { nodes: 13, branches: 5, hubCount: 1, bossPlacement: branch_end, enemyHp: 220, enemyAtk: 42, bonus: +3, bossHp: 880, bossAtk: 84, exp: 3.5x }
  ancient:  # 4지층 + 심연, 다중 허브 (Hive). 두 허브는 angular π 정반대
    - { nodes: 14, branches: 6, hubCount: 2, bossPlacement: multi_hub, enemyHp: 180, enemyAtk: 35, bonus: +0, bossHp: 720,  bossAtk: 70,  exp: 1.0x }
    - { nodes: 15, branches: 6, hubCount: 2, bossPlacement: multi_hub, enemyHp: 220, enemyAtk: 42, bonus: +1, bossHp: 880,  bossAtk: 84,  exp: 1.5x }
    - { nodes: 15, branches: 6, hubCount: 2, bossPlacement: multi_hub, enemyHp: 270, enemyAtk: 50, bonus: +2, bossHp: 1080, bossAtk: 100, exp: 2.5x }
    - { nodes: 16, branches: 6, hubCount: 2, bossPlacement: multi_hub, enemyHp: 330, enemyAtk: 60, bonus: +3, bossHp: 1320, bossAtk: 120, exp: 3.5x }
    # Phase 2: 심연 (Abyss) 추가
```
...
| 마이크로 구조 | 절차적 (지형 변주) | 절차적 (RoomGraph + 룸 프리팹 + PCG Fill) — DEC-037 |
| 시드 체계 | 월드 시드 1개 | PRNG(itemUID + stratumIndex) |
| 지속성 | 영구 저장 | 영속 탐험 상태 (ItemWorldProgress) |
| 멀티플레이 | 동일 월드 공유 | 파티 리더 아이템에 종속 (1-2인) |
...
아이템계 RoomGraph의 각 노드는 RoomType이 지정된다 (DEC-037). RoomType은 런타임에 해당 방의 몹 스폰, 특수 콘텐츠, 클리어 조건을 결정한다.
