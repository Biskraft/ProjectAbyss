...
| :---------- | :-------- | :-------- | :------------ | :-------- |
| **물 (2)** | → ID 0 *(증발, 영구)* | → ID 4 *(동결, 영구)* | — *(변환 없음, 효과 적용)* | — *(변환 없음, 효과 적용)* |
| **얼음 (4)** | → ID 2 *(해빙, 영구)* | — | — *(변환 없음, 효과 적용)* | — |
| **가시 (5)** | — | 얼음 가시로 강화 *(효과 추가, 비변환)* | — | — |
| **거미줄 (6)** | → ID 0 *(소각, 영구)* | → ID 1 유사 *(동결 고화, 영구)* | → ID 0 *(방전 소각, 영구)* | → ID 0 *(날려버림, 영구)* |
...
- **화 + 물(2) → ID 0 (증발):** 물은 열에 의해 증발한다. 반경 1타일 범위 증기 피해 발생 후 타일 소멸. 빠른 이동 경로를 개척하거나 침수 구역을 제거하는 데 사용 가능.
- **화 + 얼음(4) → ID 2 (해빙):** 얼음은 열에 녹아 물이 된다. 연쇄 반응의 핵심 — 뇌 공격과 조합 가능.
- **빙 + 물(2) → ID 4 (동결):** 물이 얼어 얼음 타일로 변한다. 도체로 변환되므로 뇌와 즉시 연쇄 가능.
- **빙 + 거미줄(6) → ID 1 유사 (동결 고화):** 거미줄이 얼어붙어 딱딱한 격자 구조물이 된다. 이동은 불가하지만 서 있을 수 있는 발판으로 변환. 충격(강공격 등)에 박살나 → ID 0.
...
- **뇌 + 거미줄(6) → ID 0 (방전 소각):** 거미줄은 도체는 아니지만, 강한 전류가 불꽃을 일으켜 소각한다. 화 속성과 동일 결과.
- **뇌 + 상승 기류(7) → 플라즈마 기류 강화:** 전류가 기류 통로를 따라 흐르며 이온화된 플라즈마 기류를 형성한다. 내부 엔티티에 지속 감전 상태이상 적용.
- **풍 + 거미줄(6) → ID 0 (날려버림):** 바람이 거미줄을 뿌리째 날려버린다. 화나 뇌와 달리 소리 없이 조용히 제거되는 것이 특징.
