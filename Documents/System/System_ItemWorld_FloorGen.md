...
| IWF-18-A      | 원소×지형 | 타일 변환 (화/빙/뇌/풍)              |    P1    | ⬜ 제작 필요 | 화+거미줄, 화/빙+물, 빙+부서지는 바닥 등 |
...
| IWF-18-D      | 원소×지형 | 광 + 어둠(8) 제거                      |    P2    | ⬜ 제작 필요 | 광 속성 히트박스 충돌 → ID 0   |
| IWF-19-A      | 지형     | 함몰 가시 (ID 10)                     |    P1    | ⬜ 제작 필요 | 주기적 on/off + 빙 동결 고정   |
| IWF-19-B      | 지형     | 화염 분출기 (ID 11)                   |    P1    | ⬜ 제작 필요 | 주기 분출 + 빙 정지/풍 방향 전환 |
...
| 11 | **화염 분출기 (Flame Jet)** | 벽/바닥에서 주기적으로 화염 분출. 접촉 시 화상(Burn) | 분출 주기 사이로 통과. 빙(Ice)으로 일시 정지(3초). 풍(Wind)으로 방향 전환 | P1 |
| 12 | **사라지는 발판 (Vanishing Platform)** | 밟으면 2초 후 소멸, 3초 후 재생. 원웨이처럼 위에서만 착지 | 연속 배치 시 리듬 기반 이동 강제. 빙(Ice)으로 동결 시 영구 유지 | P1 |
| 13 | **용암 (Lava)** | 접촉 시 최대 HP 20% 고정 피해 + 화상(Burn). 물처럼 채워진 구역 | 빙(Ice)으로 표면만 동결 → 얼음(ID 4) 위를 걸을 수 있음 (3초 유지 후 재용해). 풍(Wind)으로 화산 파편 날림 | P2 |
| 14 | **산성 (Acid)** | 접촉 시 최대 HP 15% 고정 피해 + 독(Poison) 3초. 물처럼 채워진 구역 | 빙(Ice)으로 동결 불가 (산성은 어는점이 낮다). 풍(Wind)으로 산성 방울 튀김 범위 피해 | P2 |
...
- 화염/뇌/풍 속성 공격 접촉 시 일시 소각: 타일 ID → 0, 불꽃 파티클 재생
...
- 원소 상호작용:
  ...
  - 풍(Wind): 화염 분출 **방향 전환** (바람 방향으로 꺾임). 적을 향해 리다이렉트 가능
...
- 원소 상호작용:
  ...
  - 풍(Wind): 용암 파편 날림 — 풍 공격 지점 주변 2타일에 화상 범위 피해
...
- 원소 상호작용:
  ...
  - 풍(Wind): 산성 방울 튀김 — 풍 공격 지점 주변 2타일에 독 범위 피해
...
| **풍 (Wind)** | 밀어낸다. 약한 구조물은 날아간다. 가벼운 것은 더 멀리 날아간다. |

> **암 (Dark) / 광 (Light) 제외 이유:** 암과 광은 적 속성 상성(System_Combat_Damage.md §2.5) 전용 원소이다. 지형은 물리 법칙에 의해 변화하는 공간이며, 암/광은 물리 법칙이 아닌 존재론적 속성이다. 단 하나의 예외가 있다 — 아래 참고.
>
> **예외 — 광 (Light) → 어둠 타일(ID 8):** 광 속성 공격이 어둠 타일에 직접 닿으면 해당 타일이 제거된다(→ ID 0). 어둠은 물리 지형이 아닌 시각 마스크이므로, 빛의 직접 조사만이 이를 걷어낼 수 있다. 광 원소 무기·스킬 보유자에게 어둠 구역 탐색의 특권을 부여한다.

##### 원소 획득 규칙 (Elemental Acquisition Rule)

> **핵심 규칙: 원소 공격은 해당 원소 이노센트를 장비에 장착했을 때만 가능하다. 스킬트리나 무기 고유 속성으로는 원소 공격이 불가능하다.**
...
| 풍 (Wind) | 관측의 눈 처치 | 복종 상태로 드랍 |
| 광 (Light) | 수호단의 잔영 처치 | 복종 상태로 드랍. **어둠(ID 8) 제거 필수 원소** |
| 암 (Dark) | 아이템계 랜덤 드랍 | 지형 상호작용 없음. 적 상성 전용 |

**지형 상호작용과 이노센트의 관계:** 어둠 타일(ID 8)은 광(光) 이노센트 없이는 제거 불가능한 유일한 하드 게이트 지형이다. 나머지 원소×지형 상호작용은 편의 수단이며, 원소 없이도 해당 구역을 통과할 수 있다.
...
| 지형 \ 원소 | 화 (Fire) | 빙 (Ice) | 뇌 (Thunder) | 풍 (Wind) |
| :---------- | :-------- | :-------- | :------------ | :-------- |
| **물 (2)** | → ID 0 *(증발, 일시 — `water_regen_ms: 8000`)* | → ID 4 *(동결, 일시 — `ice_melt_ms: 15000`)* | — *(변환 없음, 효과 적용)* | — *(변환 없음, 효과 적용)* |
| **얼음 (4)** | → ID 2 *(해빙, 영구)* | — | — *(변환 없음, 효과 적용)* | — |
| **가시 (5)** | — | 얼음 가시로 강화 *(효과 추가, 비변환)* | — | — |
| **거미줄 (6)** | → ID 0 *(소각, 일시 — `web_regen_ms: 10000`)* | → ID 1 유사 *(동결 고화, 영구)* | → ID 0 *(방전 소각, 일시 — `web_regen_ms: 10000`)* | → ID 0 *(날려버림, 일시 — `web_regen_ms: 10000`)* |
| **상승 기류 (7)** | 화염 기류로 강화 *(효과 추가, 비변환)* | → ID 1 *(기류 동결 차단, 영구)* | 플라즈마 기류로 강화 *(효과 추가, 비변환)* | → ID 7 강화 *(풍속 ×2, 영구)* |
| **어둠 (8)** | — | — | — | — |
| **부서지는 바닥 (9)** | → ID 0 *(즉시 붕괴, 대기 0초)* | → ID 1 *(동결 고화, 영구)* | — | → ID 0 *(즉시 붕괴, 대기 0초)* |
| **함몰 가시 (10)** | 주기 가속 *(cycle ×0.5)* | **현재 상태 동결 고정** *(돌출 중→영구 가시, 함몰 중→영구 안전)* | — | — |
| **화염 분출기 (11)** | 분출 범위 3→5타일 *(10초)* | **일시 정지 3초** *(연료 동결)* | — | **분출 방향 전환** *(바람 방향으로)* |
| **사라지는 발판 (12)** | → 즉시 소멸 *(타이머 무시)* | **동결 → 영구 유지** *(화로 해빙 시 타이머 재시작)* | — | — |
| **용암 (13)** | — *(이미 화염)* | **표면 동결 → ID 4 (3초 유지 후 재용해)** | — *(비도체)* | 파편 날림 *(주변 2타일 화상 범위 피해)* |
| **산성 (14)** | 산성 증기 *(위 2타일 독 안개 5초)* | — *(동결 불가)* | **산성 내 감전 확산** *(flood-fill, 산성=전해질=도체)* | 산성 방울 튀김 *(주변 2타일 독 범위 피해)* |
| **압력판 (15)** | — | — | — | — |
...
- **빙 + 부서지는 바닥(9) → ID 1 (동결 고화):** 약한 구조물이 얼어붙어 오히려 안전한 영구 발판이 된다. 도끼로 즉시 파괴 가능.
...
- **풍 + 거미줄(6) → ID 0 (날려버림, 일시):** 바람이 거미줄을 뿌리째 날려버린다. 화나 뇌와 달리 소리 없이 조용히 제거되는 것이 특징. `web_regen_ms: 10000` — 10초 후 재생.
- **풍 + 상승 기류(7) → 강화 기류:** 바람과 기류가 같은 방향으로 중첩되어 상승력이 2배가 된다. 수직 이동의 극단적 가속.
- **풍 + 부서지는 바닥(9) → ID 0 (즉시 붕괴):** 바람이 약한 구조물을 즉시 무너뜨린다. 도끼의 즉시 파괴와 동일한 결과를 원거리에서 달성.
...
| 지형 \ 원소 | 화 (Fire) | 빙 (Ice) | 뇌 (Thunder) | 풍 (Wind) |
| :---------- | :-------- | :-------- | :------------ | :-------- |
| **물 (2)** | — *(변환됨)* | — *(변환됨)* | **감전 확산** — 연결된 모든 물 타일 flood-fill 감전 구역화 (2초, Shock 상태이상) | **파도** — 물 위 엔티티 수평 강제 밀기 (넉백 ×1.5) |
| **얼음 (4)** | — *(변환됨)* | — | **감전 확산** — 연결된 모든 얼음 타일 flood-fill 감전 구역화 (얼음은 도체) | **한기 바람** — 얼음 위 이동 속도 추가 ×0.7 (기본 ×0.2에 누적), 지속 5초 |
| **가시 (5)** | — | **얼음 가시** — 접촉 시 HP 10% 고정 피해 + 빙결(Freeze) 상태이상 동시 적용 (2초) | — | — |
| **거미줄 (6)** | — *(변환됨)* | — *(변환됨)* | — *(변환됨)* | — *(변환됨)* |
| **상승 기류 (7)** | **화염 기류** — 기류 내 진입 시 화상(Burn) 상태이상 지속 적용. 기류는 유지 | — *(변환됨)* | **플라즈마 기류** — 기류 내 진입 시 감전(Shock) 상태이상 지속 적용 | — *(강화됨)* |
| **어둠 (8)** | — | — | — | — |
| **부서지는 바닥 (9)** | — *(변환됨)* | — *(변환됨)* | — | — *(변환됨)* |
...
- **풍 + 물(2) 파도:** 물 위에 있는 엔티티를 수평으로 강제 이동시킨다. 물 속에서는 이동이 느려지지만, 파도는 그 저항을 무시한다. 물 구역을 건너는 적을 가시 쪽으로 밀거나, 플레이어가 빠르게 탈출하는 데 활용 가능.
- **풍 + 얼음(4) 한기 바람:** 얼음 위에서 바람이 불면 마찰이 0에 가까워져 사실상 제동이 불가능해진다. 넉백을 받으면 극단적으로 멀리 밀린다.
...
얼음(4) 자연 배치 (마찰 ×0.2)
    ...
    빙(Ice) 공격 → 가시(5) 얼음 가시 강화
           ↓
    풍(Wind) 공격 → 얼음(4) 한기 바람 효과 (마찰 ≈ 0)
           ↓
적이 얼음 위에 진입 → 사실상 멈출 수 없음 → 얼음 가시로 밀려들어감
           ↓
        HP 10% 고정 피해 + 빙결(Freeze) 1.5초
```

얼음 지형 + 가시 조합에 풍까지 더하면 엔티티가 완전히 통제력을 잃고 함정에 빠진다. 3단계 원소가 모두 관여하는 고급 체인.
...
```
풍(Wind) 공격 → 부서지는 바닥(9) 즉시 붕괴 → ID 0
                               ↓
...
부서지는 바닥을 풍으로 미리 제거하면, 그 아래에 숨겨진 물 구역이 드러난다. 적을 가까이 유인하고 뇌를 쏘면 추락 + 감전의 완벽한 처치 콤보가 성립한다.
...
| **대검 (Greatsword)** | 빙 (Ice) | 얼음(4) + 풍 (Wind) | 빙으로 물을 얼린 뒤, 풍 강공격으로 한기 바람을 적용한다. 얼음 위 적에게 대검 강공격 넉백 ×2 + 풍 추가 밀기 = 벽까지 날아가는 극단적 밀어내기. |
| **도끼 (Axe)** | 화 (Fire) | 거미줄(6) → 상승 기류(7) | 도끼 화 속성 강공격으로 거미줄을 소각해 기류를 개통한다. 이후 화를 다시 쏘아 화염 기류를 만든다. 도끼는 지형 자체를 변형하여 전장을 재설계하는 무기다. |
| **창 (Spear)** | 뇌 (Thunder) | 물(2) + 상승 기류(7) | 물에 뇌를 쏘아 flood-fill 감전 구역을 만든 뒤, 기류로 올라가 공중 찌르기 체공 전투를 유지한다. 감전 구역이 지상을 통제하는 동안 창은 공중에서 안전하게 딜을 넣는다. |
...
| **채찍 (Whip)** | 풍 (Wind) | 부서지는 바닥(9) + 상승 기류(7) | 채찍의 풍 속성 공격으로 원거리에서 부서지는 바닥을 즉시 붕괴시키면서 스윙 이동으로 기류를 타고 공중을 유지한다. 지상을 닿지 않고 전투 구역을 재편하는 극단적 기동 전략. |
...
| 풍 + 거미줄(6) → ID 0 | LOW | P1 | 화 소각과 동일 로직, 트리거 원소만 다름 |
| 풍 + 부서지는 바닥(9) → 즉시 붕괴 | LOW | P1 | `crumbleDelay = 0` 조건 추가 (화와 동일) |
...
| 풍 + 얼음(4) → 한기 바람 | MED | P2 | `speedMult` 임시 버프 타이머 관리 필요 |
| 풍 + 물(2) → 파도 강제 이동 | MED | P2 | 물 타일 위 엔티티에 수평 impulse 적용 |
| 풍 + 상승 기류(7) → 기류 강화 | MED | P2 | `updraftForce × 2` 조건 적용 |
...
| 광 + 어둠(8) → ID 0 제거 | MED | P2 | 광 속성 히트박스 충돌 시 어둠 타일 ID → 0 |
...
| 원소 | 지형 | 제외 이유 |
| :--- | :--- | :--- |
| 화, 뇌, 풍 | 가시(5) | 가시는 금속/돌 고정 구조물. 열, 전기, 바람으로 변형되지 않는다 (빙 강화 제외) |
| 빙, 뇌, 풍 | 어둠(8) | 어둠은 물리 지형이 아닌 시각 마스크. 물리 원소가 영향을 줄 수 없다 |
| 화, 빙, 뇌, 풍 | 빈 공간(0), 솔리드(1), 원웨이(3) | 기본 구조 지형은 변환 불가 (생성 고정 구조물) |
...
| 대검 | 얼음(4), 가시(5), 물(2) | 부서지는 바닥(9) | 빙 → 물 동결 후 풍 한기 바람으로 극단적 밀어내기 |
| 도끼 | 부서지는 바닥(9), 거미줄(6), 상승 기류(7) | 얼음(4) | 화 → 거미줄 소각으로 기류 개통 → 화염 기류 전환 |
| 창 | 상승 기류(7), 물(2) | 좁은 공간 | 뇌 → 물 감전 후 기류 타고 공중전 유지 |
| 지팡이 | 물(2), 얼음(4), 상승 기류(7) | 거미줄(6) | 빙 → 물 동결 → 뇌 flood-fill 감전 확산 (번개 홍수) |
| 활 | 어둠(8), 높은 지형, 거미줄(6) | 물(2) | 화 → 거미줄 소각으로 어둠 조명 + 경로 개척 동시 수행 |
| 채찍 | 상승 기류(7), 높은 천장, 부서지는 바닥(9) | 얼음(4) | 풍 → 부서지는 바닥 원거리 즉시 붕괴 + 기류 스윙 이동 |
