...
> **원소 공격 조건:** 해당 원소 이노센트가 장비에 장착되어 있어야 한다. 이노센트 미장착 시 모든 공격은 무(None) 속성이다. 원소 속성의 유일한 획득 경로는 이노센트이며, 스킬트리나 무기 고유 속성으로는 원소 공격이 불가능하다. 월드 보스가 각 원소 이노센트를 확정 드랍한다 (`System_Enemy_BossDesign.md` 참조).
