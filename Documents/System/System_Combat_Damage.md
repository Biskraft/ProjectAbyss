...
| 빙 (Ice) | 파랑   | 빙결 (Freeze)     | `freeze_duration_s`초간 이동 불가, 피해 `freeze_damage_bonus`×100% 증가 |
...
| 세이브 포인트        | 즉시 전회복 (텔레포트 진입 시) | —        | 사교 공간, 준비 공간   |
...
| 세이브 포인트 진입               | 즉시 전회복        | —                  | 세이브 포인트 배경 BGM 전환과 동시           |
...
| 지속 (DoT)     | DEF       | 없음      | 상태이상(화상/독). 방어 `dot_defense_factor` 비례 감산 적용 |
...
| 세이브 포인트에서 전투 행동 시도                          | 세이브 포인트에서는 전투 불가. In-Combat 상태 전이 없음. HP 전회복 유지           |
