...
| DMG-08-C   | 회복       | MP 공격 적중 회복                    |    P1    | 📅 대기      | 기본 공격=MP 연료 구조        |
| DMG-08-D   | 회복       | 포션 시스템                          |    P2    | 📅 대기      | 공간별 차등 (최대 3개 소지)   |
| DMG-08-E   | 회복       | HP/MP 오브 드랍                      |    P1    | 📅 대기      | 녹색=HP, 파란색=MP 오브       |
...
6가지 원소가 존재하며, 상성 관계에 따라 데미지 배율이 변동한다.

> **원소 공격 조건:** 에코(Echo)로 장착 무기에 인챈트를 적용해야 한다 (`System_Combat_Action.md` §2.2-A 참조). 인챈트 미적용(무 속성) 상태에서는 모든 공격이 물리 데미지만 발생한다. **에코의 원소 획득 경로:** 화(Fire)는 에코 기본 내장. 나머지 4원소(빙/뇌/풍/광)는 월드 보스 처치 시 에코가 영구 흡수. 원소 이노센트(Burner 등)는 인챈트 효과를 강화하는 보조 수단이지 원소 활성화 수단이 아니다.
>
> **원소 해금:** 화(Fire)는 게임 시작부터 사용 가능. 나머지 4원소(빙/뇌/풍/광)는 월드 보스 처치 시 순서대로 해금된다 (`System_Innocent_Core.md` §2.2 원소 해금 순서 참조).
>
> **원소 이노센트의 역할:** 원소 이노센트(Burner/Freezer/Shocker/Galer/Luminer)는 원소 공격의 조건이 아니라, **인챈트 효과를 강화**하는 역할이다. 이노센트 장착 시 해당 원소의 데미지 배율 상승, 상태이상 적용 확률 상승, 지형 반응 효과 지속시간 증가가 적용된다. 상세 수치는 `System_Innocent_Core.md` §2.2 원소 이노센트 규칙 참조.
...
Hollow Knight의 "공격=회복 연료" 철학을 채택한다. HP 회복의 주 경로를 전투 행동(적 처치)과 연동하여, 적을 피해 다니는 소극적 플레이보다 적극적인 전투가 생존에 유리한 구조를 만든다. 단, Hollow Knight와 달리 Project Abyss는 3-Space 간 긴장감 차등이 필요하므로 공간별로 회복률을 조정한다.
...
### 2.10. MP 회복 시스템 (MP Recovery System)
...
| 패링 성공               | 최대MP × 10%       | 최대MP × 10%       | 패링 이펙트와 동시 파란 파티클      |
...
### 2.11. HP/MP 오브 드랍 시스템 (Recovery Orb Drop System)
...
recovery:
  ...
  # --- MP 전투 외 자동 회복 (MP Out-of-Combat Regen) ---
  mp_regen_threshold: 0.50             # 회복 발동 임계값 (현재MP/최대MP < 50%)
  mp_regen_rate_world: 0.03            # 월드 초당 MP 회복률 (최대MP 3%)
  mp_regen_rate_item_world: 0.02       # 아이템계 초당 MP 회복률 (최대MP 2%)
  mp_regen_tick_ms: 1000               # 회복 틱 간격 (1초)
  ...
  # --- MP 전투 중 회복 (MP In-Combat Recovery) ---
  mp_attack_hit_world: 0.02            # 월드 기본 공격 적중 MP 회복 (최대MP 2%)
  mp_attack_hit_item_world: 0.015      # 아이템계 기본 공격 적중 MP 회복 (최대MP 1.5%)
  mp_combo_finish_world: 0.05          # 월드 3타 콤보 완료 보너스 (최대MP 5%)
  mp_combo_finish_item_world: 0.03     # 아이템계 3타 콤보 완료 보너스 (최대MP 3%)
  mp_parry_success: 0.10               # 패링 성공 MP 회복 (최대MP 10%, 공간 무관)
  mp_kill_world: 0.05                  # 월드 적 처치 MP 회복 (최대MP 5%)
  mp_kill_item_world: 0.03             # 아이템계 적 처치 MP 회복 (최대MP 3%)
  mp_potion_regen: 0.40                # MP 포션 사용 회복 (최대MP 40%)

  # --- 포션 드랍 확률 (Potion Drop Chance) ---
  hp_potion_drop_chance: 0.05          # 일반 적 HP 포션 드랍 확률 (5%)
  mp_potion_drop_chance: 0.03          # 일반 적 MP 포션 드랍 확률 (3%)
  ...
  mp_orb_regen: 0.01                   # MP 오브 1개당 회복 (최대MP 1%)
...
| 시나리오                                        | 처리 방식                                                                   |
| :---------------------------------------------- | :-------------------------------------------------------------------------- |
| 암 원소 저주(Curse) 상태에서 회복 시도          | 저주 상태는 회복량을 50% 감소시킨다. 모든 회복 트리거에 적용 (포션 포함)    |
...
* [ ] 패링 성공 시 MP 10% 회복이 공간 무관하게 적용되는가
...
* [ ] 암 원소 저주(Curse) 상태에서 모든 회복량이 50% 감소하는가
