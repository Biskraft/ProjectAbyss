...
> **원소 공격 조건:** 에코(Echo)로 장착 무기에 인챈트를 적용해야 한다 (`System_Combat_Action.md` §2.2-A 참조). 인챈트 미적용(무 속성) 상태에서는 모든 공격이 물리 데미지만 발생한다. 스킬트리나 무기 고유 속성으로는 원소 공격이 불가능하다. 에코 인챈트가 원소의 유일한 공급 경로이다.
>
