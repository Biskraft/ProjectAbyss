...
> "눈에 보이는 성장이 수치로 증명되고, 아이템계 한 층 한 층이 체감되는 데미지"

데미지 공식은 캐릭터 성장(레벨, 장비, 이노센트)의 결과를 전투에서 직접 확인할 수 있어야 한다. 아이템계에서 장비를 10층 더 강화한 결과가 데미지 숫자로 즉시 체감되는 것이 핵심 설계 목표이다.
...
| Item World 야리코미            | 아이템계 층 클리어 = 장비 레벨업 = ATK/DEF 상승 = 데미지 숫자 증가. 성장의 직접 증거    |
...
| 문제                                                    | 해결 방향                                                                  |
| :------------------------------------------------------ | :------------------------------------------------------------------------- |
| 레벨/장비 격차로 콘텐츠가 무의미해지지 않는가            | 아이템계 층수별 적 스케일링 + 레벨 차이 보정으로 항상 도전적인 전투 유지   |
...
| :--------------- | :------------------------------------- | :----------------------------------------- |
| ATK 극대화 빌드  | DEF 부족으로 피격 시 대량 피해         | 최고 DPS, 빠른 층 클리어                   |
...
### 2.8. 아이템계 층수 보정 (Item World Floor Scaling)

아이템계에서 적의 강도는 층수에 비례하여 증가한다.

```
Floor_Enemy_ATK = Base_ATK * (1 + Floor_Number * Floor_ATK_Scale)
Floor_Enemy_DEF = Base_DEF * (1 + Floor_Number * Floor_DEF_Scale)
Floor_Enemy_HP  = Base_HP  * (1 + Floor_Number * Floor_HP_Scale)
```

| 파라미터          | 값     | 설명                        |
| :---------------- | :----- | :-------------------------- |
| Floor_ATK_Scale   | 0.03   | 층당 ATK 3% 증가            |
| Floor_DEF_Scale   | 0.025  | 층당 DEF 2.5% 증가          |
| Floor_HP_Scale    | 0.04   | 층당 HP 4% 증가             |

100층 기준: ATK 4.0배, DEF 3.5배, HP 5.0배.

보스 층(10, 20, ... 100)에서는 추가 배율이 적용된다:
...
```yaml
item_world_scaling:
  # --- 층수별 적 스케일링 (Floor Enemy Scaling) ---
  floor_atk_scale: 0.03                # 층당 ATK 3% 증가
  floor_def_scale: 0.025               # 층당 DEF 2.5% 증가
  floor_hp_scale: 0.04                 # 층당 HP 4% 증가

  # --- 보스 추가 배율 (Boss Multipliers) ---
  boss_general:                        # 아이템 장군 (10, 20, 30층)
    atk_multi: 1.5
    def_multi: 1.3
    hp_multi: 2.0
  boss_king:                           # 아이템 왕 (40, 50, 60층)
    atk_multi: 2.0
    def_multi: 1.5
    hp_multi: 3.0
  boss_god:                            # 아이템 신 (70, 80, 90층)
    atk_multi: 2.5
    def_multi: 2.0
    hp_multi: 5.0
  boss_grand_god:                      # 아이템 대신 (100층)
    atk_multi: 3.0
    def_multi: 2.5
    hp_multi: 8.0
...
### 5.5. 아이템계 깊은 층수 예외 (Deep Floor Edge Cases)

| 시나리오                                | 처리 방식                                                         |
| :-------------------------------------- | :---------------------------------------------------------------- |
| 100층 적의 스탯이 플레이어보다 압도적   | 의도된 설계. 장비 강화(야리코미)를 통해 극복하는 것이 핵심 루프   |
| 재귀 아이템계(깊이 2~3)에서의 적 스케일링| 재귀 깊이 보정: 깊이 2는 적 HP +20%, 깊이 3은 +50%               |
| Ancient 등급 아이템의 100층 보너스       | 100층 이후 5층 단위 추가 보스 등장. 보너스 10층(총 110층)         |
