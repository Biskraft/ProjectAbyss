...
> **원소 공격 조건:** 해당 원소 이노센트가 장비에 장착되어 있어야 한다. 이노센트 미장착 시 모든 공격은 무(None) 속성이다. 원소 속성의 유일한 획득 경로는 이노센트이며, 스킬트리나 무기 고유 속성으로는 원소 공격이 불가능하다. **예외: 에코(Echo)는 대장간 화로에서 단련된 망치로 화(Fire) 속성을 기본 보유한다.** 나머지 4원소(빙/뇌/풍/광)는 월드 보스 확정 드랍 이노센트로 획득 (`System_Enemy_BossDesign.md` 참조).
