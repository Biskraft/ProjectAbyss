...
| EQP-06-A | 전생     | 아이템 전생 (Reincarnation)         |    P2    | 대기      | Phase 2 추가                        |
...
Project Abyss의 장비 슬롯 시스템은 다음 한 문장으로 정의한다:

> "아이템이 캐릭터의 일부가 되어, 아이템을 강화하는 것이 곧 나 자신을 강화하는 것이다"

장비 슬롯은 단순한 스탯 교체 창이 아니다. 각 장비 아이템은 내부에 아이템계(던전)를 품고 있으며, 플레이어는 그 아이템을 착용함으로써 그 장비의 강함을 전투에서 직접 증명한다. "이 검은 내가 100층을 직접 클리어하며 키운 검"이라는 감정적 애착이 야리코미의 핵심 판타지이다.
...
| 아이템 전생을 Phase 2로 연기 | 전생은 반복 플레이의 극한 콘텐츠. Phase 1에서 기본 루프 안정화 후 추가                |

### 1.3. 3대 기둥 정렬 (Pillar Alignment)

| 기둥                  | 장비 슬롯 시스템에서의 구현                                                              |
| :-------------------- | :--------------------------------------------------------------------------------------- |
| Metroidvania 탐험     | 장비 ATK/DEF/INT 등이 스탯 게이트 해금 조건. 더 강한 장비 = 새 구역 접근 가능            |
...
| 전생(Reincarnation)이 기존 강화 노력을 초기화하는가? | 전생은 "기반 스탯 재설정 + 잠재력 상한 증가"로 설계. 이노센트 슬롯 수는 유지하여 파밍 가치 보존 (Phase 2 정의) |

### 1.5. 위험과 보상 (Risk & Reward)

| 전략                     | 위험 (Risk)                              | 보상 (Reward)                                  |
| :----------------------- | :--------------------------------------- | :--------------------------------------------- |
| 고레어리티 장비 착용     | 아이템계 100층 클리어 부담               | 최대 스탯 배율 + 이노센트 슬롯 8개             |
| 저레어리티 장비 강화 유지 | 고레어리티 대비 스탯 열세               | 이미 키운 이노센트 활용 가능, 아이템계 층수 부담 감소 |
| 여러 슬롯 고르게 강화    | 각 아이템계에 드는 시간 분산             | 균형 잡힌 스탯, 세트 효과 활성화 가능           |
| 한 슬롯(무기)에 집중     | 방어 스탯 부족                           | 최고 공격 스탯, 빠른 층 클리어                  |
...
| EQP-R04 | 아이템계 탐험 중에도 착용/해제 가능 (허브 복귀 후) | 아이템계 내부에서의 조작은 허브 복귀 후 적용 |
...
| Phase 2 | 무기, 보조무기, 머리, 갑옷, 망토, 장신구 x2 | 세트 효과, 아이템 전생 추가     |
| Phase 3 | Phase 2 유지 + 특수 슬롯 검토          | 시즌/이벤트 장비 슬롯 확장 검토  |
...
### 3.5. 아이템 전생 규칙 (Reincarnation Rules) - Phase 2

- 아이템계 100층 클리어 후 아이템 전생 가능
- 전생 시 아이템 레벨 1 초기화, 레어리티 유지, 이노센트 슬롯 수 유지
- 기반 스탯 상한 상승 (전생 횟수에 비례)
- 전생 최대 횟수: `EQP_MAX_REINCARNATION` 파라미터 참조
- MVP에서는 데이터 구조만 정의하며 기능 미구현
...
item_data_structure:
  ...
  itemWorldDepth: 30               # 아이템계 최대 층수 (레어리티에 따라 결정)
  currentItemWorldFloor: 0         # 현재 클리어된 아이템계 층수
  reincarnationCount: 0            # 전생 횟수 (Phase 2)
...
mvp_base_values:
  ...
  stat_gate_example:
    zone_b_str_gate: 20    # STR 20 이상 시 B구역 통과 가능 (MVP 예시)
    # 장비 EquipStat 합산으로 달성해야 하는 값
```

### 4.4. 전생 파라미터 (Reincarnation Parameters) - Phase 2

```yaml
reincarnation_params:
  EQP_MAX_REINCARNATION: 10        # 아이템당 최대 전생 횟수
  EQP_REINCARNATION_STAT_BONUS: 0.05  # 전생 1회당 기반 스탯 상한 증가율 (5%)
  EQP_REINCARNATION_UNLOCK_FLOOR: 100 # 전생 가능 조건: 아이템계 100층 클리어
  # Phase 2에서 상세 정의
```
...
| 아이템계 탐험 중 착용 슬롯 변경 시도 | 아이템계 탐험 중 착용 변경 불가. 허브 복귀 후 가능 안내                 |
