...
스탯 시스템은 캐릭터 레벨, 장비, 이노센트라는 세 성장 축을 하나의 공식으로 통합한다. 플레이어가 아이템계 한 층을 돌파했을 때 STR이 올라 데미지 숫자가 커지는 것을 즉시 체감하는 것이 핵심 설계 목표이다.
...
| Item World 야리코미   | 아이템계 클리어 → 장비 강화 → EquipStat 상승 → FinalStat 상승 → 더 깊은 층 진입       |
