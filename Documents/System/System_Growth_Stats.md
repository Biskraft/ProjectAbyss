...
| STAT-02-A  | 공식       | FinalStat 합산 공식                     |    P1    | 📅 대기   | Base + Equip + Memory Shard            |
...
| STAT-04-C  | 전투 연동  | ~~DEX → 명중/회피/크리티컬~~ (삭제)     |    —     | ❌ 폐기   | 기억 단편로 대체                    |
| STAT-04-D  | 전투 연동  | HP → MaxHP 연동                          |    P1    | 📅 대기   | 레벨 + 장비 HP 합산               |
| STAT-04-E  | 전투 연동  | ~~SPD → 이동/공격 속도~~ (삭제)          |    —     | ❌ 폐기   | 기억 단편로 대체                    |
| STAT-04-F  | 전투 연동  | ~~LCK → 드랍률/크리티컬~~ (삭제)         |    —     | ❌ 폐기   | 기억 단편로 대체                    |
| STAT-05-A  | 기억 단편   | 기억 단편 보너스 합산 처리               |    P1    | 📅 대기   | System_Memory Shard_Core.md 연동       |
...
ECHORIS의 성장 스탯 시스템은 다음 한 문장으로 정의한다:

> "수치 하나 오를 때마다 전투에서 체감되고, 아이템계를 클리어하는 이유가 스탯에 있다"

스탯 시스템은 캐릭터 레벨, 장비, 기억 단편라는 세 성장 축을 하나의 공식으로 통합한다. 플레이어가 아이템계 한 지층을 돌파했을 때 ATK가 올라 데미지 숫자가 커지는 것을 즉시 체감하는 것이 핵심 설계 목표이다.
...
| FinalStat = Base + Equip + Memory Shard | 세 성장 축의 기여가 눈에 보인다. 어느 축을 올려야 효율적인지 플레이어가 판단 가능  |
...
| 크리티컬/드랍률은 기억 단편로   | 기억 단편 파밍의 가치를 높이고 기본 스탯 구조를 단순화. 야리코미 특화                        |
...
| Online 멀티플레이     | 파티원 역할 분담. ATK 딜러 / HP 탱커 / INT 마법사 등 빌드 다양성 (기억 단편로 세분화)  |

### 1.4. 저주받은 문제 검증 (Cursed Problem Check)

| 문제                                                          | 해결 방향                                                                        |
| :------------------------------------------------------------ | :------------------------------------------------------------------------------- |
| 기억 단편 스태킹으로 특정 스탯이 폭주하지 않는가              | 기억 단편 레벨 상한 + 슬롯 수 상한으로 기억 단편 보너스 총량 제어 (System_Memory Shard_Core.md) |
| 레벨 캡 이후 성장 동기가 사라지지 않는가                     | MVP에서 캡(Lv 10) 도달 후 장비/기억 단편 성장으로 계속 진행. 레벨 캡은 Phase 2에서 확장   |
| ATK 극대화로 다른 스탯이 무의미해지지 않는가                 | HP가 생존에 필수, INT가 원소 게이트 해금에 필수. 기억 단편로 세부 빌드 분화          |
| ~~SPD 극대화~~ (삭제됨)                                      | 이동/공격속도는 기억 단편로만 보정. 상한은 기억 단편 레벨 캡으로 제어               |
| 장비 교체 시 기억 단편 보너스가 사라지는 것이 손실이 너무 크지 않은가 | 기억 단편는 장비에 귀속. 교체 비용 설계로 의사결정 긴장감 부여 (System_Memory Shard_Core.md) |
...
| 기억 단편 특화        | 기본 전투 스탯 분산, 평균 딜 낮음             | 크리티컬/드랍률/이동속도 보정으로 파밍 효율 극대화     |
...
ECHORIS의 모든 전투, 탐험, 성장 계산은 다음 3대 기본 스탯을 기반으로 한다.
...
| ~~DEX~~ | ~~민첩~~      | —          | ~~삭제: 기억 단편로 대체~~                              | 크리티컬/명중/회피 → 기억 단편  |
| ~~VIT~~ | ~~체력~~      | —          | ~~삭제: HP로 통합~~                                    | MaxHP, DEF → HP + 장비 DEF    |
| ~~SPD~~ | ~~속도~~      | —          | ~~삭제: 기억 단편로 대체~~                              | 이동/공격속도 → 기억 단편       |
| ~~LCK~~ | ~~행운~~      | —          | ~~삭제~~                                               | 드랍률/크리티컬 → 기억 단편     |
...
- 이동/공격속도는 기억 단편로만 보정된다.

#### ~~LCK (행운)~~ — 삭제됨

- 드랍률, 크리티컬은 기억 단편로만 보정된다.
...
| Memory ShardBonus | 복종 상태의 기억 단편가 부여하는 스탯 보너스 합산  | Sheets/Content_System_Memory Shard_Pool.csv |
...
3. 복종 상태인 기억 단편의 보너스를 모두 합산하여 Memory ShardBonus를 구한다.
4. `FinalStat = BaseStat + EquipStat + Memory ShardBonus`
...
| 기억 단편 상태 변경 시   | Memory ShardBonus 재합산 후 FinalStat 갱신                |
| 전투 중 버프/디버프 발생 | FinalStat 기반 2차 파생 스탯에 배율 적용 (런타임)     |

### 3.3. 스탯 표시 규칙

- 스탯 패널에는 FinalStat을 표시한다.
- 항목별로 BaseStat / EquipStat / Memory ShardBonus 분해 값을 툴팁으로 제공한다.
...
- EquipStat = 0으로 처리한다.
- FinalStat = BaseStat + Memory ShardBonus로 계산된다.
...
stat_formula:
  # FinalStat 합산
  final_stat: "BaseStat + EquipStat + Memory ShardBonus"
...
```yaml
# Lv 1, 장비 미착용, 기억 단편 없음 기준 (3스탯 체계)
example_lv1_no_equip:
  ...
  FinalStat:
    ...
    INT: 8
    MaxHP: 100
  # DEF는 장비 없으므로 0. 크리티컬/드랍률은 기억 단편 없으므로 기본값.
```
...
example_lv5_normal_sword:
  ...
  # DEF는 장비에서 직접 제공. 크리티컬은 기억 단편 없으므로 기본 5%.
```
...
| Memory ShardBonus가 음수인 경우             | 저주(Curse) 기억 단편 구현 전까지 Memory ShardBonus는 0 이상만 허용 |
...
- 스택 배율 우대 없음 (선형 합산). 이 규칙은 기억 단편 보너스에도 동일 적용된다.
...
| 검증 항목                                            | 기대 결과                                                     |
| :--------------------------------------------------- | :------------------------------------------------------------ |
| Lv 1 캐릭터, 장비/기억 단편 없음                      | FinalStat = BaseStat. MaxHP = 200, MaxMP = 107               |
...
| 기억 단편 유무에 따른 드랍률 차이 체감             | 기억 단편 보정으로 장기 파밍에서 드랍 빈도 차이 인식          |
...
| 기억 단편 → 이동/공격 속도 반영 여부    | System_3C_Character.md         |
| 기억 단편 → 크리티컬/드랍률 반영 여부   | System_Combat_Damage.md        |
| EquipStat → 레어리티 배율 적용 여부    | System_Equipment_Slots.md      |
| Memory ShardBonus → 복종 상태 기억 단편 합산 | System_Memory Shard_Core.md       |
