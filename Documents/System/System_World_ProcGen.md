...
* 구역 상세 설계: `Documents/Design/Design_World_ZoneMap.md`
...
Project Abyss의 월드 생성은 **핸드크래프트의 감성과 절차적 생성의 리플레이성**을 동시에 달성하는 하이브리드 시스템이다. 플레이어는 매 시즌 새로운 던전 구조를 탐험하면서도, 각 구역이 가진 고유한 분위기와 내러티브 흐름은 일관되게 경험한다.
...
| 계층                | 생성 방식      | 대상                                      | 레퍼런스                                 |
| :------------------ | :------------- | :---------------------------------------- | :--------------------------------------- |
| 매크로 (Macro)      | 핸드크래프트   | 구역 배치, 구역 간 연결, 내러티브 흐름     | 월하의 야상곡 (성 구조 고정 배치)         |
| 마이크로 (Micro)    | 절차적 생성    | 구역 내부 Room Grid, Chunk 배치, 오브젝트  | 스펠렁키 (4x4 Grid), 데드셀 (PCG 파이프라인) |

- **매크로 계층**: 7개 구역(중앙 성채, 마법 연구소, 빙결 동굴, 지하 수로, 묘지/카타콤, 천공의 탑, 심연의 구)의 위치와 연결 관계는 모든 시드에서 동일하다. 탐험의 방향성과 내러티브 흐름을 보장한다.
- **마이크로 계층**: 각 구역 내부의 Room 배치, Room 내 Chunk 조합, 적/아이템/함정 배치는 시드에 의해 절차적으로 생성된다. 리플레이 가치를 제공한다.

### 1-3. 문제-해결 점검 (Cursed Problem Check)

| 문제                                            | 해결 방안                                                                                 |
| :---------------------------------------------- | :---------------------------------------------------------------------------------------- |
| 절차적 생성이 탐험감을 해치지 않는가?            | 매크로 구조는 고정하여 구역 간 이동의 감성을 유지한다. 마이크로만 절차적으로 변형한다.      |
| 모든 시드에서 클리어가 가능한가?                 | Critical Path 알고리즘이 생성 후 경로 유효성을 검증한다. 실패 시 재시드한다.                |
| 시즌 리셋 시 플레이어 경험이 단절되는가?         | 매크로 구조가 동일하므로 방향감은 유지된다. 마이크로 변화가 신선함을 제공한다.              |

### 1-4. 리스크와 보상 (Risk & Reward)

- **리스크**: Chunk 풀 부족 시 구역 내 반복감 증가. 바이옴별 최소 40개 Chunk를 확보해야 한다.
- **보상**: 시즌마다 새로운 내부 구조로 탐험 동기를 지속적으로 부여한다.
...
```mermaid
flowchart TD
    A["Phase 1: 매크로 구조 로드\n(구역 연결 그래프)"] --> B["Phase 2: 구역별 Room Grid 생성\n(3x3 ~ 5x5)"]
...
서버에 저장된 구역 연결 그래프(Zone Connectivity Graph)를 로드한다. 이 그래프는 모든 시드에서 동일하다.

```mermaid
graph LR
    CK["중앙 성채\n(Central Keep)"] --- ML["마법 연구소\n(Magic Lab)"]
    CK --- IC["빙결 동굴\n(Ice Cavern)"]
...
- **Action**: 서버가 시즌 시드를 결정하고, 매크로 그래프를 로드한다.
- **Reaction**: 각 구역의 Grid 크기와 바이옴이 확정된다.
- **Effect**: 구역 간 연결 포인트(Portal)의 위치가 고정된다.

### 2-3. Phase 2 — 구역별 Room Grid 생성 (Room Grid Generation)

각 구역은 구역별로 정해진 크기의 2D Grid를 생성한다. Grid의 각 셀이 하나의 Room이다.
...
- **Action**: 구역 난이도에 따른 몬스터 밀도 테이블을 참조하여 적 스폰 포인트를 결정한다.
...
| 바이옴 ID          | 구역명          | 최소 Chunk 수 | 특수 Chunk                        |
| :------------------ | :-------------- | :-----------: | :-------------------------------- |
| castle_interior     | 중앙 성채       |      40       | 왕좌실, 무기고, 대회랑            |
| arcane              | 마법 연구소     |      40       | 마법진, 포탈 방, 실험실           |
| ice                 | 빙결 동굴       |      40       | 빙벽, 미끄러운 바닥, 고드름 함정  |
| water               | 지하 수로       |      40       | 수중 통로, 수문, 급류             |
| dark                | 묘지/카타콤     |      40       | 석관, 함정 통로, 은밀한 방        |
| sky                 | 천공의 탑       |      40       | 부유 발판, 강풍 구간, 전망대      |
...
구역 난이도(Difficulty)에 따라 Room 내 전투 타일 비율을 결정한다.
...
| 게이트 종류                   | 배치 위치              | 용도                                  |
| :---------------------------- | :--------------------- | :------------------------------------ |
| 능력 게이트 (Ability Gate)    | 구역 경계에만 배치      | 특정 능력(이중 점프 등) 필요           |
| 스탯 게이트 (Stat Gate)       | 구역 내부에도 배치 가능 | 특정 스탯 임계값(공격력, 방어력) 필요  |

- 능력 게이트는 Critical Path 위에 배치되지 않는다. Critical Path는 기본 능력만으로 통과 가능하다.
- 스탯 게이트는 보상 경로(비경로)에 배치하여 선택적 도전을 제공한다.
- 구역 경계의 능력 게이트는 해당 능력을 획득할 수 있는 구역이 반드시 선행 구역이어야 한다.
...
3. 각 구역 내부의 Critical Path가 Type 0 Room에 의해 차단되지 않았는지 확인한다.
...
world_procgen:
  ...
  zones:
    central_keep:
      ...
      is_start_zone: true
    magic_lab:
      display_name: "마법 연구소 (Magic Lab)"
      grid_size: "4x3"
      difficulty: 3
      biome: "arcane"
      is_start_zone: false
    ice_cavern:
      display_name: "빙결 동굴 (Ice Cavern)"
      grid_size: "3x4"
      difficulty: 3
      biome: "ice"
      is_start_zone: false
...
- 폴백 맵은 각 구역의 최소 Grid(3x3)로 구성된 사전 검증 완료 맵이다.
...
### 5-3. 시즌 리셋 중 접속한 플레이어 처리 (Season Reset During Play)

- 시즌 리셋은 서버 점검 시간에만 실행한다. 리셋 30분 전 경고 알림을 전송한다.
- 리셋 시점에 접속 중인 플레이어는 강제 로비 복귀 처리한다.
- 플레이어의 인벤토리, 레벨, 능력 등 캐릭터 데이터는 리셋 전에 저장을 완료한다.
- 신규 시드의 월드는 점검 시간 중에 사전 생성하여 검증을 마친다.

### 5-4. 월드 생성 실패 시 폴백 (Generation Failure Fallback)

- 서버 메모리 부족, 타임아웃 등으로 생성 자체가 실패하는 경우 대비한다.
- 생성 타임아웃: 구역당 최대 5초, 전체 월드 최대 30초.
- 타임아웃 초과 시 마지막으로 성공한 월드를 재사용한다.
- 최초 생성 시에는 내장된 기본 월드(Default World)를 사용한다.

---

## 월드 생성과 아이템계 Floor Gen 비교 (World ProcGen vs. Item World Floor Gen)

| 비교 항목           | 월드 절차적 생성 (본 문서)               | 아이템계 Floor Gen (SYS-IW-02)          |
| :------------------ | :--------------------------------------- | :-------------------------------------- |
| 문서 ID             | WPG (본 문서)                            | SYS-IW-02                               |
| 생성 범위           | 전체 월드 (7개 구역)                     | 단일 아이템 내부 던전 (1개 아이템)       |
| 매크로 구조         | 핸드크래프트 (구역 배치 고정)            | 없음 (전체 절차적)                       |
| 마이크로 구조       | 절차적 (Room Grid + Chunk)               | 절차적 (Floor 단위 생성)                 |
| 시드 수명           | 3~6개월 (시즌 단위)                      | 1회 진입 시 생성, 퇴장 시 소멸           |
| Grid 크기           | 구역별 3x3 ~ 5x5                        | 층별 고정 크기                           |
| 바이옴              | 구역별 고유 바이옴 (7종)                 | 아이템 속성에 따라 결정                  |
| Critical Path       | 구역 내 + 구역 간 연결 모두 검증         | 층별 독립 검증                           |
| 난이도 결정         | 구역별 고정 난이도 (1~5)                 | 층 깊이에 비례                           |
| 게이트 시스템       | 능력 게이트 + 스탯 게이트                | 스탯 게이트만                            |
| 리플레이 주기       | 시즌 리셋 (3~6개월)                      | 매 진입마다 새로 생성                    |
