...
| 고레어리티 무기 아이템계 진입 | 많은 지층, 강한 적, 전멸 시 진행 손실                      | 높은 ATK, 더 많은 이노센트 슬롯, 레어리티 승급 가능성   |
...
| 하방 공격이 아이템계 지층 경계(바닥)에 닿는 경우 | 바운스 물리 적용 후 착지 판정. 지층 이탈하지 않는다                                     |
...
| 검 레어리티별 ATK 차이 체감                      | Normal -> Legendary 진행 시 데미지 증가가 육안으로 확인 가능                       | 더미 대상 동일 타격 데미지 수치 비교          |
...
| 히트박스 29x19가 너무 작아 전투가 불편하다                  | Normal 검 장착 후 일반 적 5마리 연속 처치 성공률                                 | 첫 플레이 기준 80% 이상 성공                           |
