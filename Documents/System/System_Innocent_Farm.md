> **⚠️ DEPRECATED:** 이 시스템은 스코프 축소로 삭제되었습니다. 이노센트의 핵심 메커닉(야생→복종)은 System_Innocent_Core.md에서 관리합니다.
...
| 팜 자동 성장 (오프라인 성장 아님) | 팜 성장이 전투 참여 횟수에 비례 → 아이템계 탐험 동기 강화. 방치 게임 느낌 방지 | 오프라인 시간 기반 성장 — 과금 게임의 "에너지 대기" 느낌, 야리코미와 정반대 |
...
| 등급 | 거래 허용 여부 | 규칙 |
| :--- | :--- | :--- |
| Common | 허용 | NPC 상점에서 매각 가능. 단, 매각 시 수수료 `Trade_Fee_Ratio`(기본값 5%) Sink 적용 |
| Uncommon | 허용 | 동일 |
...
| `Trade_Fee_Ratio` | 0.03~0.1 | 0.05 | NPC 매각 수수료 Sink 강도 |
