...
| CMB-01-DA  | 대시       | 대시 규칙 (i-frame 제거)             |    P1    | 📅 미구현    | 무적 없음, 쿨다운 400ms       |
...
PC 키보드와 게임패드 모두에서 직관적이고 접근성 높은 전투 경험을 제공하는 것이 최우선 목표이다. 개별 액션은 단일 버튼 입력으로 완결되며, 전투의 깊이는 "어떤 스킬을 언제 사용할 것인가"라는 판단에서 발생한다.

### 1.2. 설계 근거 (Reasoning)

| 결정                           | 근거                                                                                      |
| :----------------------------- | :---------------------------------------------------------------------------------------- |
| 자동 콤보 3타                  | 연타만으로 콤보가 진행되어 전투 흐름을 끊지 않음. 접근성 확보                              |
| 스킬 4슬롯 원터치              | 방향키 조합 스킬(커맨드) 대신 단일 버튼 = 단일 스킬로 1:1 매핑. 직관적 조작                |
| 자동 조준                      | 전투 쾌적성과 몰입감을 위해 타겟팅을 자동화                                                |
| 백스텝/슬라이딩 공격 제거      | 방향키+버튼 동시 조합은 복잡도를 높임. 단일 버튼 원칙에 위배                               |
...
| 대시 (이동)       | 쿨다운 400ms 소비, 공중 1회 제한       | 위치 재설정으로 공격 회피, 3타 후딜 캔슬로 안전한 이탈 |
...
| 무기 카테고리 | 1타              | 2타              | 3타                  | 특성                  |
| :------------ | :--------------- | :--------------- | :------------------- | :-------------------- |
| 블레이드 (Blade)    | 수평 베기        | 대각 베기        | 수평 대회전          | 표준형, 밸런스 (ATK 100%) |
| 클리버 (Cleaver)| 대사선 베기   | 수평 대소용돌이  | — (2타 종료)         | 느림, 충격파, 넓은 범위 (ATK 120%) |
| 시브 (Shiv) | 찌르기          | 역수 베기        | 상향 슬래시 → 4타 돌진 | 최고 속도, 배후 강타 (ATK 80%) |
| 레일보우 (Railbow)      | 속사            | 속사             | 충전 화살            | 원거리 물리, 카이팅 (ATK 90%) |
| 에미터 (Emitter)| 마법탄          | 마법탄           | 강화 마법탄          | 원거리 원소, 인챈트 증폭 (INT 100%) |

각 무기의 히트박스 크기, 활성 프레임, 피해 배율은 `Content_Stats_Weapon_List.csv`에서 참조한다.

> **7무기 정합(DEC-026) 미완:** 하픈(Harpoon)·체인(Chain) 2종 행 추가 필요. 위 5종은 명칭만 DEC-026으로 정합, 콤보·배율은 재설계 대상.
...
- 에르다는 무기 슬롯에 별도의 전투 무기(검/대검/단검/활/지팡이)를 장착하여 전투한다.
- 아이템계 진입 중 에코는 기억 단편(Memory Shard) 대상에게 공명하는 도구로서 패시브 보너스를 부여한다: **기억 단편 대상 피해 +10%** (아이템계 전용).
...
```
화(Fire) → 빙(Ice) → 뇌(Thunder) → 무(None) → 화(Fire) → ...
```
...
8. 기본 공격은 MP를 소비하지 않는다. 대시(렐릭 해금 후)도 MP를 소비하지 않는다.
...
### 2.5-A. 대시 규칙 — i-frame 제거 (Dash Rules — No Invincibility Frames)

대시에 i-frame(무적 프레임)을 부여하지 않는다. 대시는 순수 이동 기동으로, 위치 재설정을 통해 공격을 회피한다.
...
1. **대시 중 피격 가능:** 대시 모션 중에도 허트박스가 정상 유지된다 (12×28px). 적 공격 범위를 벗어나는 것이 회피 수단이다.
2. **쿨다운:** `dash_cooldown_ms`(400ms). i-frame 없이 빈번한 사용을 허용하여 기동성을 보상한다.
3. **콤보 캔슬:** 3타 후딜 중 대시 입력 시 후딜 캔슬. "위치 이탈"이 회피 가치이다.

#### 파라미터

```yaml
dash:
  dash_cooldown_ms: 400        # 쿨다운 (i-frame 없으므로 짧게 설정)
  # dash_iframe_ms: 제거됨     # 2026-04-08 대시 i-frame 완전 제거
```

#### 설계 근거

i-frame을 제거한 이유: SotN, Celeste, Ori 등 다수의 액션 레퍼런스에서 대시는 순수 이동 기동이다. i-frame이 있으면 쿨다운 주기마다 안정적인 대형 공격 회피가 보장되어 위협이 희석된다. i-frame 제거 대신 쿨다운을 400ms로 단축하여 빈번한 위치 재설정을 가능하게 했다. 회피의 핵심은 "정확한 타이밍에 무적"이 아니라 "정확한 방향으로 이탈"이다.
...
| :-------------- | :---------------- | :-------------------------------- |
| 약 (Light)      | 3                 | 기본 공격 1-2타                   |
| 중 (Medium)     | 3                 | 일반 스킬                         |
| 강 (Heavy)      | 4                 | 대형 스킬, 보스 공격              |
| 극강 (Critical) | 6                 | 크리티컬 히트, 슈퍼 아머 파괴     |
| 콤보 피니셔     | 7                 | 기본 공격 3타 (콤보 마무리) — SSoT: `Content_Combat_Combo.csv` |

### 2.6-A. 인챈트 타격 피드백 & 검 Ego 전투 코멘트 (Enchant Hit Feedback & Ego Combat Barks)
...
아이템계 내부에서 에코는 기억 단편 및 기억 구조물과 공명한다. 기억 단편 대상에게 타격을 가한 무기에서 **공명 글로우(Resonance Glow)** 이펙트가 0.3초간 표시된다. 이 글로우는 아이템계 진입 시 제단에 에코를 내리칠 때 나타나는 균열 이펙트와 동일한 색조(금빛-백색)이다. 에코가 전투 무기가 아닌 기억의 도구임을 아이템계 내부에서 시각적으로 상기시키는 장치이다.

#### 검 Ego 전투 코멘트 (Sword Ego Combat Barks)

화자는 검 Ego(녹쇠)다. 에르다 본인은 침묵한다(DEC-033). 검 Ego는 단조된 무기의 시선으로 적을 분석하고 코멘트한다. 이 코멘트는 Act 1(코미디 톤)에서 적 처치 시 **랜덤 확률**로 재생된다. 같은 코멘트가 연속으로 재생되지 않도록 최근 N회 이력을 제외한 풀에서 선택한다.

**Act 1 코멘트 예시 (처치 시 트리거):**
...
코멘트 발동 조건 및 운영 규칙은 추후 `System_NarrativeVoice.md`에서 상세 정의한다 (발동 확률, 쿨다운, Act별 풀 분리 등).
...
* 아이템계(Item World)에서는 Out-of-Combat MP 회복이 적용되지 않는다 (자원 관리 난이도 유지). MP 회복은 포션, 미스터리 룸(병원), MP 회복 기억 단편를 통해서만 가능하다.
...
3. 히트 판정은 매 프레임 수행한다.
4. 멀티 히트 공격(범위 지속 스킬 등)은 `multi_hit_interval_frames`(10프레임) 간격으로 히트 리스트를 초기화한다.
...
| 대시 중 피격 판정                       | 대시 중에도 허트박스 정상 유지. 피격 시 대시 캔슬 + Hit 상태 전이    |
| 대시 방향 벽 충돌                       | 대시 즉시 종료, 쿨다운은 소비. 벽에 고정 피해 없음                  |
| 대시와 넉백 동시 발생                   | 대시 중 피격 시 Hit 상태 전이. 넉백 정상 적용                       |
...
### 5.4. 공중 전투 예외 (Air Combat Edge Cases)
...
* [ ] 대시 중에도 피격 판정(허트박스)이 정상 유지되는가
* [ ] 대시 쿨다운이 400ms이며, 쿨다운 중 대시 입력이 무시되는가
...
* [ ] 아이템계 내부에서 기억 단편 피격 시 공명 글로우(Resonance Glow)가 0.3초간 표시되는가
* [ ] Act 1에서 검 Ego 처치 코멘트가 랜덤으로 재생되되 직전 재생 이력이 제외되는가
