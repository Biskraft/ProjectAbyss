...
| CMB-01-DA  | 대시       | 대시 i-frame 규칙                    |    P1    | 📅 미구현    | 240ms(14f), 지상/공중 차이     |
| CMB-01-DB  | 넉백       | 벽 충돌 넉백 처리 방식               |    P1    | 📅 미구현    | 정지 방식 채택 (SotN 레퍼런스) |
| CMB-01-DC  | 패링       | 패링 시스템 프레임워크               |    P1    | 📅 미구현    | 200ms 창, 근접 공격 전용       |
...
**원소 인챈트 적용:** 에르다의 모든 무기 기본 공격은 현재 인챈트된 원소를 적용한다. 인챈트 미적용(무 속성) 상태에서는 물리 데미지만 발생한다. 인챈트 시스템은 §2.2-A 참조.

#### 에코 (Echo) — 영구 장착 대장장이 도구

에코는 주인공 에르다 벤-나흐트(Erda ven-Nacht)의 대장간 망치이자 이야기의 핵심 도구이다. **전투 무기가 아니며**, 다음 두 가지 역할만 수행한다:

1. **원소 인챈트**: 무기에 원소를 입히는 인챈트 액션 (§2.2-A 참조)
2. **아이템계 진입**: 모루/제단 위의 무기를 에코로 두드려 기억의 균열을 열고 아이템계에 진입하는 행위 (기억의 두드림, Echo Strike)

**에코 장착 규칙:**
- 에코는 해제(unequip)가 불가능한 영구 도구이다. 무기 슬롯을 차지하지 않는다.
- 에르다는 무기 슬롯에 별도의 전투 무기(검/단검/대검/도끼/창/지팡이/활/채찍)를 장착하여 전투한다.
- 아이템계 진입 중 에코는 이노센트(Innocent) 대상에게 공명하는 도구로서 패시브 보너스를 부여한다: **이노센트 대상 피해 +10%** (아이템계 전용).

### 2.2-A. 원소 인챈트 시스템 (Elemental Enchant System)

에코로 자신의 무기를 두드려 원소를 입히는 행위이다. 이 인챈트가 에코의 전투 기여 방식이다.

**인챈트 액션:**

| 항목          | 내용                                                                       |
| :------------ | :------------------------------------------------------------------------- |
| 입력          | ↑+공격 버튼 (또는 전용 인챈트 버튼)                                        |
| 애니메이션    | 1초. 에르다가 무기를 잠깐 들어올리고 에코로 가볍게 두드린다                |
| 무적          | 없음. 1초 모션 동안 피격 가능. 전투 중 인챈트는 위험 부담이 존재            |
| 원소 전환     | 인챈트 시마다 다음 원소로 순환                                              |
| 지속 시간     | 변경 전까지 유지 (타이머 없음)                                              |

**원소 순환 순서:**

```
화(Fire) → 빙(Ice) → 뇌(Thunder) → 풍(Wind) → 광(Light) → 무(None) → 화(Fire) → ...
```

- 해금된 원소만 순환에 포함된다. 게임 시작 시 화(Fire)만 해금되어 있으며, 순서대로 월드 보스 처치 시 해금된다.
- 무(None)는 항상 포함 (인챈트 해제 상태).
- 순환은 단방향이다. 역방향 전환은 불가하다.
...
| :------------ | :------------------------------------------ | :------------------------------- |
| 인챈트 미적용 | 무기 기본 상태                              | 없음                             |
| 인챈트 적용 중 | 무기에 원소 색상 글로우 + 파티클 지속 표시  | 원소별 미세 잔향음 (루프)        |
| 인챈트 시전 시 | 에코가 무기를 두드리는 섬광 + 원소 파티클 폭발 | 에코 타격음 + 원소 차징 사운드   |

**설계 의도:** 인챈트 전환은 1초의 빈틈을 감수해야 하므로, 전투 전 사전 선택이 장려된다. 전투 중 상황에 따라 원소를 전환하는 것은 고위험 고보상 플레이이다. 에코가 전투에서 비주얼적으로 활성화되는 방식으로 에르다의 대장장이 정체성을 지속적으로 상기시킨다.
...
### 2.5-A. 대시 i-frame 규칙 (Dash Invincibility Frames)

대시 도중 적용되는 무적 프레임(i-frame)의 정확한 범위와 조건을 정의한다.

#### 레퍼런스 비교

| 게임            | 대시 i-frame 지속 | 설계 의도                               |
| :-------------- | :---------------- | :-------------------------------------- |
| **SotN**        | 없음              | 대시는 이동 수단, 회피는 무기 타이밍    |
| **Dead Cells**  | 롤 전구간 400ms   | 롤이 핵심 회피 메커닉                   |
| **Hollow Knight** | 없음 / Shadow Dash 전구간 | 업그레이드로 i-frame 부여      |
| **Project Abyss** | 240ms (14f @60fps) | Dead Cells보다 짧게, 쿨다운 2초로 보완  |

#### 상세 규칙

1. **i-frame 시작 타이밍:** 대시 입력 즉시 (0f 지연). 입력 지연이 없으므로 반응성이 보장된다.
2. **i-frame 지속 시간:**
   * 지상 대시: `dash_iframe_ms`(240ms / 14f @60fps)
   * 공중 대시: `dash_air_iframe_ms`(160ms / 10f @60fps) — 공중에서는 이동 자유도가 높으므로 i-frame을 더 짧게 설정하여 위험 보상 균형 유지
3. **쿨다운 중 i-frame:** 쿨다운(`dash_cooldown_ms` 2000ms) 동안 i-frame은 전혀 없다. 쿨다운이 완료된 시점의 대시 입력 즉시 i-frame이 발생한다.
4. **i-frame 종료 후 판정 복귀:** i-frame 종료 시점부터 즉시 피격 판정이 재개된다 (0f 유예 없음).
5. **i-frame 중 슈퍼 아머 공격:** i-frame은 피격 판정만 무시한다. 슈퍼 아머 파괴 판정은 별도이며, i-frame 중에도 슈퍼 아머 파괴 데미지를 적용하지 않는다.

#### 파라미터 추가

```yaml
dash:
  dash_iframe_ms: 240          # 지상 대시 i-frame 지속 (14f @60fps)
  dash_air_iframe_ms: 160      # 공중 대시 i-frame 지속 (10f @60fps)
  dash_cooldown_ms: 2000       # 쿨다운 (i-frame 없는 구간)
  dash_iframe_start_delay_f: 0 # i-frame 시작 지연 프레임 (즉시)
```

#### 설계 근거

Dead Cells 롤(400ms)보다 짧게 설정한 이유: Project Abyss는 대시가 단일 버튼 원칙 하에 쿨다운 2초로 제한된다. i-frame을 너무 길게 설정하면 쿨다운 주기마다 안정적인 대형 공격 회피가 보장되어 위협이 희석된다. 240ms는 "정확히 대시한 직후의 공격"은 피하지만 "대시 궤도 끝에서 날아오는 투사체"는 피격될 수 있는 수준이다.

#### 에르다 전용 대시 연출 (Erda Dash Animation)

에르다의 대시 애니메이션은 **단조 회피 구르기(Forge Roll)** 이다. 일반적인 횡이동 슬라이드가 아니라, 대장간에서 불꽃을 피해 몸을 말고 구르는 동작에서 착안한 모션이다. 기능은 다른 무기 사용자의 대시와 동일하지만, 에르다의 대장장이 신체 언어를 전투 중에도 일관되게 드러내는 코스메틱 차별화이다.

---

### 2.5-B. 벽 충돌 넉백 처리 (Wall Collision During Knockback)

넉백 중 엔티티가 타일맵 벽 충돌체에 접촉했을 때의 동작을 정의한다.

#### 채택 방식: 정지 방식 (SotN / Hollow Knight 레퍼런스)

| 처리 방식           | 설명                          | 장점                                | 단점                                |
| :------------------ | :---------------------------- | :---------------------------------- | :---------------------------------- |
| **정지 방식 (채택)** | vx = 0 즉시, vy 유지          | 구현 단순, 추가 물리 계산 없음      | 동적 느낌 부족                      |
| 바운스 방식         | vx 반전, 속도 감소            | 동적·물리적 느낌                    | 웹 환경 추가 물리 계산, 예측 어려움 |
| 슬라이드 방식       | vx = 0, vy 전환               | 공중 콤보 연계 가능                 | 의도치 않은 위치 이동 발생 가능     |

**정지 방식 채택 이유:** 웹 환경(`requestAnimationFrame` 기반)에서 바운스 물리의 프레임 독립성 보장이 복잡하다. 정지 방식은 구현이 단순하고 충돌 후 적의 행동 재개 타이밍이 예측 가능하여 플레이어에게 공정하게 느껴진다.

#### 상세 규칙

1. **벽 충돌 판정 조건:** 엔티티의 hitbox가 타일맵 충돌체(TilemapCollider)와 AABB 겹침이 발생할 때.
2. **충돌 시 속도 처리:**
   * `vx = 0` 즉시 (수평 넉백 종료)
   * `vy` 유지 (중력은 계속 적용)
3. **추가 피해:** 없음. 벽 충돌로 인한 추가 피해는 Phase 2에서 "벽 부수기" 특수 시너지 검토 (`3.5 스킬 시너지` 테이블의 "물리 넉백 → 벽 충돌" 조건 참조).
4. **스턴 여부:** 없음. 적은 벽 충돌 직후 즉시 행동을 재개할 수 있다.
5. **플레이어 벽 충돌:** 플레이어가 넉백 중 벽에 닿아도 동일한 정지 방식을 적용한다. 추가 피해 없음.
6. **천장 충돌 (수직 넉백):** 띄우기 공격으로 발생한 수직 넉백이 천장 타일과 충돌하면 `vy = 0` 즉시, 이후 중력으로 낙하.

#### 구현 현황

```text
- 넉백 규칙 2.5 "넉백 중 벽 충돌: 벽에 닿으면 넉백 즉시 정지" — 기존 규칙과 일치
- 실제 TilemapCollider 연동 로직: 📅 미구현 (CMB-01-DB)
```

---

### 2.5-C. 패링 시스템 프레임워크 (Parry System Framework)

패링은 적의 근접 공격 타이밍에 방어 입력을 맞춰 반격 기회를 얻는 고위험-고보상 메커닉이다.

#### 패링 3요소 원칙 (리서치 §3.2 기반)

| 요소           | 명세                                                                     | 레퍼런스                         |
| :------------- | :----------------------------------------------------------------------- | :------------------------------- |
| **텔레그래프** | 패링 가능 공격에 흰색 예비 동작 오버레이 표시 (50ms 선행)                | Hollow Knight 흰색 플래시        |
| **타이밍 창**  | `parry_window_ms`(200ms / 12f @60fps)                                    | HK보다 관대, Dead Cells보다 엄격  |
| **보상**       | 반격 데미지 ×1.5 + 무적 2초(`parry_invincibility_ms`) + MP 회복 10%      | Dead Cells 패링 보상 구조 참조    |

#### 패링 가능 공격 분류

| 공격 유형     | 패링 가능 여부 | 이유                                             |
| :------------ | :------------: | :----------------------------------------------- |
| 적 근접 공격  | 가능           | 패링의 핵심 대상. 타이밍 읽기와 반응이 보람 있음  |
| 투사체        | 불가           | 투사체 경로 예측은 별도 스킬 영역 (방패 스킬 등)  |
| 범위기 / AoE  | 불가           | 광역 공격은 회피(대시)가 올바른 대응              |
| 보스 특수기   | 불가 (기본값)  | 보스별 개별 설정으로 일부 허용 가능               |

#### 상세 규칙

1. **패링 입력 조건:** 가드 버튼(`System_3C_Control.md` 참조) 입력 시, 패링 가능 적 공격의 히트 판정 발생 `parry_window_ms`(200ms) 이내이면 패링 성공.
2. **패링 성공 처리:**
   * 적 공격의 피해를 0으로 무효화.
   * 플레이어에게 `parry_invincibility_ms`(2000ms) 무적 부여. 점멸 이펙트 적용.
   * 적에게 플레이어 ATK × `parry_counter_multiplier`(1.5)의 반격 데미지 적용.
   * 현재 최대 MP의 `parry_mp_restore_ratio`(10%)를 즉시 회복.
3. **패링 실패 처리:** 가드 입력이 `parry_window_ms` 외부이거나, 패링 불가 공격에 사용한 경우 — 일반 피격과 동일하게 처리한다. 추가 페널티 없음 (패링 시도 비용은 기회비용뿐).
4. **연속 패링 제한:** 동일 적의 연속 공격에 대해 `parry_cooldown_ms`(500ms) 내 재패링은 불가. 패링 남용으로 무한 무적 상태 방지.

#### 패링 연출

* `SYS-CMB-07`의 Heavy 피드백 등급 적용 (히트스탑 4f, 방향성 카메라 쉐이크).
* 패링 전용 스파크: `HitSparkManager.spawn(type: 'parry')` — 흰색+금색 방사형 스파크, 수명 250ms.
* 패링 성공음: 금속 공명음 (충격음 위에 고주파 공명 레이어 추가).

#### 3-Space별 적용

| 공간        | 패링 활성화 | 역할                                                         |
| :---------- | :---------: | :----------------------------------------------------------- |
| 월드        | 활성         | 솔로 탐험 중 생존 도구. 강적 일격 무효화로 리스크 감소       |
| 아이템계    | 활성         | 파티 탱커 역할 강화. 반격 데미지로 딜 기여 가능              |
| 허브        | 비활성       | 허브는 전투 없는 소셜 공간                                   |

#### 파라미터

```yaml
parry:
  parry_window_ms: 200              # 패링 타이밍 창 (12f @60fps)
  parry_invincibility_ms: 2000      # 패링 성공 후 무적 시간
  parry_counter_multiplier: 1.5     # 반격 데미지 배율
  parry_mp_restore_ratio: 0.10      # 패링 성공 시 MP 회복 비율
  parry_cooldown_ms: 500            # 연속 패링 방지 쿨다운
  parry_telegraph_lead_ms: 50       # 흰색 예비 동작 선행 표시 시간
```
...
### 2.6-A. 인챈트 타격 피드백 & 에르다 전투 독백 (Enchant Hit Feedback & Combat Monologue)

#### 인챈트 적용 무기 타격음 (Enchanted Weapon Strike SFX)

인챈트된 무기의 타격음은 기본 타격음 위에 원소별 레이어가 추가된다. 예: 화 인챈트 검은 금속 마찰음 + 불꽃 파열음. 무 속성(인챈트 미적용) 시에는 무기 고유 타격음만 재생된다.

인챈트 크리티컬 히트 발생 시:
- 무기에 부착된 원소 글로우 강도가 일시적으로 최대치로 증폭된다 (0.5초, 통상 글로우의 약 3배 밝기).
- 이 증폭 이펙트는 에코 인챈트 시전 시의 파티클 폭발과 동일한 색조로 재생된다. 전투 중 크리티컬이 터질 때마다 인챈트 행위가 시각적으로 연상되도록 의도한 연결이다.

#### 에코 공명 글로우 (Echo Resonance Glow) — 아이템계 전용

아이템계 내부에서 에코는 이노센트 및 기억 구조물과 공명한다. 이노센트 대상에게 타격을 가한 무기에서 **공명 글로우(Resonance Glow)** 이펙트가 0.3초간 표시된다. 이 글로우는 아이템계 진입 시 제단에 에코를 내리칠 때 나타나는 균열 이펙트와 동일한 색조(금빛-백색)이다. 에코가 전투 무기가 아닌 기억의 도구임을 아이템계 내부에서 시각적으로 상기시키는 장치이다.

#### 에르다 전투 독백 (Erda Combat Monologue)

에르다는 대장장이의 시선으로 적을 분석하고 독백한다. 이 독백은 Act 1(코미디 톤)에서 적 처치 시 **랜덤 확률**로 재생된다. 같은 독백이 연속으로 재생되지 않도록 최근 N회 이력을 제외한 풀에서 선택한다.

**Act 1 독백 예시 (처치 시 트리거):**

| 상황          | 대사                                                  |
| :------------ | :---------------------------------------------------- |
| 일반 처치     | "이 관절부 결합이 이상한데... 아, 죽었네."           |
| 일반 처치     | "충격 흡수 구조가 없어. 한 방이지."                  |
| 일반 처치     | "소재가 나쁘진 않은데, 마감이 엉망이야."             |

독백 발동 조건 및 운영 규칙은 추후 `System_NarrativeVoice.md`에서 상세 정의한다 (발동 확률, 쿨다운, Act별 풀 분리 등).
...
5. 아이템계에서 파티 전멸 시 현재 지층 처음부터 재시작한다.
...
* [ ] 지상 대시 i-frame이 240ms(14f), 공중 대시가 160ms(10f)로 차별화되는가
* [ ] 쿨다운 중(2초) 대시에 i-frame이 없는가
* [ ] i-frame이 입력 즉시(0f 지연) 시작되는가
* [ ] 대시 쿨다운이 2초이며, 쿨다운 중 대시 입력이 무시되는가
* [ ] 공중 대시가 착지 전 1회만 가능한가
* [ ] 넉백 중 벽 충돌 시 vx = 0 즉시, vy는 유지되는가 (정지 방식)
* [ ] 벽 충돌 시 추가 피해가 발생하지 않는가
* [ ] 천장 충돌 시 수직 속도(vy)가 즉시 0이 되는가
* [ ] 패링 타이밍 창(200ms) 내 가드 입력 시 패링이 성공하는가
* [ ] 패링 가능 공격(근접 공격)에 흰색 예비 동작 오버레이(50ms 선행)가 표시되는가
* [ ] 패링 성공 시 반격 데미지(ATK × 1.5), 무적(2초), MP 회복(10%)이 적용되는가
* [ ] 투사체 및 범위기 공격에 패링이 적용되지 않는가
* [ ] 패링 실패 시 추가 페널티 없이 일반 피격과 동일하게 처리되는가
* [ ] 연속 패링 쿨다운(500ms)이 정상 적용되는가
* [ ] 허브에서 패링이 비활성화되는가
...
* [ ] 에코가 무기 슬롯을 차지하지 않으며 장착/해제 UI에 표시되지 않는가
* [ ] 아이템계 진입 중 에코의 이노센트 대상 피해 +10% 패시브가 적용되는가
* [ ] ↑+공격(또는 전용 인챈트 버튼) 입력 시 1초 인챈트 모션이 재생되는가
* [ ] 인챈트 모션 중 피격이 가능한가 (무적 프레임 없음)
* [ ] 인챈트 시 원소가 해금된 원소 내에서만 순환되는가 (미해금 원소 건너뜀)
* [ ] 원소 순환 순서가 화→빙→뇌→풍→광→무→화 순서인가
* [ ] 인챈트 상태가 변경 전까지 유지되는가 (타이머 없음)
* [ ] 인챈트 적용 시 무기에 해당 원소 색상 글로우와 파티클이 표시되는가
* [ ] 인챈트 미적용(무 속성) 상태에서 물리 데미지만 발생하는가
* [ ] 인챈트 크리티컬 히트 시 원소 글로우 강도가 최대치로 증폭(0.5초)되는가
* [ ] 아이템계 내부에서 이노센트 피격 시 공명 글로우(Resonance Glow)가 0.3초간 표시되는가
* [ ] Act 1에서 에르다 처치 독백이 랜덤으로 재생되되 직전 재생 이력이 제외되는가
