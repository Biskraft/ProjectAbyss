...
#### 에코 (Echo) — 주인공 전용 기본 무기

에코는 주인공 에르다 벤-나흐트(Erda ven-Nacht)의 서명 무기이자 이야기의 핵심 도구이다. 대장장이의 작업 해머에서 유래한 망치형 무기로, 무게감 있는 타격과 광역 충격이 특징이다.

**에코의 자동 콤보 3타:**

| 타격 | 모션                 | 히트박스                | 특성                                    |
| :--- | :------------------- | :---------------------- | :-------------------------------------- |
| 1타  | 수평 스윙 (좌→우)    | 넓고 낮음 (43×22px)     | 주변 다수 적 동시 타격에 유리           |
| 2타  | 머리 위 내려치기     | 좁고 깊음 (22×36px)     | 수직 범위 우선, 단일 대상 집중 피해     |
| 3타  | 어퍼컷 타격          | 중간 (32×28px)          | 소형 적을 공중으로 띄움 (air-launch)    |

**에코 전용 타격감 규칙:**
- 에코의 히트스탑은 동등 위력의 검 계열보다 **1프레임 추가** 적용된다. 망치의 물리적 무게감을 직접적인 시간 정지로 표현한다. (예: 1~2타 = Light + 1f = 3f, 3타 = Medium + 1f = 4f)
- 에코 타격 시 피격 대상 위에 **공명 글로우(Resonance Glow)** 이펙트가 0.3초간 표시된다. 이 글로우는 아이템계 진입 시 제단에 에코를 내리칠 때 나타나는 균열 이펙트와 동일한 색조(금빛-백색)를 사용한다. 에코가 단순한 무기가 아님을 전투 중에도 지속적으로 상기시키는 시각적 장치이다.

**에코 장착 규칙:**
- 에코는 해제(unequip)가 불가능한 스토리 무기이다. 주무기 슬롯에 고정되어 있으며, 세컨더리 무기 슬롯에 별도 무기를 장착할 수 있다.
- 아이템계 진입 중 에코는 패시브 보너스를 획득한다: **이노센트(Innocent) 대상 피해 +10%**. 에코는 기억으로 이루어진 존재들과 공명하므로, 그들에게 더욱 강하게 반응한다.
...
### 2.6-A. 에코 전용 타격 피드백 & 에르다 전투 독백 (Echo Hit Feedback & Combat Monologue)

#### 에코 타격음 (Echo Strike SFX)

에코 망치 타격음은 검 계열 무기의 날카로운 금속 마찰음과 명확히 구분된다. **묵직한 금속 울림(metallic ring)** — 망치가 모루를 내리칠 때의 공명 잔향을 기반으로 설계한다. 이는 에르다의 대장장이 정체성을 청각적으로 끊임없이 환기하는 요소이다.

크리티컬 히트 발생 시:
- 공명 글로우(Resonance Glow) 강도가 일시적으로 최대치로 증폭된다 (0.5초, 통상 타격의 약 3배 밝기).
- 이 공명 증폭 효과는 아이템계 진입 시 에코가 제단 무기를 타격할 때 나타나는 균열 개방 연출과 동일한 색조로 재생된다. 전투 중 크리티컬이 터질 때마다 아이템계 진입 장면이 무의식적으로 연상되도록 의도한 시각적 연결이다.
...
* [ ] 에코 3타 콤보가 1타(수평 스윙) → 2타(내려치기) → 3타(어퍼컷) 순서로 정상 재생되는가
* [ ] 에코 3타가 소형 적을 공중으로 띄우는가 (air-launch)
* [ ] 에코 타격 시 히트스탑이 동급 검 공격보다 1프레임 더 길게 적용되는가
* [ ] 에코 타격 시 공명 글로우(Resonance Glow) 이펙트가 0.3초간 표시되는가
* [ ] 크리티컬 히트 시 공명 글로우 강도가 최대치로 증폭(0.5초)되는가
* [ ] 에코가 주무기 슬롯에서 해제 불가 상태로 고정되는가
* [ ] 아이템계 진입 중 에코의 이노센트 대상 피해 +10% 패시브가 적용되는가
* [ ] Act 1에서 에르다 처치 독백이 랜덤으로 재생되되 직전 재생 이력이 제외되는가
* [ ] 에코 타격음이 검 계열 타격음과 명확히 구분되는가 (금속 울림 vs. 금속 마찰)
