...
| 공간 | 퀘스트 유형 | 설명 |
| :--- | :--- | :--- |
| **월드 (World)** | 메인/사이드 퀘스트 | 구역 탐험 중 마주치는 핸드크래프트 서사 |
...
| **양산 효율** | 아키타입 × 변수 풀 × 맥락 레이어 조합으로 100개 이상 퀘스트 생산 | 6 아키타입 × 7 위치 구역 × 5 보상 유형 = 210 최소 조합 |
...
```
월드 탐험 ──→ 아이템 획득 ──→ 기억의 지층 진입 ──→ 장비 강화 ──→ 스탯/능력 게이트 해금 ──→ 새 구역 탐험
    ↑              │                  │                    │                 │                    │
  [서사 훅]    [아이템 기원]      [기억 탐구]           [이노센트]        [퀘스트 개방]         [새 퀘스트]
월드 퀘스트가   아이템이 가진      지층 미니퀘스트로     이노센트 복종      해금된 구역의          연쇄 결과가
...
| :--- | :--- | :--- |
| **슬롯 1: 위치** | 공간 × 구역 | 월드 7개 구역 + 지층 6개 바이옴 + 허브 구역 |
...
레이어 1: 지역 테마 ────────────────── 월드 구역 테마 / 지층 바이옴 테마 / 허브 분위기
```
...
| 6 | 결과 | 즉각 결과 + 장기 파급 (세력 관계, 구역 상태, 후속 퀘스트 활성화) |

**변수 슬롯 (PA 전용):**

| 슬롯 | 설명 | PA 예시 값 |
| :--- | :--- | :--- |
| [갈등 유형] | 갈등의 근본 원인 | 이노센트 소유권 분쟁, 아이템 기원 은폐, 구역 지배 세력 교체 |
...
| [장기 결과] | 세계 상태 영구 변경 | 해당 구역 이노센트 풀 변화, 새 NPC 등장, 세력 지도 갱신 |
...
| 3 | 반전 트리거 | 예상치 못한 진실 노출. 대상 물건 조사, NPC 2차 대화, 특정 구역 도달 |
...
| 슬롯 | 설명 | PA 예시 값 |
| :--- | :--- | :--- |
| [사건 유형] | 조사할 사건의 종류 | 이노센트 대량 소멸, 구역 몬스터 이상 행동, 아이템 기원 은폐 |
...
| 2 | 각 입장 파악 | 양측의 내부 사정과 논리를 직접 체험. 구역 탐험 또는 지층 공략 중 확인 |
| 3 | 선택 요구 | 플레이어에게 편들기를 강제하는 이벤트 발생. 두 세력 모두 충분히 준비된 상태여야 함 |
| 4 | 세력 지원 | 선택한 세력을 위한 퀘스트 수행 |
| 5 | 결과 | 지역/구역 상태 영구 변화. 세계 지도 갱신, NPC 생존/사망 확정 |
...
| :--- | :--- | :--- |
| 탐험 깊이 (방문 구역 수) | 비선형 경로 퀘스트 증가, 숨겨진 분기 개방 | 직선적 안내 퀘스트 우선 |
...
| 공간 | 적용 방식 |
| :--- | :--- |
| 월드 | 두 능력 게이트 중 어느 쪽을 먼저 해금해도, 각각의 경로가 동등한 분량의 구역·보스·서사를 가진다 |
...
| 능력 게이트 우선순위 | 탐험가(새 구역 탐험 욕구) vs 장인(현재 장비 강화 욕구) |
...
| 적용 공간 | 고유성 콘텐츠 |
| :--- | :--- |
| 월드 탐험 | STR 경로: 물리 보스·전사형 이노센트·무기 풀 / INT 경로: 마법 보스·마법형 이노센트·주문 풀 |
...
| 스탯 게이트 투자 | STR / INT / DEX / VIT / SPD / LCK | 각 스탯이 다른 구역·콘텐츠 유형을 해금 | 어떤 탐험가가 될 것인가 |
| 능력 게이트 우선 | 이단 점프 / 벽 타기 / 안개 변신 / 수중 호흡 / 역중력 | 각 능력이 다른 월드 구역 접근 허용 | 캐릭터의 성장 서사 |
...
| 스탯 투자 방향 | 고급 (6갈래) | 6개 스탯 = 6개 탐험 방향 |
...
| **간접 영향** | 결과가 다른 퀘스트의 변수를 변경 | 세력 A 지원 → 해당 구역 NPC 구성 변화, 새 의뢰 유형 등장 |
...
| PA_LOC_16 | 허브 | 거래소 구역 | 음모, 정보 | 도덕, 세력 |
...
| :--- | :--- | :--- | :--- | :--- |
| 도덕적 딜레마 | 당사자 A 지원 | 당사자 B 지원 | 양측 화해 중재 | 제3자 개입 (STR/INT 조건) |
| 기대 전복 | 표면 목표 완수 (모름 상태) | 진실 이해 후 선택 | 진실을 숨기는 선택 | — |
| 탐정 절차 | 추론 기반 결정 | 증거 확보 후 결정 | 의뢰인 배신 선택 | 감추어진 4번째 진실 (LCK 조건) |
...
| 11 | 기억 탐구 | 기억의 지층 (Legendary) | 기원 아이템 전 주인 유령 | 왕국 봉인 검의 기원 완전 해명 | 검의 기원이 왕국 멸망과 직결되어 있으며, 이노센트들이 그 죄책감을 품고 있었다 | 월드 구역 봉인 해제 + 신규 구역 개방 |
