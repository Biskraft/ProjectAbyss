...
| ENM-02-A | 감지 | 원형 감지 반경 시스템 | P1 | 대기 | 반경 단독 감지. 적 유형별 반경 차이 (플레이어 스텔스 FoV/LoS 제거, 적 은신 A-07은 유지 2026-06-03) |
...
* 몬스터 아키타입 시스템 (분류 SSoT): `Documents/System/System_Enemy_MonsterArchetype.md`
* 일반 적 아키타입 설계 리서치 (출처): `Documents/Research/EnemyDesign_MobArchetype_Research.md`
...
ECHORIS의 적 AI 시스템은 다음 한 문장으로 정의한다:
...
| Detect | 감지 반경 내 플레이어 존재 | detect_confirm_ms 경과: Chase/Retreat 전환. 반경 이탈 복귀: Patrol 전환 | 이동 정지. 감지 확인 딜레이. 시각적 느낌표 이펙트 표시 |
...
#### 2.2-C. 앰부시(A-07) 은신 상태 (Ambush)

A-07 앰부시형은 표준 7상태 앞에 **HIDDEN(은신)** 상태를 추가로 갖는다. *적의 은신 위협*이며 플레이어 스텔스와 별개다(§2.3).

| 상태 | 진입 | 탈출 | 행동 |
| :--- | :--- | :--- | :--- |
| HIDDEN | 스폰 직후 | 플레이어가 기습 반경(`ambush_radius_tiles`, ~2.5) 진입 | 환경 위장. 화면 안에서 미세 단서 노출(질감 변화/눈 깜빡임). 피해 판정 없음, 피격은 가능 |
| REVEAL | 기습 반경 진입 | 출현 모션(4f) 완료 → Attack | 출현 애니메이션 = Tell. 화면 밖이면 출현 보류(공정성) |

출현 후에는 표준 Attack → Cooldown → (도주 또는 재은신: HIDDEN 재진입) 순환. 재은신은 플레이어가 기습 반경 밖으로 이탈했을 때만 가능하다. 화면 밖 출현·기습은 금지(§3.1).

### 2.3. 감지 시스템 (Detection System)

> **2026-06-03 정합:** *플레이어 측 스텔스*(적의 시야를 사각·엄폐로 회피) 제거에 따라 감지는 **원형 반경 단독**으로 단순화됐다. 시야각(FoV)·시선(LoS) 판정은 폐기. 이는 현재 코드 구현(거리 단독 감지)과도 일치한다. 플레이어는 사각·엄폐로 적 감지를 회피할 수 없다. 단 **적의 은신 위협**(A-07 앰부시형)은 유지한다 — 은신은 별도 HIDDEN 상태(§2.2-C)로 기습 반경 진입 시 출현하며, FoV/LoS가 아닌 반경 기반이다. 적 은신과 플레이어 스텔스는 별개 개념이다.

#### 감지 조건 (반경 단독)

| 조건 | 설명 | 실패 시 |
| :--- | :--- | :--- |
| 감지 반경 (Detect Radius) | 적을 중심으로 한 원형 범위(`detect_radius_tiles`) 내에 플레이어가 존재하는지 판정 | 범위 외: 감지 불가 |

전방위 판정이다. 후방 접근으로 감지를 회피할 수 없다. 적이 도달할 수 없는 위치(높이/낙하 한계 초과)의 플레이어는 감지하더라도 Chase가 `lose_target_delay_ms` 후 Patrol로 복귀하므로, 벽 너머 무한 추적은 발생하지 않는다(§2.2-A 도달 가능성 규칙).

#### 감지 확인 딜레이

감지 반경 진입 즉시 Chase로 전환하지 않는다. `detect_confirm_ms` 동안 플레이어가 반경 내에 유지되어야 Detect 확정이 된다. 이 딜레이 동안 플레이어가 반경을 이탈하면 감지가 취소된다.
...
> 아키타입 분류의 **SSoT는 `Documents/System/System_Enemy_MonsterArchetype.md`** 다. 2026-06-03 정합으로 11종 단일 체계로 통합됐다(A-07 앰부시형 적 은신은 유지, 플레이어 스텔스 FoV/LoS만 제거). 본 문서는 AI 상태 머신·감지·스케일링만 정의하며, 아키타입 정의·조우 매트릭스·테마 매핑·행동 모디파이어(MOD)는 위 SSoT를 따른다. 아래는 AI 구현용 요약이다.

| ID | 아키타입명 | 기존 적 매핑 | 위협 축 | 이동/공격 패턴 | MovementType | 도입 지층 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **A-01** | Charger (근접 돌격형) | Skeleton | 거리 압박 | 직선 추적 → 근접 1타 | ground | 지층 1 |
| **A-02** | Jumper (점프형) | Slime | 불규칙 궤도 | 포물선 점프 → 착지 범위 피해 | ground | 지층 1 |
| **A-03a** | Shooter (직선 사격/기동) | Ghost | 기동 사격(카이팅) | 이동-정지 반복 → 투사체 1-3발 | flying/ground | 지층 1 후반 |
| **A-03b** | Bombardier (범위 포격) | (신규) | 지역 거부 | 후방 유지 → 포물선 착탄 마커 폭발 | ground | 지층 2 |
| **A-04** | Shielder (방어/방패) | (신규) | 정면 무효화 | 느린 전진 → 방패 강타. 배후 취약 | ground | 지층 2 후반 |
| **A-05** | Flier (비행/공중) | Spark Bat (계획) | 수직 위협 | 자유 비행 → 하강 급습 → 재상승 | flying | 지층 1 후반 |
| **A-06** | Swarmer (군집) | Cinder Imp (계획) | 수량 압도 | 무리 이동 → 약근접 빠른 반복 | ground | 지층 2 초반 |
| **A-07** | Ambusher (앰부시) | (신규) | 기습/은신 | HIDDEN 은신·위장 → 기습 반경 진입 시 출현 1타 (§2.2-C) | ground/wall | 지층 2 |
| **A-08** | Summoner (지원/소환) | (신규) | 전장 증폭 | 후방 유지 → 아군 강화/Swarmer 소환 | ground | 지층 3 |
| **A-09** | Elite (엘리트) | GoldenMonster | 다층 위협 | 2-3 아키타입 패턴 조합 | 상속 | 모든 지층 보상방 |
| **A-10** | Sentinel (고정 포대) | (신규) | 고정 고화력 | 고정 위치 → 고데미지 직선 투사체 | static | 지층 2 후반 |

> **구 9종 → 11종 변경 요약:** 구 A-03(원거리)을 A-03a(기동 Shooter)/A-10(고정 Sentinel)으로 분리, A-03b(Bombardier) 추가. A-07 앰부시형(적 은신)은 독립 아키타입으로 유지. 조우 구성 매트릭스·테마-적 연결·MOD 8종은 SSoT(`System_Enemy_MonsterArchetype.md` §3·§5)에 있다. 본 문서에서는 중복 정의하지 않는다.
...
- 감지는 **원형 반경 단독**으로 성립한다. 시야각(FoV)·시선(LoS) 판정은 폐기됐다(2026-06-03). 적은 사각·엄폐로 회피할 수 없다.
- 감지 확인 딜레이(`detect_confirm_ms`) 도중 플레이어가 감지 반경을 이탈하면 감지가 즉시 취소되고 Patrol 상태로 복귀한다.
- 이미 Chase 또는 Retreat 상태인 적이 플레이어를 놓쳤을 경우(감지 반경 이탈 또는 §2.2-A 도달 불가): `lose_target_delay_ms` 경과 후 Patrol 상태로 복귀한다.
...
enemy_skeleton:
  ...
  # 감지 파라미터
  detection:
    detect_radius_tiles: 6        # 감지 반경 (타일 단위) — 반경 단독 감지
...
enemy_ghost:
  ...
  # 감지 파라미터
  detection:
    detect_radius_tiles: 8        # 감지 반경 (타일 단위) — 반경 단독 감지
...
enemy_spark_bat:
  ...
  # 감지 파라미터
  detection:
    detect_radius_tiles: 7      # 반경 단독 감지 (전방위)
...
enemy_cinder_imp:
  ...
  # 감지 파라미터
  detection:
    detect_radius_tiles: 5      # 반경 단독 감지 (전방위)
...
| 상황 | 처리 방식 |
| :--- | :--- |
| 플레이어가 감지 반경 경계에 걸친 경우 | 중심 좌표 기준 반경 내 판정. 경계 진입 시 detect_confirm_ms 타이머 시작 |
| 감지 확인 딜레이 도중 플레이어가 감지 반경을 이탈한 경우 | 감지 즉시 취소. 딜레이 타이머 리셋. Patrol 복귀 |
| 이미 Chase 상태인 적이 §2.2-A 도달 불가(높이/낙하 한계 초과) 판정을 받은 경우 | lose_target_delay_ms 동안 마지막 위치로 이동 시도. 딜레이 경과 후 Patrol 복귀 |
...
| 반경 단독 감지 | 감지 반경 내/외 위치에서 각각 접근 테스트 | 반경 내 진입 시에만 감지 성립. 후방·엄폐 접근으로 회피 불가 확인 |
