...
| ENM-02-A | 감지 | 원형 감지 반경 시스템 | P1 | 대기 | 반경 단독 감지. 적 유형별 반경 차이 (스텔스·엄폐 메커닉 제거 2026-06-03) |
...
> **2026-06-03 정합:** 스텔스·엄폐 메커닉 제거에 따라 감지는 **원형 반경 단독**으로 단순화됐다. 시야각(FoV)·시선(LoS) 판정은 폐기. 이는 현재 코드 구현(거리 단독 감지)과도 일치한다. 적은 사각·엄폐로 회피할 수 없으며, 플레이어 측 은신 플레이는 설계에서 제외한다.
...
> 아키타입 분류의 **SSoT는 `Documents/System/System_Enemy_MonsterArchetype.md`** 다. 2026-06-03 정합으로 10종 단일 체계로 통합됐다(구 9종에서 A-07 앰부시형을 A-01 Charger에 흡수, 스텔스 제거). 본 문서는 AI 상태 머신·감지·스케일링만 정의하며, 아키타입 정의·조우 매트릭스·테마 매핑·행동 모디파이어(MOD)는 위 SSoT를 따른다. 아래는 AI 구현용 요약이다.

| ID | 아키타입명 | 기존 적 매핑 | 위협 축 | 이동/공격 패턴 | MovementType | 도입 지층 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **A-01** | Charger (근접 돌격형) | Skeleton | 거리 압박 (+급습 변형) | 직선 추적 → 근접 1타. 급습은 가속 진입 강타(은신 없음) | ground | 지층 1 |
...
> **구 9종 → 10종 변경 요약:** 구 A-03(원거리)을 A-03a(기동 Shooter)/A-10(고정 Sentinel)으로 분리, A-03b(Bombardier) 추가. 구 A-07 앰부시형(스텔스)은 폐기하고 A-01의 급습 변형으로 흡수. 조우 구성 매트릭스·테마-적 연결·MOD 8종은 SSoT(`System_Enemy_MonsterArchetype.md` §3·§5)에 있다. 본 문서에서는 중복 정의하지 않는다.
