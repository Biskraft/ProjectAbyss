...
| HEL-07 | 이노센트 | 회복형 이노센트 패시브 | P2 | ⬜ 제작 필요 | 스파이크 직결 |
...
| 결정 | 근거 | 레퍼런스 |
|:---|:---|:---|
| Flask + 소모품 + 이노센트 (전투 연동 삭제) | 흡혈 메커닉은 대장간/단조 톤과 부정합. Flask가 유일한 능동 회복 → 한 방울이 무거움 | Dark Souls Estus + Dead Cells Flask |
...
| **3차: 이노센트 패시브** | 회복형 이노센트가 조건부 자동 회복 | 아이템계 (파밍 보상) | P2 |
...
> **난이도 스케일링:** Flask 감소가 Dead Cells의 Boss Cell과 동일한 역할. 높은 레어리티는 전투 연동 회복과 이노센트 의존도가 올라감.
...
이노센트 슬롯에 회복형 이노센트를 장착하면 조건부 자동 회복.
...
회복형 이노센트는 아이템계에서만 획득 가능. 더 좋은 회복 이노센트 = 더 깊은 지층 도전 가능 = 아이템계 재진입 동기. "아이템 속에 들어가야 하는 이유"가 하나 더 생긴다.
...
| 전투 중 | Flask 사용 |
| 적 처치 | 소모품 드랍 (확률), 이노센트 Reaper (P2) |
| 방 클리어 | 이노센트 Mender (P2) |
...
| 7 | 이노센트 패시브 회복 | ⬜ P2 | Phase 2 이노센트 시스템 동시 |
...
- `System_Innocent_Core.md` - 이노센트 시스템 (회복형 이노센트 Phase 2)
