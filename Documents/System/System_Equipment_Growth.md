...
| EGP-03-B  | 승급     | 승급 시 슬롯/지층/배율 갱신                 |    P2    | 대기         | 기억 단편·레벨 보존 필수                                   |
| EGP-04-A  | 기억 단편 | 기억 단편 슬롯 추가 (보스 처치 보상)         |    P2    | 대기         | Phase 2. 아이템 왕 처치 조건                              |
...
* 기억 단편 시스템: `Documents/System/System_Memory Shard_Core.md` (Phase 2)
...
ECHORIS의 장비 성장 경로 시스템은 다음 한 문장으로 정의한다:

> "장비는 드랍된 순간이 아니라, 플레이어가 직접 아이템계를 정복하며 성장시킨 순간에 진정한 가치를 갖는다"

일반 RPG에서 장비는 드랍 즉시 완성품이다. ECHORIS에서는 다르다. 드랍된 Rare 검은 잠재력의 씨앗이며, 플레이어가 그 내부의 지층을 하나씩 정복할수록 레벨이 오르고, 보스를 처치할수록 스탯이 영구적으로 강화되며, 모든 지층을 클리어하는 순간 다음 레어리티로의 승급 기회가 열린다. "내가 직접 키운 장비"라는 감정적 애착이 야리코미의 핵심 판타지다.
...
| 승급 시 기억 단편·레벨 보존 | 야리코미 노력의 가치를 보존해야 한다. 승급을 위해 쌓은 모든 것이 증발하면 승급 자체가 패널티 | 승급 시 리셋 — 파밍 무효화, 이탈 유발 |
| 기억 단편 슬롯 추가 = 보스 처치 보상 연계 | 기억 단편 슬롯 확장이 아이템계 진입의 독립적 목표가 된다. "이 검의 왕을 잡으면 슬롯 +1" | 슬롯 고정 — 야리코미 목표 단조화 |
...
| Normal 장비 가치 vs 고레어리티 파밍 | Normal이 쓸모없으면 초반 경험 공허 | Normal로 족하면 고레어리티 추구 동기 소멸 | Normal 기억 단편를 Rare에 이식 가능(Phase 2). Normal 아이템계 경험이 고레어리티 진입 전 학습 단계 |

### 1.5. 위험과 보상 (Risk & Reward)

| 전략 | 위험 (Risk) | 보상 (Reward) |
| :--- | :--- | :--- |
| Normal 장비 레벨 Max (10) 달성 | 레벨 상한이 낮아 스탯 성장 한계 명확 | 빠른 달성, 기억 단편 슬롯 초기 확보, 승급 발판 |
| Ancient 장비 레벨 99 파밍 | 4지층 + 심연, 최고 난이도, 장시간 파밍 | ×2.0 스탯 배율 + 보스 보너스 누적 = 최종 빌드 |
| 레어리티 승급 즉시 시도 | 재료 소모 + 실패 가능성 | 레어리티 배율 증가 + 지층 수 확장 + 기억 단편 슬롯 추가 |
...
FinalEquipStat =
  ...
  + memory shardBonus                       ← 기억 단편 레이어 (Phase 2)
```
...
> `memory shardBonus` 계산: `System_Memory Shard_Core.md` (Phase 2)
...
| 기억 단편 (복종 상태) | **보존** — 기존 기억 단편 유지, 추가 슬롯만 개방 |
| 기억 단편 슬롯 수 | 상위 레어리티 기준으로 증가 |
...
Phase 2에서 기억 단편 슬롯은 두 가지 경로로 확장된다.
...
> 레어리티별 기억 단편 슬롯 상한: `System_Equipment_Rarity.md` §2.1

슬롯 추가는 즉시 적용되며, 이미 복종된 기억 단편에게는 영향이 없다. 빈 슬롯은 야생 기억 단편 포획 또는 이식으로 채울 수 있다.
...
| 3단계: 기억의 완성 | Ancient 레벨 99 달성 | 칭호 "기억의 완성(Perfected Memory)" 부여, 특수 기억 단편 슬롯 +1 |
...
| EGP-R25 | 승급으로 증가한 기억 단편 슬롯은 야생 기억 단편 자동 배정이 아닌 빈 슬롯으로 개방된다 | 없음 |
...
- 레벨 99는 하드캡이다. 심연 진화 3단계 달성 후에는 더 이상 레벨이 오르지 않는다. 심연 페이즈 재진입으로 기억 단편 추가 파밍만 가능하다.
- "레벨 99 달성" 이후 목표 부재는 기억 단편 최적화·세트 효과 탐구(Phase 2)·신규 장비 파밍으로 유도한다.
...
| 장비 레어리티 (SYS-EQP-01) | `RarityMultiplier`, 레어리티별 기억 단편 슬롯 상한, 레어리티별 지층 수 | `System_Equipment_Rarity.md` |
| 장비 슬롯 (SYS-EQP-02) | `BaseStat`, 장비 착용/해제 상태 | `System_Equipment_Slots.md` |
| 기억 단편 (Phase 2) | `memory shardBonus` 합산값 | `System_Memory Shard_Core.md` |
...
| 수신 시스템 | 제공 데이터 | 용도 |
| :--- | :--- | :--- |
| 장비 슬롯 (SYS-EQP-02) | `FinalEquipStat` (레벨+보스보너스+기억 단편 합산) | 캐릭터 스탯 계산 |
...
consumes:
  ...
  - from: System_Equipment_Rarity
    data: ["rarityMultiplier", "maxLevel", "maxMemory ShardSlots"]
```
...
| 기억 단편 슬롯 추가 | 아이템 왕 처치 시 슬롯 +1, 최대 슬롯 상한 초과하지 않는 것 확인 |
