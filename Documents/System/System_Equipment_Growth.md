...
| EGP-05-A  | 전생     | 아이템 전생 (Reincarnation) 기반 구조 설계  |    P2    | 대기         | Phase 2. `EQP-06-A`와 연동                                |
...
- 장비가 거래소에 올라가거나 분해되면 피티 카운터는 초기화된다. 피티 카운터는 장비 소유자가 아닌 장비 자체에 귀속된다.
- 단, 거래를 통해 피티가 쌓인 장비를 매입하여 승급 성공을 노리는 전략은 의도된 경제 행동이다.
...
| 스탯 게이트 | 성장한 EquipStat이 게이트 판정에 사용됨 | `Documents/Design/Design_Architecture_3Space.md` |
