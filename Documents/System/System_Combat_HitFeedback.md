...
| CMB-07-N   | 무기 카테고리  | 무기 종류별 피드백 차이 (8종)                     |    P2    | 📅 대기         | 현재 검(Sword) 고정                    |
...
| CMB-07-V   | SFX 차별화    | 무기별 SFX 프로파일 (8종)                             |    P2    | 📅 대기         | 현재 검(Sword) 고정                     |
...
| **모바일 햅틱 부재** | PC는 컨트롤러 럼블 가능, 모바일 웹은 제한적 | 화면 진동 + 화면 플래시를 시각적 햅틱 대체로 설계 | 일부 모바일 브라우저에서 여전히 피드백 강도가 약함 |
...
### 2.5. 모바일 대응 (Mobile Adaptation)

| 항목 | PC 기준 | 모바일 조정 | 이유 |
| :--- | :--- | :--- | :--- |
| **햅틱** | 컨트롤러 럼블 (향후) | Web Vibration API: 라이트 10ms, 헤비 20ms, 킬 30ms | 터치에서 물리 진동이 더 직접적 |
| **스파크 수** | 라이트 4 / 헤비 7 | 라이트 3 / 헤비 5 (25% 감소) | 저사양 드로우콜 절감 |
| **화면 플래시** | 표준 알파 | ×1.2 (20% 강화) | 모바일 밝기 가변성 보상 |
| **카메라 흔들림** | 표준 강도 | ×0.7 (30% 감소) | 작은 화면에서 동일 px = 상대적으로 큰 흔들림 → 멀미 방지 |
| **엔티티 진동** | 표준 px | ×1.3 (30% 확대) | 작은 화면에서 미세 진동 가독성 확보 |
| **히트스탑** | 표준 | 변경 없음 | 프레임 기반이라 화면 크기 무관 |
...
현재 검(Sword) 단일 프로파일 고정. `System_Combat_Weapons.md` 8종 무기 확정 후 적용.
...
| 도끼 (Axe) | 1.3x | 1.2x | 빨강 `#FF4400` | 수직 하향 | 내리찍기 |
| 창 (Spear) | 0.9x | 0.8x | 흰색/연파랑 | 수평 찌르기 | 관통 느낌 |
| 지팡이 (Staff) | 0.8x | 0.5x | 보라 `#AA44FF` | 방사형 | 마법 충격 |
| 활 (Bow) | 0.5x | 0.6x | 흰색 | 미약한 수평 | 원거리, 약한 피드백 |
| 채찍 (Whip) | 0.7x | 0.9x | 흰색/노란색 | 좁고 빠른 수평 | 스냅 느낌 |
...
| 도끼 (Axe) | 강한 둔탁 + 파쇄음 | 없음 (둔기) | 무거운 반응 | 파괴적 타격 |
| 창 (Spear) | 중간 관통음 | 중간 찌르기 | 관통 반응 | 정밀한 느낌 |
| 지팡이 (Staff) | 마법 공명음 | 없음 | 마법 폭발 반응 | 원소 색상 연동 |
| 활 (Bow) | 화살 충돌음 | 없음 | 관통 반응 (약) | 원거리 피드백 |
| 채찍 (Whip) | 파열음 (스냅) | 없음 | 표면 반응 | 유연한 타격 |
...
30fps 기기에서도 최소 1프레임 히트스탑이 보장되어야 한다: `hitstopRemaining = Math.max(hitstopMS, deltaTime)`
