...
| CMB-07-N   | 무기 카테고리  | 무기 종류별 피드백 차이 (7종, DEC-026)                     |    P2    | 📅 대기         | 현재 검(Blade) 고정                    |
...
| CMB-07-V   | SFX 차별화    | 무기별 SFX 프로파일 (7종, DEC-026)                             |    P2    | 📅 대기         | 현재 검(Blade) 고정                     |
...
| 항목 | 월드 (World) | 아이템계 (Item World) | 세이브 포인트 |
| :--- | :--- | :--- | :--- |
...
현재 검(Blade) 단일 프로파일 고정. `System_Combat_Weapons.md` 7무기(DEC-026) 확정 후 적용. 아래 표는 구 5종 기준 — 하픈(Harpoon)·체인(Chain) 프로파일 추가 필요.

| 무기 | 히트스탑 계수 | 넉백 계수 | 스파크 색상 | 쉐이크 스타일 | 설계 의도 |
| :--- | :---: | :---: | :--- | :--- | :--- |
| 블레이드 (Blade) | 1.0x | 1.0x | 흰색/노란색 | 수평 지향 | 기준. 균형 타격감 |
| 클리버 (Cleaver) | 1.5x | 1.3x | 주황 `#FF8800` | 수직 하향 | 무거운 타격, 긴 정지 |
| 시브 (Shiv) | 0.6x | 0.7x | 흰색/밝은 파랑 | 가벼운 진동 | 빠른 다중 히트 |
| 레일보우 (Railbow) | 0.5x | 0.6x | 흰색 | 미약한 수평 | 원거리, 약한 피드백 |
| 에미터 (Emitter) | 0.8x | 0.5x | 보라 `#AA44FF` | 방사형 | 마법 충격 |

**무기별 SFX 프로파일 [미구현 — P2]:**

| 무기 | 임팩트 (L12) | 슬래시 (L13) | 피격 반응 (L14) | 설계 의도 |
| :--- | :--- | :--- | :--- | :--- |
| 블레이드 (Blade) | 중간 금속 충돌음 | 높은 슬래시 | 표준 피격 반응 | 기준 사운드 프로파일 |
| 클리버 (Cleaver) | 강한 저주파 둔탁음 | 낮은 슬래시 + 바람소리 | 무거운 반응 + 여운 | 무게감 극대화 |
| 시브 (Shiv) | 약한 금속음 | 높고 빠른 슬래시 | 빠른 반응 | 속도감 강조 |
| 레일보우 (Railbow) | 화살 충돌음 | 없음 | 관통 반응 (약) | 원거리 피드백 |
| 에미터 (Emitter) | 마법 공명음 | 없음 | 마법 폭발 반응 | 원소 색상 연동 |
