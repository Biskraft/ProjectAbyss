...
월드 환경 베드는 자연/빌더/현 문명 *삼중 레이어 메타포*를 청각으로 운반한다. 단 비중은 *Tier 깊이 × 공간 유형* 2축으로 결정된다 (자체 정밀화 2026-05-05).

#### 공간 유형 분류 (영구 룰 #8, 2026-05-05)

| 공간 유형 | 자연 비중 | 자연 자산 ON/OFF | 예시 |
|:---|:---:|:---:|:---|
| **Shaft** (광활한 수직 공동) | 0–5% | OFF (빌더 단독) | The Shaft 핵심부, Tier 3 메인 통로 |
| **빌더 폐허** (구조물 내부) | 5–10% | 약하게 ON | Tier 3–4 빌더 인공 영역 |
| **균열 영역** (벽 노출, 자연 침입) | 15–25% | ON | Tier 5–6 노후 영역 |
| **자연 동굴** (메가스트럭처 너머) | 40–60% | 우세 | Tier 6–7 깊은 자연 |

*세계 전체 흐름*에서 자연 비중이 0 이 되어선 안 되지만 *특정 영역 0% 정합*. 메이드 인 어비스의 *깊이 따라 자연 비중 점진 증가* 모티프 정합 (Tier 2 → 7 으로 갈수록 자연 우세).

#### Tier별 비중 (공간 유형 평균)

| 레이어 | 청각 신호 | Tier 2 비중 | Tier 3–4 | Tier 5 | Tier 6 | Tier 7 |
|:---|:---|:---:|:---:|:---:|:---:|:---:|
| **자연** | 낙수(천장에서 떨어지는 결로/광물수), 광물 결정 흐름, 광물 침출수, 깊은 동굴 잔향, 지질 활동 | 5% | **10%** | 15% | 25% | 60% |
| **빌더 구조물** | 메가스트럭처 금속 진동, 통풍구 바람, 케이블 긴장음, 빌더 발걸음 (멀리) | 60% | **80%** | 75% | 65% | 35% |
| **현 문명 잔재** | *떠난 그림자 거주자의 활동 메아리* — 멀리 도시 공사 망치질(쿵·깡), 금속 부딪힘, 거대 기계 활동, 잔류 인간형 신호, 세이브 포인트 화덕, 깃발 펄럭. *인간 발성 0* | 35% | 10% | 10% | 10% | 5% |

> 자연 레이어 *최소 비중 = 5%* (Tier 2 에서도 0 금지). BLAME!/메이드 인 어비스 원작도 *완전 자연 부재 X* — 비·바람·생체 잔재가 메가스트럭처에 압도된 모습이 정합. 자연 추가 = niche 신호 강화 (희석 아님).
>
> 깊이에 따라 *현 문명 → 빌더 → 자연* 순으로 청각 비중이 이동한다. Tier 7 에서는 자연 단일 레이어 + BGM 부재.

> **자연 잔재 — 메가스트럭처 내부 신호 한정 (영구 룰 #6, 2026-05-05):** 자연 키워드는 *메가스트럭처 내부에 시각적으로 존재 가능한 신호*만 채택한다. 외부 생명체(새·곤충·풀·짐승)·비(시각 부재)·외풍 *영구 금지*. 정합 신호: 광물 결정 흐름, 광물 침출수, 깊은 동굴 잔향, 멀리 광물 부서지는 지질 활동. 사유: BLAME! 원작 톤 정확 + 시각/청각 루도나러티브 정합 + 생명체 신호가 1차 niche 톤(공허한 메가스트럭처)을 파괴.

> **디제틱/비디제틱 분리 — 영구 룰 #7 (2026-05-05):** 환경 베드(비디제틱 공간 톤)와 디제틱 SFX(시각적 출처와 동기)는 영구 분리. 낙수·물방울·창고 삐걱임 등 *시각 출처가 있는 소리*는 환경 베드에서 빼고, AmbientLayer Layer C 근접 트리거가 시각 입자/스프라이트와 동기 재생한다. 환경 베드에 시각 출처 X 인 소리가 들어가면 *루도나러티브 충돌* (시각 부재 + 청각 존재). Plan_Audio_Demo §3-1 #1D 디제틱 SFX 카테고리 SSoT.

> **현 문명 잔재 — 인간 활동 도구 신호 한정 (영구 룰 #9, 2026-05-05):** 현 문명 잔재 레이어는 *떠난 그림자 거주자의 활동 청각 메아리*다. 도구 신호(망치·금속 공사·기계 활동·깃발·화덕)만 채택. 인간 발성·외침·이름 부르기·합창 *영구 금지* — 검 Ego 단독 화자 원칙(DEC-033) + 큐레이션 C1 정합. 게이트: "no human voices, no shouting". 레퍼런스 톤: 신세기 에반게리온 아야나미 레이 아파트 주변 도시 공사 소리(쿵·쿵·깡·깡). 1차 niche 톤 정합 강 (BLAME!/메이드 인 어비스/Transistor 팬과 교집합 큼).
...
Layer A: 환경 기저 (Environmental Bed)
  — 항상 재생. 공간 특성을 정의. 루프
  — *다중 자산 합성 가능*: 월드 환경 베드는 빌더 / 자연 / 현 문명 잔재 자산을
    동시 재생 + 볼륨 비율 합성 (§2-5 §B Tier별 비중 표 = 코드 볼륨으로 직접 매핑)
    사유: ElevenLabs SE 가 단일 프롬프트로 삼중 레이어 비중 강제 불가
    (자체 검증 2026-05-05 — 자연 키워드 단일 프롬프트 무시 패턴 발견)
  — *순차 변주 루프 패턴 (영구 룰 #10, 2026-05-05)*: 한 레이어 내부에서 같은
    프롬프트의 3변주를 *순차 재생 + 1.5초 크로스페이드*로 90s 사이클 운영.
    단일 30s 루프의 *반복 인지* 한계 회피. ElevenLabs SE 프롬프트 응답성
    변주를 자산 다양성으로 흡수. 1차 적용: #1C 현 문명 잔재. 데모 검증 후
    다른 환경 베드 (#1A 빌더, #1B 자연) 에도 적용 검토
...
### 12-4. Event Catalog — `Sheets/Content_System_Audio_Events.csv` (SSoT)

오디오 이벤트의 **단일 진실 원본**. 본 시스템 문서는 *디렉션* — 어떤 이벤트가 존재하고 어디서 트리거되는지는 CSV가 답한다. 새 사운드를 코드에 추가하기 전 항상 CSV 를 먼저 읽고 갱신한다.

**컬럼:**

| 컬럼 | 의미 |
|:---|:---|
| `event_id` | 이벤트 고유 키. `AudioBus.add(id, ...)` / `AudioBus.play(id, ...)` 의 인자와 1:1 일치. `SFX.play()` 의 큐 이름도 동일 |
| `channel` | `bgm` / `ambient` / `sfx` / `voice` (AudioBus 채널 라우팅) |
| `category` | 분류 — `movement` / `combat` / `item_world` / `world_ambient` / `ego` / `spike` / `memory_core` / `ui_reward` / `bgm` |
...
| 카테고리 | live | synth | pending | 비고 |
|:---|:---:|:---:|:---:|:---|
| `world_ambient` | 4 | 0 | 4 | Tier 3 builder + civ x3 가동, 자연/물방울/지질 음 미생성 |
| `ui_reward` | 0 | 2 | 0 | upgrade / milestone100 — DEC-040 Phase 2 자산 교체 예정 |
| `item_world` (capture) | 0 | 1 | 0 | capture — 동일 |
| `item_world` (Plaza/Memorial/Sanctum) | 0 | 0 | 7 | 베드 4 + 거리 이벤트 3 |
| `ego` | 0 | 0 | 6 | 각성 / Speak-In / typetick x4 |
| `movement` | 0 | 0 | 6 | 발소리 3 재질 x 2 변주 |
| `combat` | 0 | 0 | 7 | whoosh / impact x3 / finisher / hitreact / crit / kill |
| `spike` | 0 | 0 | 6 | DEC-039 Trapdoor 시퀀스 |
| `memory_core` | 0 | 0 | 5 | DEC-036 5색 기질 펄스 |
| `bgm` | 0 | 0 | 5 | 탐험 / 전투 / Rust Lane / 보스 / 세이브 |
| **합계** | **4** | **3** | **46** | |

**워크플로 (신규/수정 시):**

1. **CSV 우선 갱신.** `Sheets/Content_System_Audio_Events.csv` 에 행 추가 또는 `status` / `trigger_location` / `mix_volume` 갱신.
2. 코드 작성 — `AudioBus.add(event_id, ...)` 에 동일 `event_id` 사용.
3. 호출지 추가 — `AudioBus.play(event_id, channel)` 또는 `SFX.play(name)`. **`options.volume` 미지정 시 CSV 의 `mix_volume` 이 자동 적용** (`game/src/data/audioEvents.ts` 가 부팅 시 CSV 를 로드).
4. 자산 도착 후 — `status` 를 `pending` → `live` 로 갱신, `trigger_location` 의 `(TODO)` 마커 제거.

**믹싱 워크플로 (Victor):**

CSV 가 게임 내부 믹서 — 별도 UI 도구를 만들지 않는다. 사유: 50+ 이벤트 슬라이더 UI 비용 ↑, VS Code 가 더 좋은 편집기 (정렬·필터·검색·diff·git history 무료), 카테고리 일괄 변경은 Find & Replace 1번.

1. 게임 실행 → 일정 구간 플레이 → "anvil 이 발소리 묻음" 인지.
2. CSV 에서 해당 행의 `mix_volume` 수정 (예: `1.0` → `0.7`).
3. 저장 → Vite HMR 리로드 (또는 F5).
4. 같은 구간 재플레이 → 비교.
5. 만족 시 커밋. **CSV 가 SSoT — 코드 호출지에 `volume:` 직접 지정 금지** (이중 SSoT 회피, `AudioBus.play` 의 fallback 우선).

**중복 차단 룰:**

- 새 SFX 트리거 추가 전 CSV 의 `category` + `trigger_location` 으로 유사 이벤트 검색. 동일 의미 이벤트가 이미 있으면 **재사용** (다중 호출지 = `;` 추가).
- 변주(variation) 는 `_01`, `_02` … 접미사로 같은 카테고리/문맥 안에 묶는다.

**dev 가드 (구현 예정):**

- `AudioBus.play()` 가 CSV 미등록 `event_id` 로 호출되면 dev 모드에서 콘솔 1회 경고. orphan 이벤트 가시화.
- `Sheets/tools/validate.mjs` 에 `event_id` ↔ 코드 grep 매칭 검증 룰 추가 (TODO). 빌드 타임 정합성 보장.
