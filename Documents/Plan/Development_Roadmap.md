...
| 0-3 | **레어리티 GDD** | `System/System_Equipment_Rarity.md` | ✅ | 5등급 배율(x1.0~x3.0), 이노센트 슬롯, 드랍 연출 |
...
### 목표
> **1개 구역 + 3층짜리 미니 아이템계**에서 "탐험 → 아이템 획득 → 아이템계 진입 → 장비 강화" 순환이 돌아가는가?
...
| M1.5-4 | 층 생성 (3층) | IW Floor Gen의 미니 버전 (3x3 Grid) | 3층 + 보스 1층 생성 |
| M1.5-5 | 층 클리어 보상 | 아이템 경험치 + 아이템 레벨업 | 층 클리어 시 장비 스탯 상승 확인 |
...
### 목표
> 7개 구역 연결 + 능력/스탯 게이트 + 아이템계 30층 + 이노센트 기초
...
| M2.5 아이템계 30층 | 10층 단위 보스, 레어리티별 층수 제한 |
...
| M3.4 아이템계 100층 | 전체 층수 + 4등급 보스 + 미스터리 룸 |
