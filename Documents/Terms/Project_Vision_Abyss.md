...
| 직업 | 독립 대장장이 (중앙 성채 외곽) |
| 나이 | 19세 여성 |
| 에코 (도구) | 망치 "에코(Echo)" — **전투 무기가 아닌 대장장이 도구.** 무기에 원소를 인챈트하고, 모루/제단에서 아이템계에 진입하는 열쇠. 기본 화(Fire), 보스 처치 시 원소 흡수 |
| 캐릭터 레퍼런스 | 윈리 록벨(강철의 연금술사) — 직업 집착, 감정적 강인함, 실용주의 |
| 스토리 톤 | 코미디 → 진지 (디스가이아 1 오마주: 웃기는 이유와 슬픈 이유가 같다) |
| 감정 핵심 | "고치는 것으로 사랑하는 사람이, 고쳐지지 않는 것을 받아들이고 나아가는 이야기" |
...
| 설계 원칙   | 장비 내부 던전, 이노센트 시스템, 재귀적 진입, 무한 성장 루프                                                  |
...
| 설계 원칙   | 혼자서도 재미있고 함께하면 더 재미있는 설계, 역할 분담, Hub 사교                          |
| 검증 질문   | "이 시스템이 혼자서도 재미있고 함께하면 더 재미있는가?"                                   |
| 3-Space     | 전체 (특히 Item World + Hub)                                                              |
...
## 3. 3-Space 분리 모델 (Three-Space Architecture)

게임 세계를 3개 공간으로 분리하여, 각 공간이 고유한 규칙과 목적을 갖도록 설계한다. 이 분리가 "메트로베니아의 탐험감"과 "온라인 멀티플레이"가 양립하는 핵심 해법이다.
...
| 진행 방식   | 능력 게이트 (메트로베니아)      | 지층 클리어 (레어리티별 2~4 지층) | 자유 이동        |
| 사망 페널티 | 세이브 포인트 복귀              | 진행 지층 손실                    | 없음             |
| 고유 자원   | 능력 렐릭, 맵 데이터, 월드 소재 | 이노센트, 아이템 EXP, 레벨 구슬 | 상점, 합성, 창고 |
| 기둥 정렬   | 탐험                            | 야리코미                        | 멀티플레이       |
...
| 문제                                                 | 해법                                                                |
| :--------------------------------------------------- | :------------------------------------------------------------------ |
| 메트로베니아의 탐험감과 온라인 멀티가 양립 가능한가? | 월드는 개인 탐험(솔로~2인), 아이템계는 협동(1~4인), 허브는 소셜    |
...
| 항목        | 방향                                                                   |
| :---------- | :--------------------------------------------------------------------- |
| 아트 스타일 | 고딕, 다크 판타지, 스프라이트 기반 하이디테일 2D                       |
| 색조        | 어두운 배경 + 선명한 캐릭터/이펙트 (실루엣 가독성 우선)                |
| 월드        | 월하의 야상곡의 고딕 성 — 연금술 연구소, 빙결 동굴, 카타콤, 천공의 탑 |
| 아이템계    | 아이템 속 추상적 공간 — 레어리티별 배경 테마 변화                     |
| 허브        | 안전하고 따뜻한 마을 분위기 — 디스가이아의 경쾌함                     |

### 청각 (Audio)

| 항목         | 방향                                            |
| :----------- | :---------------------------------------------- |
| 월드 BGM     | 고딕 오케스트라 + 앰비언트 (월하의 야상곡 참조) |
| 아이템계 BGM | 긴장감 있는 전투 루프 (지층 깊어질수록 BPM 증가) |
| 허브 BGM     | 편안한 마을 테마                                |
| 타격음       | 무기별 고유 타격음, 묵직하고 임팩트 있는 사운드 |

### 스토리 톤 (Story Tone Progression)

```
[Act 1: 코미디] — 에르다의 기술 독백, 아이템계를 부업으로 취급, 오렌과의 케미
...
```
[허브: 안전/경쾌] → [월드 입구: 기대/긴장] → [탐험: 호기심/발견]
→ [보스: 긴장/도전] → [아이템계: 집중/파밍 쾌감] → [깊은 지층: 스릴/위험]
→ [모든 지층 클리어: 성취/환희] → [허브 복귀: 안도/자랑]
```
...
| 자동 사냥 Legendary 이상 드랍 | 직접 플레이의 가치 보존. Legendary 이상은 수동 전투에서만       |
| 이노센트 자동 사냥 획득       | 이노센트는 아이템계 직접 전투로만 획득                          |
...
| 핵심 동기   | "내가 직접 키운 장비"에 대한 애착, 숨겨진 구역 발견의 쾌감 |
...
| 플레이 패턴 | 매일 30분~1시간 (출퇴근 모바일 허브 접속 포함)  |
| 핵심 동기   | 이노센트 수집/합성, 거래소 경제 활동, 장비 자랑 |
...
| 플랫폼             | 지원 범위 | 조작 방식                   |
| :----------------- | :-------- | :-------------------------- |
| PC 웹 브라우저     | 풀 지원   | 키보드+마우스 / 게임패드    |
| 모바일 웹 브라우저 | 풀 지원   | 가상 패드 (Virtual Gamepad) |

### 크로스 플랫폼 설계 원칙

PC와 모바일에서 동일한 게임 경험을 제공한다. 이를 위해 조작 체계를 스킬 슬롯 기반으로 설계하여, 가상 패드로도 모든 콘텐츠를 플레이할 수 있도록 한다.

| 원칙                | 설명                                                                        |
| :------------------ | :-------------------------------------------------------------------------- |
| 스킬 슬롯 기반 전투 | 기본 공격 + 4~6개 스킬 슬롯. 복잡한 콤보 입력 대신 스킬 버튼 터치로 전투    |
| 가상 패드 최적화    | 왼쪽 D-Pad(이동) + 오른쪽 스킬 버튼 4~6개. 화면 가림 최소화                 |
| 조작 단순화         | 백스텝 등 정밀 타이밍 조작 제거. 스킬 쿨다운 기반 전투로 대체              |
| 자동 조준 보조      | 가장 가까운 적 방향으로 스킬 자동 조준 (터치 환경 보조)                     |
...
| Phase 3 | 베타       | 파티 플레이/무한 파밍이 작동하는가? | WebSocket 멀티, 아이템계 전 지층, 허브, 자동 사냥      |
...
- [ ] 모든 시스템 문서가 3-Space(World/ItemWorld/Hub) 중 작동 공간을 명시하는가?
- [ ] 순환 구조의 각 연결이 구체적 메커닉으로 뒷받침되는가?
- [ ] 핵심 금지 규칙이 모든 기획 결정에서 준수되는가?
- [ ] 이중 게이트(능력+스탯)가 탐험↔야리코미 순환을 강화하는가?
- [ ] 크로스 플랫폼(PC + 모바일) 설계 원칙이 명확히 정의되어 있는가?
