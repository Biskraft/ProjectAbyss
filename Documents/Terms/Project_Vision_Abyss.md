...
| 핵심 경험   | "이 검을 100층까지 클리어하면 Ancient으로 승급할 수 있다" → 층 클리어 → "이노센트 3마리 잡고 레어리티 승급!" |
...
| 핵심 경험   | "혼자선 70층 보스를 못 잡는다" → 파티 구성 → "탱커가 어그로 잡는 사이에 서포터가 버프!" |
...
| 아이템계 BGM | 긴장감 있는 전투 루프 (층 깊어질수록 BPM 증가)  |
| 허브 BGM     | 편안한 마을 테마                                |
...
→ [보스: 긴장/도전] → [아이템계: 집중/파밍 쾌감] → [깊은 층: 스릴/위험]
→ [100층 클리어: 성취/환희] → [허브 복귀: 안도/자랑]
```
...
| :------ | :--------- | :---------------------------------- | :----------------------------------------------------- |
| Phase 1 | 프로토타입 | 핵심 루프가 재미있는가?             | 이동/전투, 타일맵, 절차적 방 생성, 아이템계 미니(10층) |
