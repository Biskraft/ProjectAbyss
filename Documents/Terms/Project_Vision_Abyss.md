...
| 서사 방식 | 대사 0줄. 행동으로만 전달. 플레이어가 행동 패턴에서 의미를 읽는다 |
...
| 항목        | 정의                                                                                                          |
| :---------- | :------------------------------------------------------------------------------------------------------------ |
| 핵심 판타지 | 장인 -- 아이템 속에 들어가 이노센트를 사냥하고, 세상에 하나뿐인 최강 장비를 만든다. 300무기 x 5레어리티 = 1,500개의 고유한 아이템계 |
...
| **메조** (지층/구역) | 5-15분 | 지층 클리어, 보스 처치 | 영구 스탯 보너스, 이노센트, 레어리티 승급 기회 |
...
ECHORIS의 월드는 **대공동(The Shaft)**이라 불리는 거대한 수직 구조물이다. 에르다는 중앙 성채(Tier 2)에서 출발하여, 심연 전쟁의 상흔이 남은 7개 층위를 아래로 하강한다. 왜 내려가는지는 플레이어에게 설명되지 않는다 -- 에르다의 행동만이 단서다.
...
각 층위는 단순한 바이옴이 아니라 심연 전쟁(Era 3)의 상흔이 남은 역사적 장소다. 에르다는 대사 없이 하강한다. 층위가 깊어질수록 환경 자체가 이야기한다:
...
| :--- | :--: | :--- | :--- | :--- |
| **Blade** (단조 공방 한손 절삭기) | 3 | 3타 피니셔 강화 | 쉬지 않고 베면 점점 강해지는 단조 기사 | ATK |
...
스탯 공식: `FinalStat = BaseStat x (1 + level x 0.05) + permanentBonus + InnocentBonus`
...
| 분류 | 기능 | 획득 |
| :--- | :--- | :--- |
| atk 이노센트 | ATK 보너스 | 아이템계 지층 |
| def 이노센트 | DEF 보너스 | 아이템계 지층 |
| hp 이노센트 | HP 보너스 | 아이템계 지층 |

> exp%/luck 이노센트는 삭제됨 (2026-04-14). 3종만 유지.
...
| 아이템 왕 | 지층 3 마지막 방 | 장군의 1.5배 | 이노센트 슬롯 +1, 스탯 영구 +3 |
| 아이템 신 | 지층 4 마지막 방 | 왕의 2배 | 승급 재료, 스탯 영구 +5 |
| 아이템 대신 | 심연 최심층 | 신의 3배 | Ancient 전용 Anchor 패스 |

### 이노센트 (야생)

아이템계 지층에서 조우하는 야생 이노센트는 전투 대상이면서 보상이다. 도주형 행동 패턴: 플레이어를 감지하면 도주. 처치 시 복종 이노센트로 전환되어 장비에 귀속.
...
첫 30분은 대사 0줄. NPC, 퀘스트, 튜토리얼 텍스트 없음. 에르다의 행동을 관찰하며 세계와 규칙을 이해한다.
...
대사 0줄 원칙 유지. 단 3개 모멘트만 심볼 프롬프트 허용:
...
```
에르다는 대사 없이 행동한다. 플레이어는 그녀의 행동 패턴과 환경 단서에서 서사를 읽는다.
아이템 내부의 기억(이전 주인의 삶)이 유일한 서사 전달 수단이다.
층위가 깊어질수록 기억의 무게가 달라지고, 에르다의 행동이 변한다.
```

BLAME! 킬리 오마주 -- 고독한 여행자가 거대 구조물을 묵묵히 내려간다. 말하지 않기 때문에, 행동 하나하나가 의미를 갖는다.
...
| 금지 규칙                     | 이유                                                            |
| :---------------------------- | :-------------------------------------------------------------- |
| 언어 텍스트 튜토리얼          | 에르다 대사 0줄 원칙 유지. 심볼 프롬프트 3개 모멘트만 허용 (DEC-025) |
...
자동사냥, 거래소, 모바일, 시즌리셋, 스킬트리, 이노센트팜, 아이템전생, 패링, 허브(3->2-Space), NPC퀘스트, 대화시스템, 풍원소, 광원소, 도끼, 창, 채찍, 재귀진입(중첩)
...
| Phase 2 | 알파       | 성장/탐험 쾌감이 있는가?            | 장비/이노센트, 스탯+능력 게이트, 월드 연결, 보스       | **진행 중** |
