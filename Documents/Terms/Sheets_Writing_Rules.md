...
- 명명 규칙: ID는 `lower_snake_case`를 사용합니다. (예: `sword_common`, `room_castle_01`, `physical`)
...
| :--- | :--- | :--- |
| 캐릭터 스탯 | `Level` | `1`, `2`, `10` |
| 무기 | `WeaponID` | `sword_common`, `sword_mythic` |
...
| 구역 | `ZoneID` | `zone_castle`, `zone_catacombs` |
...
| 무기 타입 | Sword / Spear / Axe / Whip / Staff / Knuckle / Bow / Greatsword | Sword / Spear / Axe / Whip / Staff / Knuckle / Bow / GS |
| 스탯 | Strength / Intelligence / Dexterity / Vitality / Speed / Luck | STR / INT / DEX / VIT / SPD / LCK |
| 파생 스탯 | Hit Points / Mana Points / Attack / Defense / Resistance | HP / MP / ATK / DEF / RES |
| 레어리티 | Common / Uncommon / Rare / Legendary / Mythic | Common / Uncommon / Rare / Legendary / Mythic (줄이지 않음) |
...
| 시트 파일명 | 연결 GDD | 주요 내용 |
| :--- | :--- | :--- |
| `Content_Stats_Character_Base.csv` | SYS-LVL-01 | Lv별 기본 스탯 (HP, MP, 6대 스탯, 경험치) |
...
| `Content_Level_RoomTemplate.csv` | SYS-WLD-05, SYS-IW-02 | Room 템플릿 메타데이터 (크기, 출구, 난이도, 태그) |
...
| `Content_Level_Zone_Config.csv` | SYS-WLD-02 | 구역별 바이옴, 난이도, 스탯 게이트 요구치 |
| `Content_System_Skill_List.csv` | SYS-LVL-03 | 스킬별 MP 비용, 쿨다운, 배율, 효과 |
| `Content_System_IW_BossTable.csv` | SYS-IW-03 | 아이템계 보스 등급별 스탯, 등장 층수 |
| `Content_System_LevelExp_Curve.csv` | SYS-LVL-02 | Lv 1~100 경험치 커브, 전생 보너스 |
...
| 스탯 | 정수 (Integer) | `10`, `100`, `300` | HP, MP, STR 등 |
...
- **상호 참조**: 다른 시트의 데이터를 참조할 때는 원본 시트의 ID를 정확히 사용합니다. (예: 몬스터 드랍 테이블에서 `sword_common` 참조)
