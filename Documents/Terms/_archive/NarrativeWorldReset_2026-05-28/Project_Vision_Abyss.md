...
> **(DEC-046, 2026-05-24 재정의)** "거대 빌더가 누비는 수직 구조물을 탐험하고, 말을 거는 무기 속으로 들어가 한 사람의 잊혀진 인생을 복원하며, 다른 플레이어와 함께 싸우는 횡스크롤 온라인 액션 RPG"
>
> 이전 정의의 *"끝없이 강화하며"* 는 *"한 사람의 잊혀진 인생을 복원하며"* 로 대체. 강화 던전 정체성 폐기 — DEC-046 / `Documents/System/System_Memory_Core.md` 참조.
...
> **핵심 스토리 SSoT:** `Documents/Content/Content_Story_Synopsis.md` (CNT-STR-001) 참조.
> **프로젝트 이름의 어원·의미 SSoT:** `Documents/Content/Content_Project_Name_Etymology.md` (CNT-NME-001) 참조. "Echo + Iris — the eye that sees the echoes of the past." (Fina, 2026-05-24)
...
### 기둥 2: 아이템계 야리코미 (Item World Yarikomi) — DEC-046 재정의

| 항목        | 정의                                                                                                          |
| :---------- | :------------------------------------------------------------------------------------------------------------ |
| 핵심 판타지 | **회상가 (Recoverer)** -- 아이템 속에 들어가 100년 전 잊혀진 한 사람의 인생을 복원한다. 300무기 = 300명의 인생 = Identity Archive의 1,200+ 문장 컬렉션 |
| 핵심 경험   | "이 측량사의 인생을 끝까지 보고 싶다" -> 지층 클리어 -> "Fire 모멘트 문장 해금" -> "다음엔 케이블 운반자 이야기" |
| 설계 원칙   | 장비 내부 던전, **Memory Recovery 시스템 (강화 → 복원 전환)**, 인물 컬렉션(Identity Archive), Re-Dive 입체화 |
| 검증 질문   | "이 시스템이 *수치 야리코미* 가 아닌 *서사 야리코미* 의 깊이를 제공하는가?" |
| 2-Space     | Item World                                                                                                    |

> **2026-05-24 변경 (DEC-046):** "장인" → "회상가", "기억 단편 사냥" → "잊혀진 인생 복원", "최강 장비 제작" → "한 사람의 인생 완성", "Ancient 승급" → "진명 확정". 디스가이아 야리코미 메커닉 채택은 폐기.
...
```
  ...
  Tier 4: 지하 수로 / Veinway
     |
~~  ── 격벽 3: 안개 변신 (Mist Form) ──~~ **DEPRECATED 2026-05-18 (안개 변신 폐기 — SotN 카피)**
     |
...
### 장비 복원 단일 축 (DEC-046 재정의)

> **(DEC-046, 2026-05-24)** "장비는 드랍된 순간이 아니라, 플레이어가 직접 아이템 속 한 사람의 인생을 끝까지 복원한 순간에 본래의 힘이 깨어난다."
>
> 이전의 *3축 성장 (레벨 / 보스 보너스 / 레어리티 승급)* 은 폐기. **단일 진행 게이지 Memory Recovery (0-100%)** 로 통합.

| 축 | 메커닉 | 동력 |
| :--- | :--- | :--- |
| **Memory Recovery (0-100%)** | 보스 처치 시 25% 단위 점프 + 일반 활동 점진 누적 | "이 사람의 다음 페이지를 보고 싶다" |
| **이름 진화 (5 Stage)** | Recovery 단계에 따라 이름이 변함. Unknown → Survey Tool → Guild Survey Tool → The Last Wedge → Surveyor's Echo Wedge | "정체를 밝혀낸다" |
| **Memory Fragment 컬렉션** | 보스 처치 시 한 문장씩 영구 해금. Identity Archive에 등록 | "다음 인물의 이야기를 보고 싶다" |

스탯 공식 (DEC-046): `effectiveStat = baseStat × (0.4 + Recovery × 0.006) × rarityMultiplier × (1 + reDive × 0.05)`

> 폐기된 항목: 아이템 레벨 0-99, 아이템 EXP, 보스 영구 ATK +N, 레어리티 승급 (피티), Memory Shard 5색 기질 단편 시스템.

### Memory Fragment + Identity Trait 시스템 (DEC-046)

> **2026-05-24 변경:** 이전의 "기억 단편 (Memory Shard) — atk/def/hp 보너스, Wild/Subdued 전환, 5색 기질" 메커닉은 폐기. 모든 보스 보상은 **Memory Fragment (한 문장)** 와 **Identity Trait (정체성 결, 단편 효과)** 의 묶음으로 통합.

| 분류 | 기능 | 획득 |
| :--- | :--- | :--- |
| **Memory Fragment (문장)** | Identity Archive에 영구 등록되는 한 문장. 보상의 *서사적* 본체 | 아이템계 지층 보스 처치 시 1개씩 |
| **Identity Trait (정체성 결)** | Fragment에 귀속된 무기의 본질적 효과. 무기의 모든 스킬·효과는 결의 합 | Fragment 해금 즉시 자동 가동 |
| **Network Fragment (관계 단편)** | 두 인물 모두 100% 복원 시 해금되는 두 사람의 관계 1차 자료 | 인물 카테고리 100% 복원 |
| **Re-Dive Fragment** | 같은 인생의 다른 해석 단편 | 100% Recovery 후 재진입 시 |

> 폐기: atk/def/hp 기억 단편 3종, 5색 기질 (Forge/Iron/Rust/Spark/Shadow), Active/Passive 슬롯 역할, 단편 전이/합성/분해.
...
| T5 연구소 | 폭주한 실험체 | 3페이즈 듀얼 | 안개 변신 해금 | **DEPRECATED 2026-05-18 (안개 변신 폐기 — SotN 카피)**
...
| 아이템계    | 부식 강판 / 기억의 지층 단면 -- 무기 안에 압축된 기억 층이 방과 복도가 되는 세계. 팔레트 반전: 주황 배경 + 청록 디테일 |
