...
## 0.4. 스파이크 원칙 — 모든 설계의 전제 조건 (Spike Principle)
...
> **"아이템에 들어가면, 그 안에 살아있는 세계가 있다"** (DEC-033, 2026-04-25)

이것이 PA를 "또 하나의 픽셀아트 메트로베니아"가 아닌 유일무이한 게임으로 만드는 차별화 요소다. 모든 설계 결정은 이 스파이크를 강화하는 방향이어야 한다.

### 스파이크 검증 질문 (모든 GDD 문서에 적용)

새로운 시스템/피처를 설계할 때 반드시 다음 질문에 답해야 한다:

```yaml
spike_validation:
  q1: "이 시스템이 아이템계 진입 경험을 강화하는가?"
  q2: "이 시스템이 없으면 스파이크가 약해지는가?"
  q3: "이 시스템이 스파이크를 희석시키지 않는가?"
  verdict: "q1 또는 q2가 NO이고 q3이 YES이면 → 삭제 또는 후순위"
```
...
| 포지셔닝 | 위험도 | 대응 |
| :--- | :--- | :--- |
| "고딕 다크 판타지 MV" | 높음 — Blasphemous, Moonscars, Crowsworn과 동일 | 기억의 지층 비주얼 언어로 차별화 |
| "소울즈라이크 MV" | 높음 — Moonscars가 "영혼 없다" 비판을 받음 | 전투는 수단, 아이템계가 목적 |
| "웹 브라우저 치고 괜찮은" | 높음 — 7/10은 묻힘 | "URL 공유로 즉시 합류"를 웹 전용 스파이크로 |

### 참조

`Documents/Research/SpikeFeature_CompetitiveAnalysis_Research.md` — 스파이크 이론 + 경쟁 환경 상세 분석
...
* 재구축: "아이템계 + 메트로베니아 탐험 능력 활용 + 온라인 협동 = 2-Space 순환"
...
* 2-Space 연계: "월드에서 습득한 벽 타기 -> 아이템계의 수직 통로 등반 -> 숨겨진 기억 단편 방 발견"
...
| 문제 | 해결 방향 |
| :--- | :--- |
| 메트로베니아 탐험감 vs 온라인 멀티 | 2-Space 분리: 월드는 솔로, 아이템계는 1-2인 (Phase 3) / 최대 4인 (Phase 4+) |
...
* 볼드(`**`) 사용 제한:
  * 테이블 셀, 본문 강조에서 볼드 서식을 사용하지 않는다.
  * 이유: 테이블 셀 내 볼드는 렌더러별로 패딩/공백 처리 방식이 달라 서식이 깨진다.
  * 대안: 강조가 필요하면 문장 구조로 중요도를 드러내거나, 테이블의 열 제목을 활용한다.
  * 볼드 허용 예외:
    * 라벨형 볼드: `**키워드:** 설명` 형식의 정의 라벨은 허용한다
    * Blockquote 라벨: `> **설계 원칙:**` 형식의 인용 라벨은 허용한다
    * 테이블 셀 내 볼드: 금지 (기존 규칙 유지)
    * 본문 강조 볼드: `이것은 **핵심** 개념이다` 형식은 금지한다. 강조가 필요하면 문장 구조로 표현한다

  | 위치 | 사용 가능 여부 | 예시 |
  | :--- | :--- | :--- |
  | 라벨형 볼드 | 허용 | `**키워드:** 설명 텍스트` |
  | blockquote 라벨 | 허용 | `> **설계 원칙:** 내용` |
  | 본문 강조 | 금지 | "이것은 핵심 개념이다" (볼드 없이) |
  | 테이블 셀 | 금지 | 셀 내용은 평문으로 |
...
점진적 업데이트를 고려하여, 기획서는 "현재 시점에 구현되어야 할 최종 스펙"을 유지하는 것을 원칙으로 한다.
...
| Design | 상위 설계 철학, 분석, 원칙 | `Documents/Design/` | `Design_Architecture_2Space.md` |
