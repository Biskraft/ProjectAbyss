...
인벤토리 UI는 플레이어가 보유한 장비 아이템(최대 20개)을 5열 4행 그리드로 표시하고, 방향키와 단축키로 아이템을 선택/장착/비교/상세조회하는 패널이다. 화면 중앙에 반투명 오버레이와 함께 팝업되며 `I`키로 토글한다. **정보 밀도는 3단계 계층(Level 1: 그리드 / Level 2: Info Box / Level 3: Detail View)** 으로 분리되어, 기본 조작은 가볍게 유지하면서 복잡한 Memory Shard/Memory Strata 정보는 필요할 때만 확장된다.
...
| **Memory Shard 인디케이터** | `(1, 15)` — 좌하단 4×4 | `item.memory shards.filter(Boolean).length > 0` | **Subdued 중심** 하양 `0xFFFFFF`, **Wild만 존재** 시 빨강 `0xFF4444` |
...
| 6 | 구분선 | `—————————` | `0x4A4A6A` |
| 7 | Memory Shard 요약 | `"Memory Shards: {subdued+wild}/{slots}"` | 흰색 (Wild만 존재 시 빨강 tint) |
...
### 3.8 Anvil 모드 차이점 — 2단계 배치 시스템

인벤토리와 공통 그리드를 사용하지만, **앤빌 슬롯**이 우측에 추가되고 키 의미가 다르다.

> **설계 근거:** Diablo 3 Kanai's Cube의 "아이템 배치 → Transmute 확정" 2단계 UX.
> 플레이어가 아이템을 앤빌 슬롯에 "올려놓는" 물리적 행위를 시각적으로 체감하게 한다.
...
| 배치됨 | 아이템 아이콘 + 레어리티 색상 테두리 + 0.8초 맥동 |
| 위치 | 우측 Info 영역 중앙 |

#### 모드별 차이점

| 항목 | inventory 모드 | anvil 모드 (상태 1: 배치 전) | anvil 모드 (상태 2: 배치 후) |
| :--- | :--- | :--- | :--- |
| 타이틀 | "INVENTORY" | "FORGE" | "FORGE" |
| 앤빌 슬롯 | 없음 | 빈 슬롯 표시 (점선) | 아이템 배치됨 (맥동) |
| `C` 키 | Compare Mode | **Place on Anvil** | **DIVE 확정** |
| `ESC` 키 | 닫기 | 닫기 (월드 복귀) | **아이템 제거** (슬롯 비움 → 상태 1) |
| `X` 키 | Equip | 비활성 | 비활성 |
| `Arrow` 키 | 그리드 탐색 | 그리드 탐색 | **비활성** (앤빌 포커스) |
| `Z` 키 | Detail View | Detail View | Detail View (배치 아이템) |
| 하단 힌트 | `[Z]Detail [X]Equip [C]Compare` | `[C]Place [Z]Detail [ESC]Close` | `[C]DIVE [ESC]Remove [Z]Detail` |
| 장착 중 아이템 | 재장착 (무해) | **차단** — "Unequip first" 토스트 | -- |
| Compare Mode | 일반 동작 | 없음 (C키가 Place 역할) | 없음 |
| 닫기 | I/ESC 토글 | ESC | ESC = 아이템 제거 (닫기는 제거 후 다시 ESC) |
| UI 닫힘 시 안전망 | -- | -- | 앤빌 슬롯 아이템 자동 인벤토리 복귀 |
