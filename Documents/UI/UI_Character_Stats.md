...
| 스탯 분해 표시 (Base + Equip + Innocent = Final) | 미구현 |
...
- **성장의 시각화:** FinalStat = Base + Equip + Innocent 분해 표시는 "나의 강함이 어디서 왔는가"를 투명하게 보여준다. Disgaea 스탯 표 구조를 참조하여 성취감과 야리코미 동기를 동시에 충족한다.
...
| "Innocent" 행 | 8px | `0x44FF44` (복종 이노센트 기여분) |
...
| 상황 | 처리 방식 |
| :--- | :--- |
| 아무 장비도 착용하지 않은 상태로 화면 열기 | 모든 슬롯 "—" 표시. 스탯 패널에 BaseStat만 표시. EquipStat/InnocentBonus 분해 행 = 0으로 표시 |
...
**스탯 패널:**
- [ ] ATK/INT/HP 수치가 FinalStat 공식(`BaseStat + EquipStat + InnocentBonus`)과 일치함
...
- [ ] 슬롯 선택 시 스탯 분해(Base/Equip/Innocent/Final) 팝업이 스탯 패널에 표시됨
...
- [ ] 스탯 분해 표시(Base/Equip/Innocent)를 통해 "이노센트를 더 강화하면 ATK가 얼마나 오르는지" 즉시 가늠 가능
